...
const CACHE_NAME = "meai-editor-v1";
const STATIC_CACHE_NAME = "meai-editor-static-v1";
const DYNAMIC_CACHE_NAME = "meai-editor-dynamic-v1";
...
async function getCacheStatus() {
  ...
  for (const name of cacheNames) {
    if (name.startsWith("meai-editor-")) {
  ...
  return {
    cacheNames: cacheNames.filter((n) => n.startsWith("meai-editor-")),
    totalEntries,
    version: CACHE_NAME,
  };
}
...
/**
 * Clear all MeAI Editor caches
 */
async function clearAllCaches() {
  const cacheNames = await caches.keys();
  await Promise.all(
    cacheNames
      .filter((name) => name.startsWith("meai-editor-"))
