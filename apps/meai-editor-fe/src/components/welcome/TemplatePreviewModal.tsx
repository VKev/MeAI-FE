...
export const TemplatePreviewModal: React.FC<TemplatePreviewModalProps> = ({ template, onClose, onApply }) => {
  ...
  const handleValueChange = useCallback((placeholderId: string, value: unknown, type: ExtendedPlaceholderType) => {
    setValues((prev) => ({
      ...prev,
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      for (const placeholder of template.placeholders) {
        if (!effectiveValues[placeholder.id] && placeholder.defaultValue !== undefined) {
      ...
      const { project, result, textClips } = templateEngine.applyScriptableTemplate(template, effectiveValues);
      ...
      if (titleEngine && textClips.length > 0) {
        for (const textClip of textClips) {
          const placeholder = template.placeholders.find((p) => p.id === textClip.placeholderId);
          const isTitle =
          ...
          titleEngine.createTextClip({
            ...
            transform: textClip.transform as Partial<import('@meai-editor/core').Transform>,
            animation: {
              preset: 'fade',
  ...
  }, [getTemplateEngine, getTitleEngine, loadProject, template, values, onApply, track]);
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className='max-w-3xl max-h-[90vh] p-0 gap-0 bg-background-secondary border-border overflow-hidden flex flex-col'>
        <DialogHeader className='p-5 border-b border-border space-y-0 shrink-0'>
          <DialogTitle className='text-lg font-semibold text-text-primary'>{template.name}</DialogTitle>
        ...
        <div className='flex-1 overflow-y-auto p-5'>
          <div className='grid md:grid-cols-2 gap-6'>
            <div>
              ...
              {template.description && <p className='text-sm text-text-secondary mb-4'>{template.description}</p>}

              {template.scenes && template.scenes.length > 0 && (
                <div className='space-y-2'>
                  <h3 className='text-xs font-medium text-text-muted uppercase tracking-wide'>Scenes</h3>
                  <div className='flex flex-wrap gap-2'>
                    {template.scenes.map((scene) => (
                      ...
                      >
                        <div className='w-2 h-2 rounded-full' style={{ backgroundColor: scene.color || '#22c55e' }} />
                        <span className='text-text-secondary'>{scene.label}</span>
                        <span className='text-text-muted'>({formatDuration(scene.endTime - scene.startTime)})</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className='space-y-4'>
              <h3 className='text-sm font-medium text-text-primary'>Customize Template</h3>
                ...
                <div className='space-y-4'>
                  {groupedPlaceholders.main.map((placeholder) => (
                    <PlaceholderInput
                      ...
                      onChange={(value) => handleValueChange(placeholder.id, value, placeholder.type)}
                    />
                  ))}
                </div>
              )}
        ...
        <div className='p-5 border-t border-border flex items-center justify-end gap-3 shrink-0'>
          ...
          <Button onClick={handleApply} disabled={isApplying} className='shadow-glow'>
            {isApplying ? (
              <>
...
const PlaceholderInput: React.FC<PlaceholderInputProps> = ({ placeholder, value, onChange }) => {
      ...
      case 'boolean':
        return (
          <div className='flex items-center gap-3'>
            ...
            <Label className='text-sm text-text-secondary cursor-pointer'>{placeholder.description || 'Enabled'}</Label>
          </div>
        );
  ...
  return (
    <div className='space-y-2'>
      <label className='flex items-center gap-2'>
        ...
        <span className='text-sm font-medium text-text-primary'>{placeholder.label}</span>
        {placeholder.required && <span className='text-red-400 text-xs'>*</span>}
      </label>
