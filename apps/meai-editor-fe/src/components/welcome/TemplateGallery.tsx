...
export const TemplateGallery: React.FC<TemplateGalleryProps> = ({ onTemplateApplied }) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<SocialMediaCategory | 'all'>('all');
  const [selectedTemplate, setSelectedTemplate] = useState<ScriptableTemplate | null>(null);
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      ...
      try {
        ...
        const cloudTemplates = await templateCloudService.listScriptableTemplates();

        const allTemplates = [
          ...(builtinTemplates.map((t) => {
            const placeholderClipMap = new Map<string, { clipId: string; trackId: string }>();
        ...
        const unique = Array.from(new Map(allTemplates.map((t) => [t.id, t])).values());
  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (selectedCategory !== 'all') {
      result = result.filter((t) => t.socialCategory === selectedCategory || t.category === selectedCategory);
    }
    ...
    <div className='space-y-6'>
      ...
      {filteredTemplates.length === 0 ? (
        <div className='flex flex-col items-center justify-center py-16'>
          ...
          <p className='text-base font-medium text-text-primary mb-1'>No templates found</p>
          <p className='text-sm text-text-muted'>Try adjusting your search or filter</p>
        </div>
      ) : (
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
          {filteredTemplates.map((template) => (
            <TemplateCard key={template.id} template={template} onClick={() => handleSelectTemplate(template)} />
          ))}
        </div>
      )}

      {showPreview && selectedTemplate && (
        <TemplatePreviewModal template={selectedTemplate} onClose={handleClosePreview} onApply={handleApplyTemplate} />
      )}
    </div>
  );
};
