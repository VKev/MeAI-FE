...
import { Play, Clock, Layers, Smartphone, Monitor, Square, Star, Crown } from 'lucide-react';
import type { ScriptableTemplate, SocialMediaCategory } from '@meai-editor/core';
import { SOCIAL_MEDIA_PRESETS } from '@meai-editor/core';
...
export const TemplateCard: React.FC<TemplateCardProps> = ({ template, onClick }) => {
  ...
  const gradient = CATEGORY_GRADIENTS[category] || 'from-primary to-emerald-500';
        ...
        {template.tags.length > 0 && (
          <div className='flex items-center gap-1.5 mt-2 overflow-hidden'>
            {template.tags.slice(0, 2).map((tag) => (
              <span key={tag} className='px-1.5 py-0.5 text-[10px] bg-background rounded text-text-muted'>
                {tag}
              </span>
            ))}
            {template.tags.length > 2 && (
              <span className='text-[10px] text-text-muted'>+{template.tags.length - 2}</span>
            )}
          </div>
        )}
      </div>
