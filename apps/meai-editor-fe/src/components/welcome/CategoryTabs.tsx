...
import { SOCIAL_MEDIA_CATEGORY_INFO, type SocialMediaCategory } from '@meai-editor/core';
...
export const CategoryTabs: React.FC<CategoryTabsProps> = ({ selectedCategory, onSelectCategory, categoryStats }) => {
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 overflow-x-auto pb-1'>
        ...
        >
          <LayoutGrid size={14} />
          All
          <span className={`text-xs ${selectedCategory === 'all' ? 'text-black/60' : 'text-text-muted'}`}>
            {categoryStats.all || 0}
          </span>
        </button>
        ...
        {PLATFORMS.map((platform) => {
          ...
          const isActive = categories.includes(selectedCategory as SocialMediaCategory);
          ...
          return (
            ...
            >
              ...
              {platform}
              {count > 0 && (
                <span className={`text-xs ${isActive || isExpanded ? 'text-black/60' : 'text-text-muted'}`}>
                  {count}
                </span>
              )}
            </button>
      ...
      {expandedPlatform && PLATFORM_CATEGORIES[expandedPlatform].length > 1 && (
        <div className='flex items-center gap-2 pl-2 overflow-x-auto pb-1'>
          {PLATFORM_CATEGORIES[expandedPlatform].map((category) => {
            const info = SOCIAL_MEDIA_CATEGORY_INFO.find((c) => c.id === category);
