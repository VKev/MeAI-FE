...
export const RecoveryDialog: React.FC<RecoveryDialogProps> = ({ saves, onRecover, onDismiss, onClearAll }) => {
      ...
      <DialogContent className='max-w-md p-0 gap-0 bg-background-secondary border-border overflow-hidden'>
        <DialogHeader className='p-5 border-b border-border space-y-0'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <DialogTitle className='text-base font-semibold text-text-primary'>Recover Your Work</DialogTitle>
        ...
        <div className='p-5'>
          ...
          >
            <div className='flex items-center gap-3 mb-3'>
              ...
              <span className='font-medium text-text-primary truncate'>{mostRecent.projectName}</span>
            </div>
            <div className='flex items-center gap-2 text-sm text-text-muted'>
              <Clock className='w-4 h-4 shrink-0' />
              <span>Last saved {formatTimeAgo(mostRecent.timestamp)}</span>
              <span className='text-text-muted/50'>•</span>
              <span className='text-text-muted/70 truncate'>{formatDate(mostRecent.timestamp)}</span>
            </div>
          </button>
                  ...
                  >
                    <div className='flex items-center justify-between gap-2'>
                      <div className='flex-1 min-w-0'>
                        ...
                        <div className='text-xs text-text-muted mt-1'>{formatDate(save.timestamp)}</div>
                      </div>
                      <div className='text-xs text-text-muted/70 shrink-0'>{formatTimeAgo(save.timestamp)}</div>
                    </div>
                  </button>
                ))}
              </CollapsibleContent>
