...
import { Smartphone, Monitor, Square, ChevronRight, Check, Info } from 'lucide-react';
import { Button, Input, Label } from '@meai-editor/ui';
...
const PRESET_GROUPS: PresetGroup[] = [
  {
    ...
    presets: ['tiktok', 'instagram-reels', 'instagram-stories', 'youtube-shorts']
  },
  {
    platform: 'Square (1:1)',
...
export const StartFromScratch: React.FC<StartFromScratchProps> = ({ onProjectCreated }) => {
  const createNewProject = useProjectStore((state) => state.createNewProject);
  const updateSettings = useProjectStore((state) => state.updateSettings);
  const { track } = useAnalytics();
  const [selectedPreset, setSelectedPreset] = useState<SocialMediaCategory>('youtube-video');
  ...
  }, [createNewProject, updateSettings, preset, projectName, info, onProjectCreated, track, selectedPreset]);
  ...
  return (
    <div className='space-y-6'>
      ...
      <div>
        <h3 className='text-sm font-medium text-text-primary mb-4'>Select Format</h3>
              ...
              <div key={group.platform} className='space-y-3'>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  {group.presets.map((presetId) => {
                    const presetInfo = SOCIAL_MEDIA_CATEGORY_INFO.find((c) => c.id === presetId);
      ...
      <div className='flex items-start gap-3 p-4 bg-background-tertiary rounded-xl border border-border'>
        ...
        <div>
          <p className='text-sm font-medium text-text-primary'>{info?.name || selectedPreset} Format</p>
          <p className='text-xs text-text-muted mt-1'>
            {preset.width}×{preset.height}px • {preset.frameRate || 30}fps
            {preset.maxDuration && ` • Max ${preset.maxDuration}s`}
            {preset.recommendedDuration && ` • Recommended ${preset.recommendedDuration}s`}
          </p>
          {preset.safeZone && (
            <p className='text-xs text-text-muted mt-0.5'>
              Safe zone: {preset.safeZone.top}px top, {preset.safeZone.bottom}px bottom
            </p>
          )}
        </div>
      </div>
      ...
      <div className='flex items-center justify-end gap-3'>
        <Button onClick={handleCreate} disabled={isCreating} className='shadow-glow'>
          {isCreating ? (
            <>
