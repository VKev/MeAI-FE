...
import type { Transform, FitMode, Clip, EditingTemplatePrimitive } from '@meai-editor/core';
import {
  ChromaKeyEngine,
...
import { EditingTemplateControls, mergeEditingTemplateControlValues } from './panels/EditingTemplateControls';
import {
...
export const InspectorPanel: React.FC = () => {
  ...
  const [expandedRecipeApplicationId, setExpandedRecipeApplicationId] = useState<string | null>(null);
  ...
  const appliedEditingTemplates = selectedTimelineClip?.metadata?.appliedTemplates || [];
  const handleRecipeControlChange = useCallback(
    (applicationId: string, controlId: string, value: EditingTemplatePrimitive) => {
  ...
  const handleToggleRecipeControls = useCallback(
    (applicationId: string, templateId: string, controlValues?: Record<string, unknown>) => {
      ...
      setExpandedRecipeApplicationId((current) => (current === applicationId ? null : applicationId));
      setRecipeControlValues((current) =>
        current[applicationId]
          ? current
          : {
  ...
  const handleUpdateRecipeControls = useCallback(
    (applicationId: string, templateId: string, controlValues?: Record<string, unknown>) => {
      ...
      const nextControlValues =
        recipeControlValues[applicationId] || mergeEditingTemplateControlValues(template, controlValues);
      const updated = updateEditingTemplateApplication(selectedTimelineClip.id, applicationId, nextControlValues);
    ...
    [getEditingTemplate, recipeControlValues, selectedTimelineClip, updateEditingTemplateApplication]
  );
          ...
          <>
            ...
            {showVideoControls &&
              ...
              (appliedEditingTemplates.length > 0 ||
                (selectedTimelineClip.effects && selectedTimelineClip.effects.length > 0)) && (
                <Section
                ...
                >
                  <div className='space-y-2'>
                    {appliedEditingTemplates.map((application) => {
                      ...
                      const isExpanded = expandedRecipeApplicationId === application.applicationId;
                      const currentControlValues = template
                        ? recipeControlValues[application.applicationId] ||
                          mergeEditingTemplateControlValues(template, application.controlValues)
                        : undefined;
                        ...
                        >
                          <div className='flex items-center justify-between gap-2'>
                            <div className='min-w-0 flex-1 flex items-center gap-2'>
                              ...
                              <p className='truncate text-[11px] font-medium text-text-primary'>{application.name}</p>
                                ...
                                onClick={() => {
                                  ...
                                  if (!removed) {
                                    toast.error(
                                      ...
                                      'The recipe could not be removed from this clip.'
                                    );
                                    return;
                                  }
                                  setRecipeControlValues((current) => {
                            ...
                            <div className='mt-2 space-y-3 rounded-lg border border-border/80 bg-background-secondary/80 p-2.5'>
                              <EditingTemplateControls
                                ...
                                onChange={(controlId, value) =>
                                  handleRecipeControlChange(application.applicationId, controlId, value)
                                }
                              />
                    ...
                    {selectedTimelineClip.effects
                      ...
                      .map((effect: { id: string; type: string; enabled?: boolean }) => (
                        ...
                        >
                          ...
                          <span
                            className={`text-[9px] font-medium ${effect.enabled !== false ? 'text-green-400' : 'text-text-muted'}`}
                          >
