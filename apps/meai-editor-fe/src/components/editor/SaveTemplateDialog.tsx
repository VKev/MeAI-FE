...
export const SaveTemplateDialog: React.FC<SaveTemplateDialogProps> = ({ isOpen, onClose }) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (saveLocation === 'cloud') {
        const result = await templateCloudService.uploadTemplate(templateWithMeta);
        if (!result.success) {
  ...
  }, [name, description, category, tags, author, saveLocation, project, getTemplateEngine, getGraphicsEngine, onClose]);
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className='max-w-lg p-0 gap-0 bg-background border-border overflow-hidden'>
        <DialogHeader className='p-4 border-b border-border space-y-0'>
          <DialogTitle className='text-lg font-semibold text-text-primary'>Save as Template</DialogTitle>
        </DialogHeader>
        ...
        <div className='p-4 space-y-4 max-h-[70vh] overflow-y-auto'>
          {success && (
            <div className='flex items-center gap-2 p-3 bg-green-500/10 border border-green-500/30 rounded-lg'>
              ...
              <span className='text-sm text-green-400'>Template saved successfully!</span>
            </div>
          )}
          ...
          <div className='space-y-2'>
            ...
            <Input
              ...
              maxLength={50}
            />
            <p className='text-[10px] text-text-muted'>{name.length}/50 characters</p>
          </div>
          ...
          <div className='space-y-2'>
            ...
            <textarea
              ...
              maxLength={500}
            />
            <p className='text-[10px] text-text-muted'>{description.length}/500 characters</p>
          </div>
          ...
          <div className='space-y-2'>
            <Label className='text-xs font-medium text-text-secondary'>Save Location</Label>
        ...
        <div className='flex items-center justify-end gap-2 p-4 border-t border-border'>
          ...
          <Button onClick={handleSave} disabled={isSaving || !name.trim() || !description.trim()}>
            {isSaving ? (
              <>
