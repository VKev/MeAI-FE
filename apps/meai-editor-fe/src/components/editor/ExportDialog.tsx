...
export const ExportDialog: React.FC<ExportDialogProps> = ({
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<string | null>('recommended');
  const [selectedPreset, setSelectedPreset] = useState<PlatformExportPreset | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      getDeviceProfile().then((profile) => {
        ...
        setCodecRecommendations(getCodecRecommendations(profile, { width: projectWidth, height: projectHeight }));
      });
    }
  ...
  useEffect(() => {
    ...
    const settings =
      activeTab === 'presets' && selectedPreset ? (selectedPreset.settings as VideoExportSettings) : customSettings;
  ...
  const handleRunBenchmark = useCallback(async () => {
    ...
    try {
      ...
      setCodecRecommendations(getCodecRecommendations(updatedProfile, { width: projectWidth, height: projectHeight }));
    } catch (error) {
  ...
  const recommendedForVideo = getRecommendedPresetsForAspectRatio(presets, aspectType);
      ...
      <DialogContent className='max-w-4xl max-h-[85vh] p-0 gap-0 bg-background-secondary border-border overflow-hidden flex flex-col'>
        <DialogHeader className='p-4 border-b border-border bg-background-tertiary space-y-0'>
          <div className='flex items-center gap-3'>
            ...
            <DialogTitle className='text-lg font-bold text-text-primary'>Export Video</DialogTitle>
          </div>
        </DialogHeader>
        ...
        >
          ...
          <TabsContent value='presets' className='flex-1 overflow-hidden flex mt-0'>
            <div className='w-48 border-r border-border overflow-y-auto'>
              ...
              >
                ...
                <span className='text-[10px] text-text-muted mt-0.5 ml-5'>{getAspectRatioLabel(aspectType)}</span>
              </button>
                ...
                {filteredPresets.map((preset) => (
                  ...
                  >
                    <div className='flex items-start justify-between mb-2'>
                      ...
                      {preset.recommended && <Star size={12} className='text-yellow-500 fill-yellow-500' />}
                    </div>
                    <p className='text-[10px] text-text-muted mb-2'>{preset.description}</p>
          ...
          <TabsContent value='custom' className='flex-1 overflow-y-auto p-4 mt-0'>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                <label className='block text-xs font-medium text-text-secondary mb-2'>Codec</label>
                <Select
              ...
              <div>
                <label className='block text-xs font-medium text-text-secondary mb-2'>Resolution</label>
                <Select
              ...
              <div>
                <label className='block text-xs font-medium text-text-secondary mb-2'>Frame Rate</label>
                <Select
              ...
              <div>
                <Label className='block text-xs font-medium text-text-secondary mb-2'>Bitrate (kbps)</Label>
                <Input
                  type='number'
              ...
              <div className='col-span-2'>
                <label className='block text-xs font-medium text-text-secondary mb-2'>Audio Settings</label>
                    ...
                    onValueChange={(value) =>
                      setCustomSettings({
                        ...
                        audioSettings: {
                          ...
                          format: value as 'mp3' | 'wav' | 'aac' | 'flac' | 'ogg'
                        }
                      })
                    }
                  >
              ...
              <div className='col-span-2 border-t border-border pt-4 mt-2'>
                ...
                {customSettings.upscaling?.enabled && (
                  <div className='space-y-3 pl-6'>
                    <div>
                      <label className='block text-[10px] text-text-muted mb-1.5'>Quality Mode</label>
                    ...
                    <div>
                      <Label className='block text-[10px] text-text-muted mb-1.5'>Sharpening</Label>
                      <Slider
                      ...
                      <div className='flex justify-between text-[10px] text-text-muted mt-1'>
                        <span>None</span>
                        <span>{Math.round((customSettings.upscaling?.sharpening ?? 0.3) * 100)}%</span>
                        <span>Max</span>
                      </div>
                    </div>

                    <p className='text-[10px] text-text-muted'>
                      Enhance quality when exporting to higher resolutions than source. Uses edge-directed interpolation
                      for sharper details.
                    </p>
                  </div>
                )}
              ...
              {timeEstimate && (
                <div className='flex items-center gap-3'>
                  <div className='flex items-center gap-1.5 text-xs'>
                    ...
                    <span className='font-medium text-text-primary'>{timeEstimate.formatted}</span>
                  </div>
            ...
            {showDeviceInfo && (
              <div className='mt-3 pt-3 border-t border-border/50 grid grid-cols-3 gap-4 text-[10px]'>
                <div>
                  ...
                  <p className='text-text-primary font-medium'>{deviceProfile.cpu.cores} cores</p>
                </div>
                <div>
        ...
        <div className='flex items-center justify-between p-4 border-t border-border bg-background-tertiary'>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  {formatFileSize(
                    activeTab === 'presets' && selectedPreset
                      ? (selectedPreset.settings as VideoExportSettings).bitrate || 8000
                      : customSettings.bitrate,
                    duration
                  )}
                </div>
                {timeEstimate &&
                  deviceProfile?.encoding[customSettings.codec as keyof typeof deviceProfile.encoding]?.hardware && (
          ...
          <div className='flex items-center gap-2'>
            ...
            <Button onClick={handleExport} disabled={activeTab === 'presets' && !selectedPreset}>
              <Play size={16} />
              Start Export
            </Button>
          </div>
