          ...
          >
            ...
            <div className='flex-1 min-w-0'>
              ...
              <div className='flex items-center gap-1.5 text-[9px] text-text-muted'>
                ...
                {(item.metadata?.duration || formatResolution()) && formatFileSize(item.metadata?.fileSize) && (
                  <span>•</span>
                )}
            ...
            {isSelected && <div className='w-2 h-2 bg-primary rounded-full shadow-[0_0_8px_#22c55e] flex-shrink-0' />}
          </div>
        </ContextMenuTrigger>
        <ContextMenuContent>
          {item.type === 'image' && onKieAI && (
      ...
      <ContextMenuTrigger asChild>
        <div className='flex flex-col'>
          ...
          <div className='mt-1.5 px-0.5'>
            <div
              className={`text-[10px] truncate font-medium ${isSelected ? 'text-primary' : 'text-text-primary'}`}
              title={item.name}
            >
              {item.name}
            </div>
            {viewMode === 'large' && (
              <div className='flex items-center gap-1.5 text-[9px] text-text-muted mt-0.5'>
                {formatResolution() && <span>{formatResolution()}</span>}
                {formatResolution() && formatFileSize(item.metadata?.fileSize) && <span>•</span>}
                {formatFileSize(item.metadata?.fileSize) && <span>{formatFileSize(item.metadata?.fileSize)}</span>}
              </div>
            )}
          </div>
...
const EmptyState: React.FC<{ onImport: () => void }> = ({ onImport }) => (
  <div className='flex-1 flex flex-col items-center justify-center p-8 text-center'>
    ...
    <p className='text-sm text-text-secondary mb-2 font-medium'>No media imported</p>
    <p className='text-xs text-text-muted mb-6'>Drag files here or click to import</p>
    <button
      onClick={onImport}
...
export const AssetsPanel: React.FC = () => {
  ...
  const missingAssetsCount = mediaItems.filter((item) => item.isPlaceholder).length;
    ...
    async (files: FileList | null) => {
      ...
      try {
        for (let i = 0; i < fileArray.length; i++) {
          ...
          setImportProgress(`Importing ${file.name} (${i + 1}/${fileArray.length})...`);
              ...
              <div className='px-4 py-4'>
                <div className='mb-6'>
                  ...
                  <div className='flex gap-1.5 mb-3 flex-wrap'>
                    {(['all', 'solid', 'gradient', 'mesh', 'pattern'] as const).map((cat) => (
                      <button
                        key={cat}
                ...
                <div className='mb-6'>
                  <h4 className='text-xs font-medium text-text-secondary mb-3'>Shapes</h4>
                  <div className='grid grid-cols-4 gap-2'>
                    {[
                      {
                ...
                <div className='mb-6'>
                  <h4 className='text-xs font-medium text-text-secondary mb-3'>SVG Import</h4>
                  <button
                    onClick={() => {
                      const input = document.createElement('input');
                      input.type = 'file';
                  ...
                  >
                    <FileCode size={16} className='text-text-secondary group-hover:text-primary transition-colors' />
                    <span className='text-xs text-text-secondary group-hover:text-text-primary'>Import SVG File</span>
                  </button>
                </div>

                <div className='mb-6'>
                  <h4 className='text-xs font-medium text-text-secondary mb-3'>Stickers & Emojis</h4>
                  <div className='grid grid-cols-4 gap-2'>
                    {['😀', '🎉', '❤️', '⭐', '🔥', '👍', '🎬', '🎵'].map((emoji, i) => (
                      <button
                        key={i}
                        onClick={async () => {
                          ...
                          if (newGraphicsTrack) {
                            ...
                            const clip = stickerLibrary.createEmojiClip(emojiItem, newGraphicsTrack.id, 0, 5);
          ...
          <div className='min-h-0 flex-1 border-t border-border/70'>
            <ScrollArea className='min-h-0 flex-1'>
              <div className='px-4 py-4 space-y-3'>
                ...
                >
                  <span className='text-lg font-bold text-text-primary'>Add Title</span>
                  <p className='text-xs text-text-muted mt-1'>Click to add text to timeline</p>
                </button>
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (newTextTrack) {
                          createTextClip(newTextTrack.id, 0, preset.text, 5, preset.style);
                        }
                      }}
      ...
      <div className='flex-1 flex flex-col min-w-0 h-full bg-background-secondary relative'>
        ...
        <div className='px-5 py-4 flex items-center justify-between border-b border-border/40 shrink-0'>
          ...
          <div className='flex gap-1 shrink-0'>
            {activeTab === 'media' && <IconButton icon={Plus} onClick={triggerFileInput} title='Import media' />}
          </div>
        </div>
        ...
        <div className='flex-1 min-h-0 relative flex flex-col overflow-hidden'>{renderSectionContent(activeTab)}</div>
      </div>
