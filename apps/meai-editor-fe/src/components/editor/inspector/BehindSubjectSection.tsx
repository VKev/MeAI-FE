...
export const BehindSubjectSection: React.FC<BehindSubjectSectionProps> = ({ clipId }) => {
  const getTitleEngine = useEngineStore((state) => state.getTitleEngine);
  const updateTextBehindSubject = useProjectStore((state) => state.updateTextBehindSubject);
    ...
    <div className='space-y-3'>
      <div className='flex items-center justify-between'>
        <div className='flex-1'>
          ...
          <p className='text-[9px] text-text-muted'>Text appears behind people in the video</p>
        </div>
        {isLoading ? (
      ...
      {isLoading && <p className='text-[9px] text-text-muted'>Loading AI model...</p>}
