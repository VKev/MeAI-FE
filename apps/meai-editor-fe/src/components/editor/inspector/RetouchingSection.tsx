...
const ToolButton: React.FC<{
  ...
  >
    <div
      className={`p-2 rounded-lg ${isActive ? 'bg-primary text-white' : 'bg-background-secondary text-text-secondary'}`}
    >
      {icon}
    </div>
    <div className='text-left'>
      <span className={`text-[11px] font-medium block ${isActive ? 'text-primary' : 'text-text-primary'}`}>
        {label}
      </span>
    ...
    <div className='flex items-center justify-center p-4 bg-background-tertiary rounded-lg'>
      ...
      >
        <div
          ...
          style={{
            background: `radial-gradient(circle, rgba(59,130,246,1) ${hardness * 100}%, rgba(59,130,246,0) 100%)`
          }}
        />
      </div>
  ...
  if (!source) {
    return (
      <div className='p-3 bg-background-tertiary rounded-lg text-center'>
        ...
        <p className='text-[10px] text-text-muted'>Alt+Click to set clone source</p>
      </div>
    );
  }
  ...
  return (
    <div className='p-3 bg-background-tertiary rounded-lg'>
      <div className='flex items-center justify-between'>
        ...
        <button onClick={onClear} className='text-[9px] text-text-muted hover:text-error transition-colors'>
          Clear
        </button>
      </div>
      <div className='mt-2 flex items-center gap-4'>
        <span className='text-[9px] text-text-muted'>
          X: <span className='text-text-primary font-mono'>{Math.round(source.x)}</span>
        </span>
        <span className='text-[9px] text-text-muted'>
          Y: <span className='text-text-primary font-mono'>{Math.round(source.y)}</span>
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='space-y-2'>
        <span className='text-[10px] text-text-secondary font-medium'>Retouching Tools</span>
        <div className='space-y-2'>
          {tools.map((tool) => (
            <ToolButton
              key={tool.id}
      ...
      {activeTool === 'cloneStamp' && (
        <div className='space-y-2'>
          <span className='text-[10px] text-text-secondary font-medium'>Clone Source</span>
          <CloneSourceIndicator source={cloneSource} onClear={onClearCloneSource} />
        </div>
      )}

      {/* Brush Settings */}
      <div className='space-y-3 p-3 bg-background-tertiary rounded-lg'>
        <span className='text-[10px] text-text-secondary font-medium'>Brush Settings</span>
      ...
      <div className='p-3 bg-background-secondary rounded-lg border border-border'>
        <div className='flex items-start gap-2'>
          ...
          <div>
            <span className='text-[10px] text-text-primary font-medium block'>How to use</span>
