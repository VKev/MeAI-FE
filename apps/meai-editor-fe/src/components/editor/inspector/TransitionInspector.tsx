...
const TransitionPreview: React.FC<{
  ...
  return (
    <div className='relative w-full h-8 rounded overflow-hidden bg-background-secondary mb-2'>
      <div className='absolute inset-0 bg-primary/20' style={{ ...styles.clipA, transition: 'none' }} />
      <div className='absolute inset-0 bg-purple-500/30' style={{ ...styles.clipB, transition: 'none' }} />
  ...
  return (
    <button
      ...
      className={`p-3 rounded-lg border transition-all text-left ${
        isSelected ? 'bg-primary/20 border-primary' : 'bg-background-tertiary border-border hover:border-text-secondary'
      }`}
    >
      <TransitionPreview type={typeInfo.type} isPlaying={isHovered || isSelected} />
      <div className='flex items-center justify-between mb-1'>
        <span className={`text-[10px] font-medium ${isSelected ? 'text-primary' : 'text-text-primary'}`}>
          {typeInfo.name}
        </span>
...
export const TransitionInspector: React.FC<TransitionInspectorProps> = ({
  ...
  const transitionTypes = useMemo(() => bridge.getAvailableTransitionTypes(), []);

  // Local state for creating new transitions
  const [selectedType, setSelectedType] = useState<TransitionType>(transition?.type || 'crossfade');
  ...
  const handleDurationChange = useCallback(
    (newDuration: number) => {
      const clampedDuration = validation.maxDuration ? Math.min(newDuration, validation.maxDuration) : newDuration;
  ...
  const handleCreate = useCallback(() => {
    const result = bridge.createTransition(clipA, clipB, selectedType, duration, params);
    ...
    <div className='space-y-4'>
      ...
      <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg border border-border'>
        <div className='flex-1 text-center'>
          ...
          <p className='text-[10px] text-text-primary truncate'>{clipA.id.substring(0, 12)}...</p>
        </div>
        <ArrowRight size={14} className='text-text-muted' />
        <div className='flex-1 text-center'>
          <p className='text-[9px] text-text-muted'>To</p>
          <p className='text-[10px] text-text-primary truncate'>{clipB.id.substring(0, 12)}...</p>
        </div>
      </div>
      ...
      <div className='space-y-2'>
        <span className='text-[10px] text-text-secondary font-medium'>Transition Type</span>
      ...
      {selectedTypeInfo?.hasCustomParams && (
        <div className='space-y-3 pt-2 border-t border-border'>
          <span className='text-[10px] text-text-secondary font-medium'>Parameters</span>
          {renderTypeParams()}
        </div>
      )}
      ...
      {!validation.valid && validation.error && (
        <p className='text-[10px] text-red-400 text-center'>{validation.error}</p>
      )}
    </div>
  );
};
