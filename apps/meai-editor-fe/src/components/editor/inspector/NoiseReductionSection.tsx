...
import { loadAudioBuffer, type AudioLoadProgress } from '../../../utils/load-audio-buffer';
...
export const getRecommendationSampleRanges = (duration: number): RecommendationSampleRange[] => {
  ...
  const maxWindowsByDuration = Math.max(
    1,
    Math.min(MAX_RECOMMENDATION_SAMPLE_WINDOWS, Math.floor(duration / MIN_RECOMMENDATION_WINDOW_SECONDS))
  );
  const windowCount = Math.max(
    1,
    Math.min(maxWindowsByDuration, Math.ceil(duration / MAX_RECOMMENDATION_SAMPLE_SECONDS))
  );
  const windowDuration = Math.min(duration, MAX_RECOMMENDATION_SAMPLE_SECONDS / windowCount);
  const firstCenter = windowCount === 1 ? 0.5 : 0.18;
  const lastCenter = windowCount === 1 ? 0.5 : 0.82;

  return Array.from({ length: windowCount }, (_, index) => {
    const center = windowCount === 1 ? 0.5 : firstCenter + ((lastCenter - firstCenter) * index) / (windowCount - 1);
    const maxStart = Math.max(0, duration - windowDuration);
    const start = Math.min(maxStart, Math.max(0, duration * center - windowDuration / 2));
...
const buildRecommendationSampleBuffer = (
  ...
  if (sampleRanges.length === 1 && sampleRanges[0].start <= 0 && sampleRanges[0].end >= audioBuffer.duration) {
  ...
  const segments = sampleRanges.map((sampleRange, index) => {
    ...
    return extractAudioSegment(audioBuffer, sampleRange.start, sampleRange.end, context);
  });
  const sampleLength = segments.reduce((total, segment) => total + segment.length, 0);
  const sampleBuffer = context.createBuffer(audioBuffer.numberOfChannels, sampleLength, audioBuffer.sampleRate);

  let offset = 0;
  for (const segment of segments) {
    for (let channel = 0; channel < audioBuffer.numberOfChannels; channel += 1) {
      sampleBuffer.getChannelData(channel).set(segment.getChannelData(channel), offset);
    }
...
export const buildRecommendationProfile = (
  ...
  const { sampleBuffer, sampleCount } = buildRecommendationSampleBuffer(audioBuffer, context, onProgress);
  const reducer = new SpectralNoiseReducer();
  const profile = reducer.learnNoiseProfile(sampleBuffer);

  onProgress?.({
    progress: 1,
    message: sampleCount > 1 ? `Analyzed ${sampleCount} clip samples` : 'Analyzed clip audio'
  });
 ...
 */
export const NoiseReductionSection: React.FC<NoiseReductionSectionProps> = ({ clipId }) => {
  ...
  const audioEffects = useProjectStore((state) => state.getAudioEffects(audioTargetClipId));
  const setAudioEffectPreviewBypass = useProjectStore((state) => state.setAudioEffectPreviewBypass);
  ...
  const [config, setConfig] = useState<NoiseReductionConfig>(DEFAULT_NOISE_REDUCTION_STATE);

  const [learningState, setLearningState] = useState<LearningState>('idle');
  const [activePresetId, setActivePresetId] = useState<NoiseReductionFocus>(defaultFocus);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [recommendation, setRecommendation] = useState<NoiseRecommendation | null>(null);
  ...
  const applyNoiseReductionConfig = useCallback(
    (nextConfig: NoiseReductionConfig, existingEffectId = effectId) => {
      ...
      if (existingEffectId) {
        const updateResult = bridge.updateNoiseReduction(audioTargetClipId, existingEffectId, nextConfig);

        if (!updateResult.success) {
          throw new Error(updateResult.error ?? 'Failed to update noise reduction');
        }
  ...
  const handleToggle = useCallback(
    (newEnabled: boolean) => {
      if (newEnabled && !effectId) {
        try {
          ...
          setErrorMessage(error instanceof Error ? error.message : 'Failed to apply noise reduction');
          return;
        }
      } else if (effectId) {
    ...
    [applyNoiseReductionConfig, audioTargetClipId, config, effectId, toggleAudioEffect]
  );
  ...
  const analyzeNoiseForClip = useCallback(async (): Promise<NoiseAnalysisResult> => {
    ...
    const mediaItem = project.mediaLibrary.items.find((candidate) => candidate.id === clip.mediaId);
    ...
    try {
      ...
      const audioBuffer = await loadAudioBuffer(audioContext, mediaItem.blob, {
      ...
      const clipBuffer = extractAudioSegment(audioBuffer, clipStart, clipEnd, analysisContext);
      ...
      const analyzedProfile = await autoLearnNoiseProfile(clipBuffer, analysisContext);
  ...
  const handleApplyPreset = useCallback(
    async (presetId: NoiseReductionFocus) => {
      ...
      try {
        ...
        setErrorMessage(error instanceof Error ? error.message : 'Failed to apply noise reduction preset');
      }
    },
  ...
  const handleLearnNoiseProfile = useCallback(async () => {
    ...
    try {
      ...
      setLearningState('error');
      setErrorMessage(error instanceof Error ? error.message : 'Failed to analyze this clip');
      setAnalysisProgress(null);
  ...
  const handleApplyRecommendation = useCallback(() => {
    ...
    try {
      ...
      setRecommendation(null);
      setAppliedMessage(`${getNoiseReductionPreset(recommendation.presetId).label} applied to this clip.`);
      setLearningState('success');
      setTimeout(() => {
        setLearningState('idle');
      }, 2000);
      ...
      setLearningState('error');
      setErrorMessage(error instanceof Error ? error.message : 'Failed to apply recommended cleanup');
    }
  }, [applyNoiseReductionConfig, recommendation]);

  const recommendationPreset = recommendation ? getNoiseReductionPreset(recommendation.presetId) : null;

  return (
    <div className={`border rounded-lg overflow-hidden ${enabled ? 'border-border' : 'border-border/50 opacity-60'}`}>
      <div className='flex items-center gap-2 p-2 bg-background-tertiary'>
        <button onClick={() => setIsOpen(!isOpen)} className='flex-1 flex items-center gap-1'>
          <ChevronDown size={12} className={`transition-transform ${isOpen ? '' : '-rotate-90'} text-text-muted`} />
          <Volume2 size={12} className='text-text-muted' />
          <span className='text-[10px] font-medium text-text-primary'>Noise Reduction</span>
        </button>
        <button
      ...
      {isOpen && (
        <div className='p-3 space-y-3'>
          <p className='text-[9px] leading-relaxed text-text-muted'>
            Reduce white noise, wind, hum, room tone, and background music while keeping speech or the wanted audio in
            front.
          </p>
            ...
            {NOISE_REDUCTION_PRESETS.map((preset) => {
              ...
              return (
                ...
                >
                  ...
                  <div className='mt-1 text-[9px] leading-relaxed opacity-80'>{preset.description}</div>
                </button>
              );
            })}
          </div>
            ...
            <div className='flex items-center justify-between gap-2'>
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-[8px] font-medium ${
                  enabled ? 'bg-green-500/15 text-green-400' : 'bg-background-tertiary text-text-muted'
                }`}
              >
                {enabled ? 'Applied' : 'Off'}
              </span>
          ...
          <div className='space-y-2 rounded-lg border border-border/70 bg-background-secondary/60 px-2 py-2'>
            <div className='text-[9px] font-medium text-text-primary'>A/B Preview</div>
            <div className='grid grid-cols-2 gap-2'>
              <button
          ...
          {errorMessage && <div className='text-[9px] text-red-500 text-center'>{errorMessage}</div>}
