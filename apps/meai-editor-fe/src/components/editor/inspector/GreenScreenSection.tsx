...
import type { RGB, ChromaKeySettings } from '@meai-editor/core';

interface GreenScreenSectionProps {
  clipId: string;
}

const ColorPreview: React.FC<{ color: RGB; onClick?: () => void }> = ({ color, onClick }) => (
  <button
    onClick={onClick}
...
const ColorPresetButton: React.FC<{
  ...
  <button
    ...
    className={`flex items-center gap-1.5 px-2 py-1 rounded text-[9px] transition-colors ${
      isActive ? 'bg-primary text-white' : 'bg-background-tertiary text-text-muted hover:text-text-primary'
    }`}
  >
    <div
...
export const GreenScreenSection: React.FC<GreenScreenSectionProps> = ({ clipId }) => {
  const project = useProjectStore((state) => state.project);
  const getChromaKeyEngine = useEngineStore((state) => state.getChromaKeyEngine);

  const [isPickingColor, setIsPickingColor] = useState(false);
  const [chromaKeyEngine, setChromaKeyEngine] = useState<import('@meai-editor/core').ChromaKeyEngine | null>(null);
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-lg border border-green-500/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Green Screen</span>
          <p className='text-[9px] text-text-muted'>Remove background color from video</p>
        </div>
        <button
      ...
      {settings.enabled && (
        <>
          <div className='space-y-2'>
            <div className='flex items-center justify-between'>
              <span className='text-[10px] font-medium text-text-primary'>Key Color</span>
            ...
            {isPickingColor && (
              <div className='p-2 bg-primary/10 border border-primary/30 rounded-lg'>
                <p className='text-[9px] text-primary text-center'>Click on the video preview to pick a color</p>
              </div>
            )}
          ...
          <div className='space-y-3 pt-2 border-t border-border'>
            <ControlSlider label='Tolerance' value={settings.tolerance} onChange={handleSetTolerance} />

            <ControlSlider label='Edge Softness' value={settings.edgeSoftness} onChange={handleSetEdgeSoftness} />
          ...
          <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg'>
            ...
            <p className='text-[9px] text-text-muted flex-1'>Place video clips below this one to use as background</p>
          </div>
        </>
      )}
      ...
      {!settings.enabled && (
        <div className='text-center py-4'>
          ...
          <p className='text-[10px] text-text-muted'>Enable to remove background color</p>
          <button
            onClick={handleToggleEnabled}
