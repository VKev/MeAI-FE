...
import { stickerLibrary, EMOJI_CATEGORIES, type EmojiItem, type StickerItem } from '@meai-editor/core';
...
const StickerCard: React.FC<StickerCardProps> = ({ sticker, onAdd }) => (
  ...
  >
    ...
    <span className='text-[8px] text-text-muted truncate max-w-full'>{sticker.name}</span>
  </button>
);
...
export const StickerPickerPanel: React.FC = () => {
  ...
  const handleAddEmoji = useCallback(
    (emoji: EmojiItem) => {
      ...
      let graphicsTrack = project.timeline.tracks.find((t) => t.type === 'graphics');
      if (!graphicsTrack) {
        addTrack('graphics');
        graphicsTrack = project.timeline.tracks.find((t) => t.type === 'graphics');
      }

      if (graphicsTrack) {
        const clip = stickerLibrary.createEmojiClip(emoji, graphicsTrack.id, 0, 5);
  ...
  const handleAddSticker = useCallback(
    (sticker: StickerItem) => {
      ...
      if (graphicsTrack) {
        const clip = stickerLibrary.createStickerClip(sticker, graphicsTrack.id, 0, 5);
        createStickerClip(clip);
      }
    },
    [project, addTrack, createStickerClip]
  ...
  return (
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r from-pink-500/20 to-rose-500/20 rounded-lg border border-pink-500/30'>
        ...
        <div>
          <span className='text-[11px] font-medium text-text-primary'>Stickers & Emojis</span>
          <p className='text-[9px] text-text-muted'>Add fun elements to your video</p>
        </div>
      </div>
      ...
      {activeTab === 'stickers' && !searchQuery && stickerCategories.length > 0 && (
        <div className='flex gap-1 overflow-x-auto pb-1'>
        ...
        <div className='grid grid-cols-6 gap-1 max-h-48 overflow-y-auto'>
          {currentEmojis.length === 0 ? (
            <div className='col-span-6 text-center py-4'>
              <Smile size={24} className='mx-auto mb-2 text-text-muted opacity-50' />
              <p className='text-[10px] text-text-muted'>No emojis found</p>
            </div>
          ) : (
            currentEmojis.map((emoji) => (
              <EmojiButton key={emoji.id} emoji={emoji} onAdd={() => handleAddEmoji(emoji)} />
            ))
          )}
        </div>
      )}
        ...
        <div className='space-y-2 max-h-48 overflow-y-auto'>
          {allStickers.length === 0 ? (
            <div className='text-center py-4'>
              <Sticker size={24} className='mx-auto mb-2 text-text-muted opacity-50' />
              <p className='text-[10px] text-text-muted'>No stickers yet</p>
              <p className='text-[9px] text-text-muted mt-1'>Import custom stickers below</p>
            </div>
          ) : (
        ...
        <button
          onClick={() => {
            ...
            input.onchange = async (e) => {
              ...
              if (file) {
                await stickerLibrary.importSticker(file, file.name.replace(/\.[^/.]+$/, ''));
              }
            };
            input.click();
          }}
