...
import { ChevronDown, RotateCcw, Eye, EyeOff, GripVertical } from 'lucide-react';
import { useProjectStore } from '../../../stores/project-store';
import type { VideoEffect, VideoEffectType } from '../../../bridges/effects-bridge';
import {
  LabeledSlider,
  DropdownMenu,
  DropdownMenuTrigger,
...
const EffectItem: React.FC<{
  ...
  return (
    <div className={`border rounded-lg ${effect.enabled ? 'border-border' : 'border-border/50 opacity-60'}`}>
      <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-t-lg'>
        ...
        <button onClick={() => setIsExpanded(!isExpanded)} className='flex-1 flex items-center gap-1 text-left'>
          <ChevronDown size={12} className={`transition-transform ${isExpanded ? '' : '-rotate-90'} text-text-muted`} />
          <span className='text-[10px] font-medium text-text-primary'>{effectLabels[effect.type] || effect.type}</span>
        </button>
        <button
      ...
      <DropdownMenuContent align='start' className='w-48 max-h-64 overflow-y-auto'>
        {EFFECT_CATEGORIES.map((category) => (
          <React.Fragment key={category}>
            ...
            {EFFECT_TYPES.filter((e) => e.category === category).map((effect) => (
              <DropdownMenuItem key={effect.type} onClick={() => onSelect(effect.type)} className='text-[10px]'>
                {effect.label}
              </DropdownMenuItem>
            ))}
          </React.Fragment>
 ...
 */
export const VideoEffectsSection: React.FC<VideoEffectsSectionProps> = ({ clipId }) => {
  const { getVideoEffects, addVideoEffect, updateVideoEffect, removeVideoEffect, toggleVideoEffect } =
    useProjectStore();
  ...
  return (
    <div className='space-y-3'>
      {effects.length === 0 ? (
        <p className='text-[10px] text-text-muted text-center py-2'>No effects applied</p>
      ) : (
