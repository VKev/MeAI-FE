...
export const ColorGradingSection: React.FC<ColorGradingSectionProps> = ({ clipId }) => {
  const { getColorGrading, updateColorGrading, resetColorGrading } = useProjectStore();
  ...
  return (
    <div className='space-y-3'>
      ...
      <SubSection title='Curves'>
        <CurvesEditor values={curvesValues} onChange={handleCurvesChange} onReset={handleCurvesReset} />
      </SubSection>

      <SubSection title='LUT'>
        <LUTLoader lutData={colorGrading.lut as LUTData | null} onChange={handleLUTChange} />
      </SubSection>

      <SubSection title='HSL'>
        <HSLControls values={hslValues} onChange={handleHSLValuesChange} onReset={handleHSLReset} />
      </SubSection>
    </div>
  );
};
