...
export const Transform3DSection: React.FC<Transform3DSectionProps> = ({ clipId }) => {
  const {
    getClip,
    getTextClip,
    getShapeClip,
  ...
  const clip = useMemo(() => {
    ...
    return null;
  }, [clipId, getClip, getTextClip, getShapeClip, getSVGClip, getStickerClip, project.modifiedAt]);
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='space-y-1'>
        <span className='text-[10px] text-text-secondary'>Transform Style</span>
        <Select value={transformStyle} onValueChange={(v) => handleTransformStyleChange(v as 'flat' | 'preserve-3d')}>
        ...
        <p className='text-[9px] text-text-muted'>
          {transformStyle === 'flat' && 'Flattens children into the plane of this element'}
      ...
      {(rotate3d.x !== 0 || rotate3d.y !== 0 || rotate3d.z !== 0) && (
        <div className='p-2 bg-primary/5 border border-primary/20 rounded-lg'>
          <p className='text-[9px] text-text-muted'>
            <span className='text-primary font-medium'>Tip:</span> 3D rotations allow you to rotate layers along X, Y,
            and Z axes for depth effects. Adjust perspective to control the 3D depth perception.
          </p>
        </div>
      )}
    </div>
