...
import { autoLearnNoiseProfile, type Project } from '@meai-editor/core';
import { createEmptyProject } from '../../../stores/project/project-helpers';
import { useProjectStore } from '../../../stores/project-store';
import { createNoiseReductionEffect, DEFAULT_NOISE_REDUCTION } from '../../../bridges/audio-bridge-effects';
import * as audioBridgeEffects from '../../../bridges/audio-bridge-effects';
import { NoiseReductionSection } from './NoiseReductionSection';

vi.mock('@meai-editor/core', async () => {
  const actual = await vi.importActual<typeof import('@meai-editor/core')>('@meai-editor/core');
  return {
    ...actual,
...
describe('NoiseReductionSection persistence', () => {
  ...
  it('shows an explicit apply action before saving analyzed recommendations', async () => {
    ...
    vi.spyOn(audioBridgeEffects.getAudioBridgeEffects(), 'learnNoiseProfile').mockResolvedValue({
      id: `profile-${clipId}`,
  ...
  it('still recommends a preset when no pure noise-only profile can be learned', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Recommendation ready')).toBeInTheDocument();
      expect(screen.getByText(/custom profile could not be isolated/i)).toBeInTheDocument();
    });
