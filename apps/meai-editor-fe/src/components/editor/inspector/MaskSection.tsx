...
import type { Mask, MaskShape } from '@meai-editor/core';
      ...
      >
        ...
        <button
          ...
          className={`p-1 rounded transition-colors ${
            mask.inverted ? 'bg-amber-500/20 text-amber-400' : 'text-text-muted hover:text-text-primary'
          }`}
        ...
        <div className='p-2 space-y-3 border-t border-border bg-background-tertiary/50'>
          <div className='space-y-1'>
            <div className='flex items-center justify-between'>
              ...
              <span className='text-[9px] text-text-secondary'>{mask.feathering}px</span>
            </div>
            <Slider
              min={0}
              max={100}
          ...
          <div className='space-y-1'>
            <div className='flex items-center justify-between'>
              ...
              <span className='text-[9px] text-text-secondary'>{mask.expansion}px</span>
            </div>
            <Slider
              min={-100}
              max={100}
          ...
          <div className='space-y-1'>
            <div className='flex items-center justify-between'>
              ...
              <span className='text-[9px] text-text-secondary'>{Math.round(mask.opacity * 100)}%</span>
            </div>
            <Slider
              min={0}
              max={100}
          ...
          <div className='flex items-center gap-2 pt-2 border-t border-border'>
            ...
            <span className='text-[8px] text-text-muted'>
              {mask.keyframes.length > 0 ? `${mask.keyframes.length} keyframes` : 'No keyframes'}
            </span>
          </div>
        </div>
      )}
...
export const MaskSection: React.FC<MaskSectionProps> = ({ clipId }) => {
  ...
  const [maskEngine, setMaskEngine] = useState<import('@meai-editor/core').MaskEngine | null>(null);
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Masking</span>
          <p className='text-[9px] text-text-muted'>Control visible regions of clip</p>
        </div>
      </div>

      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          <span className='text-[10px] font-medium text-text-secondary'>Add Mask Shape</span>
        </div>
      ...
      {masks.length > 0 ? (
        <div className='space-y-2'>
          <div className='flex items-center justify-between'>
            <span className='text-[10px] font-medium text-text-secondary'>Masks ({masks.length})</span>
            <button
        ...
        <div className='text-center py-4'>
          <Square size={24} className='mx-auto mb-2 text-text-muted opacity-50' />
          <p className='text-[10px] text-text-muted'>No masks on this clip</p>
          <p className='text-[9px] text-text-muted mt-1'>Click a shape above to add a mask</p>
        </div>
      )}

      <div className='pt-2 border-t border-border'>
        <p className='text-[9px] text-text-muted text-center'>Masks control which parts of the clip are visible</p>
      </div>
    </div>
  );
};
