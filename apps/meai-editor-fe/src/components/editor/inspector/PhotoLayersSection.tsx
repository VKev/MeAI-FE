...
import { Layers, Eye, EyeOff, Lock, Unlock, Trash2, Copy, Plus, GripVertical, ChevronDown } from 'lucide-react';
import type { PhotoBlendMode, PhotoLayer } from '@meai-editor/core';
import {
        ...
        <DropdownMenuContent align='end' className='w-32 max-h-48 overflow-y-auto'>
          {BLEND_MODES.map((mode) => (
            <DropdownMenuItem
              ...
              className={`text-[10px] ${mode.value === value ? 'text-primary bg-background-tertiary' : ''}`}
            >
              {mode.label}
            </DropdownMenuItem>
          ))}
  ...
  return (
    ...
    >
      ...
      <div className='flex-1 min-w-0'>
        ...
        <span className='text-[9px] text-text-muted capitalize'>{layer.type}</span>
      </div>
      ...
      <div className='flex items-center gap-1'>
        <button
          ...
          className={`p-1 rounded transition-colors ${
            layer.visible ? 'text-text-secondary hover:text-text-primary' : 'text-text-muted hover:text-text-secondary'
          }`}
        ...
        <button
          ...
          className={`p-1 rounded transition-colors ${
            layer.locked ? 'text-warning hover:text-warning/80' : 'text-text-muted hover:text-text-secondary'
          }`}
...
export const PhotoLayersSection: React.FC<PhotoLayersSectionProps> = ({
  ...
  return (
    <div className='space-y-4'>
      {/* Layer List Header */}
      <div className='flex items-center justify-between'>
        <span className='text-[10px] text-text-secondary font-medium'>Layers ({layers.length})</span>
        <button
          onClick={onAddLayer}
      ...
      {selectedLayer && (
        <div className='space-y-3 pt-3 border-t border-border'>
          <span className='text-[10px] text-text-secondary font-medium'>Layer Properties</span>
