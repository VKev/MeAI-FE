    ...
    >
      ...
      <div className='mt-2 flex gap-1 flex-wrap'>
        {preset.effects.slice(0, 3).map((effect, index) => (
          <span key={index} className='px-1.5 py-0.5 text-[8px] bg-background-secondary rounded text-text-muted'>
            {effect.type}
          </span>
        ))}
      ...
      {isHovered && !isApplied && (
        <div className='absolute inset-0 flex items-center justify-center bg-background-tertiary/80 rounded-lg'>
          <span className='text-[10px] text-primary font-medium'>Click to Apply</span>
        </div>
      )}
    </button>
  );
...
export const FilterPresetsPanel: React.FC<FilterPresetsPanelProps> = ({ clipId }) => {
  ...
  const presets = useMemo(() => getPresetsByCategory(selectedCategory), [selectedCategory]);
  ...
  if (!targetClipId) {
    return (
      <div className='p-4 text-center'>
        ...
        <p className='text-[10px] text-text-muted'>Select a video clip to apply filters</p>
      </div>
    );
  }
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center gap-2 p-2 bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div>
          <span className='text-[11px] font-medium text-text-primary'>Filter Presets</span>
      ...
      {appliedPresetId && (
        <div className='space-y-3 p-3 bg-background-tertiary rounded-lg'>
          <div className='flex items-center justify-between'>
            ...
            <span className='text-[10px] font-mono text-text-primary'>{intensityValue}%</span>
          </div>
          <Slider
            min={0}
            max={100}
      ...
      <p className='text-[9px] text-text-muted text-center'>
        {FILTER_PRESETS.length} presets across {FILTER_CATEGORIES.length} categories
      </p>
    </div>
  );
};
