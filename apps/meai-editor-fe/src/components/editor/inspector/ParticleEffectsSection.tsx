...
import { Sparkles, Plus, Trash2, ChevronDown, ChevronRight, Eye, EyeOff, Play } from 'lucide-react';
import {
  Button,
  Slider,
  Label,
  ...
  return (
    <div className='space-y-4'>
      <div className='flex gap-2'>
        <Select value={selectedPreset} onValueChange={setSelectedPreset}>
          ...
          <SelectContent>
            {Object.entries(groupedPresets).map(([type, presets]) => (
              <div key={type}>
                <div className='px-2 py-1.5 text-xs font-semibold text-text-muted uppercase tracking-wider'>{type}</div>
                {presets.map((preset) => (
        ...
        <Button variant='default' size='sm' onClick={handleAddEffect} disabled={!selectedPreset} className='h-8 px-3'>
        ...
        <ScrollArea className='max-h-[400px]'>
          <div className='space-y-2 pr-2'>
            {effects.map((effect) => {
              ...
              return (
                <div key={effect.id} className='bg-background-tertiary rounded-lg border border-border overflow-hidden'>
                  <div className='flex items-center gap-2 px-3 py-2'>
                    <button
                      onClick={() => toggleExpanded(effect.id)}
                      className='p-0.5 rounded hover:bg-background-elevated text-text-muted'
                    >
                      {expandedEffects.has(effect.id) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    </button>

                    <span className='flex-1 text-xs font-medium text-text-primary capitalize'>{effect.type}</span>
                  ...
                  {expandedEffects.has(effect.id) && (
                    <div className='px-3 pb-3 space-y-3 border-t border-border/50 pt-3'>
                      <div className='grid grid-cols-2 gap-3'>
                        <div>
                          <Label className='text-[10px] text-text-muted mb-1'>Start Time (s)</Label>
                          <Input
                            type='number'
                        ...
                        <div>
                          <Label className='text-[10px] text-text-muted mb-1'>Duration (s)</Label>
                          <Input
                            type='number'
                        ...
                        <CollapsibleContent className='pt-2 space-y-3'>
                          <div>
                            ...
                            <Slider
                              ...
                              onValueChange={([v]) => handleConfigChange(effect.id, 'particleCount', v)}
                              min={10}
                              max={500}
                              step={10}
                              className='w-full'
                          ...
                          <div>
                            <Label className='text-[10px] text-text-muted mb-1'>Speed: {effect.config.speed}</Label>
                            <Slider
                              value={[effect.config.speed]}
                              onValueChange={([v]) => handleConfigChange(effect.id, 'speed', v)}
                              min={10}
                              max={500}
                              step={10}
                              className='w-full'
                          ...
                          <div>
                            <Label className='text-[10px] text-text-muted mb-1'>Gravity: {effect.config.gravity}</Label>
                            <Slider
                              value={[effect.config.gravity]}
                              onValueChange={([v]) => handleConfigChange(effect.id, 'gravity', v)}
                              min={-500}
                              max={500}
                              step={10}
                              className='w-full'
                          ...
                          <div>
                            ...
                            <Slider
                              ...
                              onValueChange={([v]) => handleConfigChange(effect.id, 'emissionRate', v)}
                              min={1}
                              max={200}
                              step={1}
                              className='w-full'
                          ...
                          <div>
                            ...
                            <Slider
                              ...
                              onValueChange={([v]) => handleConfigChange(effect.id, 'turbulence', v)}
                              min={0}
                              max={100}
                              step={5}
                              className='w-full'
                          ...
                          <div>
                            <Label className='text-[10px] text-text-muted mb-1'>Blend Mode</Label>
                            <Select
                              value={effect.config.blendMode}
                              onValueChange={(v) => handleConfigChange(effect.id, 'blendMode', v)}
                            >
