...
import { LabeledSlider, Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@meai-editor/ui';
...
const PresetSelector: React.FC<{
  ...
  <div className='space-y-2'>
    ...
    <p className='text-[9px] text-text-muted'>{ANIMATION_PRESETS.find((p) => p.value === value)?.description}</p>
  </div>
);
...
export const TextAnimationSection: React.FC<TextAnimationSectionProps> = ({ clipId }) => {
  ...
  const currentPreset: TextAnimationPreset = textClip?.animation?.preset || 'none';
  ...
  const handleInDurationChange = useCallback(
    (newInDuration: number) => {
      applyTextAnimationPreset(clipId, currentPreset, newInDuration, outDuration);
    },
  ...
  const handleOutDurationChange = useCallback(
    (newOutDuration: number) => {
      applyTextAnimationPreset(clipId, currentPreset, inDuration, newOutDuration);
    },
  ...
  return (
    <div className='space-y-4'>
        ...
        <>
          <div className='space-y-3 p-3 bg-background-tertiary rounded-lg'>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[10px] text-text-secondary font-medium'>Timing</span>
            </div>
          ...
          <div className='p-3 bg-background-secondary rounded-lg border border-border'>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[10px] text-text-secondary font-medium'>Preview</span>
            </div>
            <p className='text-[9px] text-text-muted'>
              Animation will play during preview and export. Total animation time:{' '}
      ...
      {currentPreset === 'fade' && <FadeParams clipId={clipId} animation={textClip.animation} />}

      {(currentPreset === 'slide-left' ||
        currentPreset === 'slide-right' ||
        currentPreset === 'slide-up' ||
        currentPreset === 'slide-down') && <SlideParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'scale' && <ScaleParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'bounce' && <BounceParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'rotate' && <RotateParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'wave' && <WaveParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'shake' && <ShakeParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'pop' && <PopParams clipId={clipId} animation={textClip.animation} />}

      {currentPreset === 'glitch' && <GlitchParams clipId={clipId} animation={textClip.animation} />}
    </div>
  );
};
...
const FadeParams: React.FC<{
  ...
  const handleChange = (start: number, end: number) => {
    ...
    applyTextAnimationPreset(clipId, 'fade', textClip.animation.inDuration, textClip.animation.outDuration, {
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Fade Settings</span>
      <Slider
        label='Start Opacity'
        value={startOpacity}
...
const SlideParams: React.FC<{
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Slide Settings</span>
      <Slider label='Distance' value={slideDistance} onChange={handleChange} min={0.05} max={1} step={0.05} unit='' />
    </div>
  );
};
...
const ScaleParams: React.FC<{
  ...
  const handleChange = (from: number, to: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'scale', textClip.animation.inDuration, textClip.animation.outDuration, {
      scaleFrom: from,
      scaleTo: to
    });
  };
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Scale Settings</span>
      <Slider
        label='Scale From'
        value={scaleFrom}
...
const BounceParams: React.FC<{
  ...
  const handleChange = (height: number, count: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'bounce', textClip.animation.inDuration, textClip.animation.outDuration, {
      bounceHeight: height,
      bounceCount: count
    });
  };
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Bounce Settings</span>
      <Slider
        label='Height'
        value={bounceHeight}
...
const RotateParams: React.FC<{
  ...
  const handleChange = (angle: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'rotate', textClip.animation.inDuration, textClip.animation.outDuration, {
      rotateAngle: angle
    });
  };
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Rotate Settings</span>
      <Slider label='Angle' value={rotateAngle} onChange={handleChange} min={-720} max={720} step={15} unit='°' />
    </div>
  );
};
...
const WaveParams: React.FC<{
  ...
  const handleChange = (amplitude: number, frequency: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'wave', textClip.animation.inDuration, textClip.animation.outDuration, {
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Wave Settings</span>
      <Slider
        label='Amplitude'
        value={waveAmplitude}
...
const ShakeParams: React.FC<{
  ...
  const handleChange = (intensity: number, speed: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'shake', textClip.animation.inDuration, textClip.animation.outDuration, {
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Shake Settings</span>
      <Slider
        label='Intensity'
...
const PopParams: React.FC<{
  ...
  const handleChange = (overshoot: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'pop', textClip.animation.inDuration, textClip.animation.outDuration, {
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Pop Settings</span>
      <Slider label='Overshoot' value={popOvershoot} onChange={handleChange} min={1} max={2} step={0.05} unit='x' />
    </div>
  );
};
...
const GlitchParams: React.FC<{
  ...
  const handleChange = (intensity: number, speed: number) => {
    if (!textClip?.animation) return;
    applyTextAnimationPreset(clipId, 'glitch', textClip.animation.inDuration, textClip.animation.outDuration, {
  ...
  return (
    <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
      <span className='text-[10px] text-text-secondary font-medium'>Glitch Settings</span>
      <Slider
        label='Intensity'
