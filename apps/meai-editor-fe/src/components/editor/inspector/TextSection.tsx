...
const FONT_CATEGORIES = {
  Popular: ['Inter', 'Poppins', 'Montserrat', 'Roboto', 'Open Sans', 'Lato', 'Outfit', 'DM Sans'],
  'Display & Headlines': [
    'Bebas Neue',
    'Anton',
    'Oswald',
...
export const TextSection: React.FC<TextSectionProps> = ({ clipId }) => {
  const { getTextClip, updateTextContent, updateTextStyle, updateTextTransform, project } = useProjectStore();

  const textClip = useMemo(() => getTextClip(clipId), [clipId, getTextClip, project.modifiedAt]);
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
        <FontSelector value={style.fontFamily} onChange={(fontFamily) => handleStyleChange({ fontFamily })} />
        <NumberInput
          label='Size'
      ...
      <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
        <span className='text-[10px] text-text-secondary font-medium'>Position on Canvas</span>
        <div className='flex items-center justify-between'>
      ...
      <div className='space-y-2 p-3 bg-background-tertiary rounded-lg'>
        <ColorField label='Text Color' value={style.color} onChange={(color) => handleStyleChange({ color })} />
        <ColorField
          label='Background'
