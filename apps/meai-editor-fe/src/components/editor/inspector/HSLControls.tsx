...
const ColorTab: React.FC<{
  ...
  <button
    ...
    className={`flex-1 py-1.5 text-[9px] font-medium rounded transition-all ${
      isActive ? 'bg-background-tertiary text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'
    }`}
    style={{
      borderBottom: isActive ? `2px solid ${color.color}` : '2px solid transparent'
    }}
  ...
  return (
    <div className='space-y-1'>
      ...
      <div className='h-1.5 bg-background-tertiary rounded-full relative overflow-hidden'>
        ...
        {isBipolar && (
          <div className='absolute top-0 w-0.5 h-full bg-text-muted/50' style={{ left: `${centerPercentage}%` }} />
        )}
        {/* Thumb */}
        <div
 ...
 */
export const HSLControls: React.FC<HSLControlsProps> = ({ values, onChange, onReset }) => {
  ...
  const hasAdjustments = useMemo(() => {
    return currentHue !== 0 || currentSaturation !== 0 || currentLuminance !== 0;
    ...
    <div className='space-y-3'>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          <div className='w-3 h-3 rounded-full' style={{ backgroundColor: activeColor.color }} />
          <span className='text-[11px] font-medium text-text-primary'>{activeColor.fullLabel}</span>
        </div>
        {hasAdjustments && (
          <button
        ...
        <div className='flex gap-1'>
          {COLOR_RANGES.map((color) => {
            ...
            return (
              <div
                key={color.key}
                className={`flex-1 h-1 rounded-full transition-all ${hasAdj ? 'opacity-100' : 'opacity-20'}`}
