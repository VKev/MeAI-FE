...
import type { WaveformScopeData, VectorscopeData, HistogramData } from '@meai-editor/core';
...
interface ScopesPanelProps {
  ...
  onScopeDataGenerated?: (data: WaveformScopeData | VectorscopeData | HistogramData) => void;
}
...
const WaveformRenderer: React.FC<{
  ...
  useEffect(() => {
    ...
    // Draw waveform data
    const drawChannel = (channelData: Uint8Array, color: string, alpha: number = 0.8) => {
      ctx.fillStyle = color;
  ...
  return <canvas ref={canvasRef} width={320} height={180} className='w-full h-auto rounded-lg border border-border' />;
};
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        if (intensity > 0) {
          ...
          ctx.fillRect(displayX, displayY, Math.max(1, scale), Math.max(1, scale));
        }
      }
    }
  }, [data]);
...
const HistogramRenderer: React.FC<{
  ...
  useEffect(() => {
    ...
    // Draw histogram bars
    const drawHistogram = (channelData: Uint32Array, color: string, alpha: number = 0.7) => {
      ctx.fillStyle = color;
      ctx.globalAlpha = alpha;
  ...
  return <canvas ref={canvasRef} width={320} height={120} className='w-full h-auto rounded-lg border border-border' />;
};
...
export const ScopesPanel: React.FC<ScopesPanelProps> = ({
  ...
  const [waveformData, setWaveformData] = useState<WaveformScopeData | null>(null);
  const [vectorscopeData, setVectorscopeData] = useState<VectorscopeData | null>(null);
  ...
  const viewContent = useMemo(() => {
    if (isLoading) {
      return (
        <div className='flex items-center justify-center h-40 text-text-muted text-xs'>Generating scope data...</div>
      );
    }

    if (!frameImage) {
      return <div className='flex items-center justify-center h-40 text-text-muted text-xs'>No frame to analyze</div>;
    }
        ...
        return (
          <div className='space-y-2'>
            ...
            <div className='flex items-center justify-between'>
              <span className='text-[10px] text-text-muted'>{showRGBWaveform ? 'RGB Parade' : 'Luminance'}</span>
              <button
  ...
  }, [activeView, frameImage, isLoading, waveformData, vectorscopeData, histogramData, showRGBWaveform]);
  ...
  return (
    <div className='space-y-3'>
      ...
      <p className='text-[9px] text-text-muted'>
        {activeView === 'waveform' && 'Waveform shows luminance distribution across the frame width.'}
        {activeView === 'vectorscope' && 'Vectorscope shows color saturation and hue distribution.'}
        {activeView === 'histogram' && 'Histogram shows RGB and luminance value distribution.'}
      </p>
    </div>
  );
};
