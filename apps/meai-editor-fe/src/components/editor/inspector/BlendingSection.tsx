...
import { getAvailableBlendModes, getBlendModeName, type BlendMode } from '@meai-editor/core';
import {
  ...
  return (
    <div className='space-y-3'>
      <div className='space-y-1'>
        ...
        <Select value={blendMode} onValueChange={(v) => handleBlendModeChange(v as BlendMode)}>
      ...
      <Slider label='Opacity' value={blendOpacity} onChange={handleOpacityChange} min={0} max={100} step={1} unit='%' />

      {blendMode !== 'normal' && (
        <div className='p-2 bg-primary/5 border border-primary/20 rounded-lg'>
          <p className='text-[9px] text-text-muted'>
            <span className='text-primary font-medium'>Tip:</span> Blend modes affect how this layer combines with
            layers below it. Experiment with different modes for creative effects.
          </p>
        </div>
      )}
    </div>
