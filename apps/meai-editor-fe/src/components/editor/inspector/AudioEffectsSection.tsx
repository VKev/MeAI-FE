 ...
 */
export const AudioEffectsSection: React.FC<AudioEffectsSectionProps> = ({ clipId }) => {
  // Get store methods
  ...
  const [eqBands, setEqBands] = useState<Array<{ frequency: string; gain: number }>>([
  ...
  const [compressorEffectId, setCompressorEffectId] = useState<string | null>(null);
  ...
  const handleEqBandChange = useCallback(
    (index: number, gain: number) => {
      ...
      setEqBands((bands) => {
        const newBands = bands.map((band, i) => (i === index ? { ...band, gain } : band));
  ...
  return (
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg'>
        ...
        <span className='text-[10px] text-text-secondary'>Audio clip: {clipId.substring(0, 8)}...</span>
      </div>

      <SubSection title='Equalizer' enabled={eqEnabled} onToggle={handleEqToggle} defaultOpen>
      ...
      <SubSection title='Compressor' enabled={compressorEnabled} onToggle={handleCompressorToggle}>
      ...
      <SubSection title='Reverb' enabled={reverbEnabled} onToggle={handleReverbToggle}>
        <div className='space-y-2'>
          <Slider
            label='Room Size'
      ...
      <SubSection title='Delay' enabled={delayEnabled} onToggle={handleDelayToggle}>
        <div className='space-y-2'>
          <Slider
            label='Time'
