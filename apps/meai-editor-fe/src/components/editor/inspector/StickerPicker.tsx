...
import type { StickerItem, EmojiItem } from '@meai-editor/core';
 ...
 */
export const StickerPicker: React.FC<StickerPickerProps> = ({ trackId, startTime, duration = 5, onSelect }) => {
  ...
  const stickerCategories = useMemo(() => bridge.getStickerCategories(), [bridge]);
 ...
 `}
      ...
      <div className='max-h-[200px] overflow-y-auto'>
        {items.length > 0 ? (
          <div className={`grid gap-1 ${activeTab === 'emojis' ? 'grid-cols-6' : 'grid-cols-4'}`}>
            {activeTab === 'emojis'
              ? (items as EmojiItem[]).map((emoji) => (
                  <EmojiGridItem key={emoji.id} emoji={emoji} onSelect={handleEmojiSelect} />
                ))
              : (items as StickerItem[]).map((sticker) => (
                  <StickerGridItem key={sticker.id} sticker={sticker} onSelect={handleStickerSelect} />
                ))}
          </div>
        ) : (
          <div className='py-8 text-center'>
            ...
            <p className='text-[10px] text-text-muted'>
              {searchQuery ? `No ${activeTab} found for "${searchQuery}"` : `No ${activeTab} in this category`}
            </p>
          </div>
        )}
      </div>
