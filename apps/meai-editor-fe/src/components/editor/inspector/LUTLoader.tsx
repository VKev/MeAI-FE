  ...
  return (
    <div className='space-y-1'>
      <div className='flex items-center justify-between'>
        ...
        <span className='text-[10px] font-mono text-text-primary'>{percentage}%</span>
      </div>
      <Slider min={0} max={100} step={1} value={[percentage]} onValueChange={(v) => onChange(v[0] / 100)} />
    </div>
  );
};
...
function parseCubeLUT(content: string): LUTData {
  ...
  if (data.length !== expectedLength) {
    throw new Error(`Invalid LUT data: expected ${expectedLength} values, got ${data.length}`);
  }
 ...
 */
export const LUTLoader: React.FC<LUTLoaderProps> = ({ lutData, onChange, onError }) => {
    ...
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (extension === 'cube') {
          ...
          throw new Error('Unsupported file format. Please use .cube or .3dl files.');
        }

        setFileName(file.name);
        onChange(parsedLUT);
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Failed to parse LUT file';
  ...
  return (
    <div className='space-y-3'>
      {/* Hidden file input */}
      <input ref={fileInputRef} type='file' accept='.cube,.3dl' onChange={handleFileSelect} className='hidden' />
        ...
        <div className='space-y-2'>
          ...
          <div className='flex items-center justify-between p-2 bg-background-tertiary rounded-lg'>
            <div className='flex-1 min-w-0'>
              <p className='text-[10px] text-text-primary truncate'>{fileName || 'LUT Loaded'}</p>
          ...
          {/* Intensity slider */}
          <IntensitySlider value={lutData.intensity} onChange={handleIntensityChange} />
