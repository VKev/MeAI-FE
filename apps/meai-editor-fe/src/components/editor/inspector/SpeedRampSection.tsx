...
import { Play, Rewind, FastForward, Pause, RotateCcw, Trash2, ChevronDown, ChevronRight } from 'lucide-react';
import { Slider } from '@meai-editor/ui';
import { useProjectStore } from '../../../stores/project-store';
import { useTimelineStore } from '../../../stores/timeline-store';
import { getSpeedEngine, type SpeedKeyframe, SPEED_MIN, SPEED_MAX, SPEED_CURVE_PRESETS } from '@meai-editor/core';
...
const SpeedCurveCanvas: React.FC<{
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = '#444';
    ctx.setLineDash([5, 5]);
    const normalY = padding + graphHeight * (1 - (1 - SPEED_MIN) / (SPEED_MAX - SPEED_MIN));
    ctx.beginPath();
    ...
    const speedToY = (speed: number) => {
      const normalized = (Math.log(speed) - Math.log(SPEED_MIN)) / (Math.log(SPEED_MAX) - Math.log(SPEED_MIN));
    ...
    sortedKeyframes.forEach((kf) => {
      ...
      ctx.fillStyle = isDragging ? '#16a34a' : isHovered ? '#4ade80' : '#22c55e';
      ctx.fill();
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2;
      ctx.stroke();
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const speed = Math.exp(Math.log(SPEED_MAX) - normalizedY * (Math.log(SPEED_MAX) - Math.log(SPEED_MIN)));

      if (time >= 0 && time <= duration && speed >= SPEED_MIN && speed <= SPEED_MAX) {
        const clickedKf = sortedKeyframes.find((kf) => {
          const kfX = padding + (kf.time / duration) * graphWidth;
          const kfY =
            padding +
            graphHeight *
              (1 - (Math.log(kf.speed) - Math.log(SPEED_MIN)) / (Math.log(SPEED_MAX) - Math.log(SPEED_MIN)));
  ...
  return (
    <div className='relative'>
      ...
      />
      <div className='absolute bottom-1 right-1 text-[8px] text-text-muted'>Click to add/remove keyframes</div>
    </div>
  );
};
  ...
  return (
    <div className='space-y-3'>
      <div className='p-2 bg-background-tertiary rounded-lg border border-border'>
        <p className='text-[10px] text-text-muted'>Effective duration: {formatDuration(effectiveDuration)}</p>
      </div>

      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          <span className='text-[10px] font-medium text-text-secondary'>Playback Speed</span>
          <span className='text-[10px] font-mono text-primary'>{currentSpeed.toFixed(2)}x</span>
        </div>
        <Slider
      ...
      <div className='space-y-1.5'>
        <span className='text-[10px] font-medium text-text-secondary'>Speed Curve Presets</span>
      ...
      >
        ...
        {keyframes.length > 0 && <span className='ml-auto text-[9px] text-primary'>{keyframes.length} keyframes</span>}
      </button>
          ...
          {keyframes.length > 0 && (
            <div className='space-y-1 max-h-24 overflow-y-auto'>
              {keyframes.map((kf, index) => (
                <div key={kf.id} className='flex items-center gap-2 p-1.5 bg-background-tertiary rounded text-[9px]'>
                  <span className='text-text-muted'>#{index + 1}</span>
                  <span className='text-text-secondary'>{kf.time.toFixed(2)}s</span>
                  <span className='text-primary font-mono'>{kf.speed.toFixed(2)}x</span>
                  <button
      ...
      >
        ...
        {freezeFrames.length > 0 && (
          <span className='ml-auto text-[9px] text-primary'>{freezeFrames.length} freeze</span>
        )}
      </button>
      ...
      {isExpanded && (
        <div className='space-y-2'>
          ...
          {freezeFrames.length > 0 && (
            <div className='space-y-1 max-h-24 overflow-y-auto'>
              {freezeFrames.map((ff) => (
                <div key={ff.id} className='flex items-center gap-2 p-1.5 bg-background-tertiary rounded text-[9px]'>
                  <Pause size={10} className='text-primary' />
                  <span className='text-text-secondary'>{ff.startTime.toFixed(2)}s</span>
                  <span className='text-text-muted'>for</span>
                  <span className='text-primary font-mono'>{ff.duration.toFixed(1)}s</span>
                  <button
