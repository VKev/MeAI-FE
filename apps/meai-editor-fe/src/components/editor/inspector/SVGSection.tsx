...
const ColorField: React.FC<{
  ...
  <div className='flex items-center justify-between gap-2'>
    ...
    <ColorPicker value={value} onChange={onChange} className='max-w-[170px]' />
  </div>
);
...
export const SVGSection: React.FC<SVGSectionProps> = ({ clipId }) => {
  ...
  const svgClip = useMemo(() => getSVGClipById(clipId), [clipId, getSVGClipById, project.modifiedAt]);
  ...
  if (!svgClip) {
    return <div className='text-center py-8 text-text-muted text-xs'>No SVG clip selected</div>;
  }
    ...
    <div className='space-y-4'>
      <div className='space-y-3'>
        ...
        {colorStyle.colorMode !== 'none' && (
          <>
            <ColorField label='Color' value={colorStyle.tintColor || '#ffffff'} onChange={handleTintColorChange} />
            <Slider
              label='Opacity'
      ...
      <div className='space-y-3'>
        <span className='text-[10px] font-medium text-text-secondary'>Entry Animation</span>
        <Select
      ...
      <div className='space-y-4'>
        <span className='text-[10px] font-medium text-text-secondary'>Exit Animation</span>
        <Select
