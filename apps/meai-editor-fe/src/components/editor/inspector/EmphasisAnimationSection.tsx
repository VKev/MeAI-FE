...
import type { EmphasisAnimation, EmphasisAnimationType } from '@meai-editor/core';
...
export const EmphasisAnimationSection: React.FC<EmphasisAnimationSectionProps> = ({ clipId }) => {
  ...
  const clipDuration = useMemo((): number => {
    ...
    return 5;
  }, [clipId, project.timeline.tracks, getTitleEngine, getGraphicsEngine, project.modifiedAt]);
  ...
  const selectedAnimation = EMPHASIS_ANIMATIONS.flatMap((cat) => cat.animations).find(
    (a) => a.type === currentAnimation.type
  );
    ...
    <div className='space-y-4'>
      ...
      {EMPHASIS_ANIMATIONS.map((category) => (
        <div key={category.category}>
          <h4 className='text-[10px] font-medium text-text-muted mb-2'>{category.category}</h4>
      ...
      {currentAnimation.type !== 'none' && (
        <>
          <div className='pt-3 border-t border-border space-y-3'>
            {selectedAnimation && <p className='text-[10px] text-text-muted italic'>{selectedAnimation.description}</p>}
            ...
            <div className='space-y-1'>
              ...
              <Slider
                ...
                onValueChange={(value) => handleAnimationChange({ speed: value[0] })}
              />
            </div>
            ...
            <div className='flex items-center justify-between'>
              <span className='text-[10px] text-text-secondary'>Loop Animation</span>
              <button
                onClick={() => handleAnimationChange({ loop: !currentAnimation.loop })}
                className={`w-10 h-5 rounded-full transition-colors ${
                  currentAnimation.loop ? 'bg-primary' : 'bg-background-tertiary border border-border'
                }`}
              >
                <div
          ...
          <div className='pt-3 border-t border-border space-y-3'>
            <div className='flex items-center gap-2 text-primary'>
              ...
              <span className='text-[9px] text-text-muted ml-auto'>Clip: {formatTime(clipDuration)}</span>
            </div>
            ...
            <div className='space-y-1'>
              <div className='flex items-center justify-between'>
                ...
                <span className='text-[10px] font-mono text-text-primary bg-background-tertiary px-1.5 py-0.5 rounded border border-border'>
                  {currentAnimation.animationDuration ? formatTime(currentAnimation.animationDuration) : 'Full clip'}
                </span>
              </div>
              <Slider
                min={0}
                ...
                step={0.1}
                value={[currentAnimation.animationDuration ?? clipDuration - (currentAnimation.startTime ?? 0)]}
                onValueChange={(value) => {
                  const val = value[0];
                  handleAnimationChange({
              ...
              <div className='flex justify-between text-[9px] text-text-muted'>
                ...
                <span>{formatTime(clipDuration - (currentAnimation.startTime ?? 0))}</span>
              </div>
            </div>
          </div>
          ...
          {currentAnimation.type === 'focus-zoom' && (
            <div className='pt-3 border-t border-border space-y-3'>
              <div className='flex items-center gap-2 text-primary'>
                <Target size={12} />
                <span className='text-[10px] font-medium'>Focus Zoom Settings</span>
              </div>

              <div className='space-y-1'>
                <div className='flex items-center justify-between'>
                  <span className='text-[10px] text-text-secondary'>Zoom Scale</span>
                  <span className='text-[10px] font-mono text-text-primary bg-background-tertiary px-1.5 py-0.5 rounded border border-border'>
              ...
              <div className='space-y-1'>
                <div className='flex items-center justify-between'>
                  <span className='text-[10px] text-text-secondary'>Hold Duration</span>
                  <span className='text-[10px] font-mono text-text-primary bg-background-tertiary px-1.5 py-0.5 rounded border border-border'>
              ...
              <div className='space-y-2'>
                <span className='text-[10px] text-text-secondary'>Focus Point</span>
                <div className='grid grid-cols-2 gap-2'>
                  <div className='space-y-1'>
                    <span className='text-[9px] text-text-muted'>X Position</span>
                    <Slider
                      min={0}
                      max={1}
                      step={0.05}
                  ...
                  <div className='space-y-1'>
                    <span className='text-[9px] text-text-muted'>Y Position</span>
                    <Slider
                      min={0}
                      max={1}
                      step={0.05}
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`py-1.5 rounded text-[9px] transition-all ${
                        currentAnimation.focusPoint?.x === preset.x && currentAnimation.focusPoint?.y === preset.y
                          ? 'bg-primary text-white'
      ...
      <div className='pt-3 border-t border-border'>
        <div className='flex items-center gap-2 text-text-muted'>
          <Zap size={10} />
          <span className='text-[9px]'>Emphasis animations play while the clip is visible (not during entry/exit)</span>
        </div>
      </div>
    </div>
  );
