...
import { Music, Zap, Play, Pause, Plus, Search, Clock, Volume2 } from 'lucide-react';
...
const SoundCard: React.FC<SoundCardProps> = ({ sound, isPlaying, onPlay, onStop, onAdd }) => {
      ...
      <div className='flex items-center gap-2'>
        <button
          ...
          className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
            isPlaying ? 'bg-primary text-white' : 'bg-background-secondary hover:bg-primary/20'
          }`}
        >
          {isPlaying ? <Pause size={14} /> : <Play size={14} className='ml-0.5' />}
        </button>
        <div className='flex-1 min-w-0'>
          <div className='text-[10px] font-medium text-text-primary truncate'>{sound.name}</div>
          <div className='flex items-center gap-2 mt-0.5'>
            <div className='flex items-center gap-1 text-[9px] text-text-muted'>
              <Clock size={10} />
              <span>{formatDuration(sound.duration)}</span>
            </div>
            {sound.bpm && <span className='text-[9px] text-text-muted'>{sound.bpm} BPM</span>}
          </div>
        </div>
        <button
          onClick={onAdd}
      ...
      {sound.tags.length > 0 && (
        <div className='flex flex-wrap gap-1 mt-2'>
          {sound.tags.slice(0, 3).map((tag) => (
            <span key={tag} className='px-1.5 py-0.5 text-[8px] bg-background-secondary text-text-muted rounded'>
              {tag}
            </span>
          ))}
        </div>
...
export const MusicLibraryPanel: React.FC = () => {
  const getSoundLibraryEngine = useEngineStore((state) => state.getSoundLibraryEngine);
  ...
  const [selectedSfxCategory, setSelectedSfxCategory] = useState<SFXCategory | 'all'>('all');
  ...
  const sounds = useMemo(() => {
    ...
    if (selectedMoods.length > 0) {
      results = results.filter((s) => s.mood?.some((m) => selectedMoods.includes(m)));
    }

    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      results = results.filter(
        (s) => s.name.toLowerCase().includes(query) || s.tags.some((t) => t.toLowerCase().includes(query))
      );
    }

    return results;
  }, [allMusic, allSfx, activeTab, selectedGenre, selectedSfxCategory, selectedMoods, searchQuery]);
  ...
  const handleAddToTimeline = useCallback(
    async (sound: SoundItem) => {
      ...
      if (!importResult.success || !importResult.actionId) {
        console.error('[MusicLibrary] Failed to import sound:', importResult.error);
        return;
      }
  ...
  return (
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div>
          <span className='text-[11px] font-medium text-text-primary'>Music & SFX</span>
      ...
      <div className='relative'>
        <Search size={14} className='absolute left-2 top-1/2 -translate-y-1/2 text-text-muted z-10' />
        <Input
          type='text'
      ...
      <div className='space-y-2 max-h-64 overflow-y-auto'>
        {sounds.length === 0 ? (
          <div className='text-center py-6'>
            <Volume2 size={24} className='mx-auto mb-2 text-text-muted opacity-50' />
            <p className='text-[10px] text-text-muted'>No sounds found</p>
            <p className='text-[9px] text-text-muted mt-1'>Try adjusting filters</p>
          </div>
        ) : (
          sounds.map((sound) => (
            <SoundCard
