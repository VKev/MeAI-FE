...
import { Volume2, VolumeX, Mic, Music, ChevronDown, ChevronRight, Check, RefreshCw, AlertCircle } from 'lucide-react';
import { AudioDucker, resolveAudibleAudioTarget, type Clip, type Project, type Track } from '@meai-editor/core';
...
const isAudioDuckingSettings = (value: unknown): value is AudioDuckingSettings => {
  ...
  return (
    ...
    (typeof candidate.sourceTrackId === 'string' || candidate.sourceTrackId === null) &&
...
const buildTriggerTrackBuffer = async (
  ...
  try {
    ...
    for (const sourceClip of overlappingClips) {
      ...
      const mediaItem = project.mediaLibrary.items.find((item) => item.id === sourceClip.mediaId);
      ...
      try {
        ...
        const mediaOffset = sourceClip.inPoint + Math.max(0, overlapStart - sourceClip.startTime) * playbackRate;
...
export const AudioDuckingSection: React.FC<AudioDuckingSectionProps> = ({ clipId }) => {
  const project = useProjectStore((state) => state.project);
  const setClipAudioDucking = useProjectStore((state) => state.setClipAudioDucking);
  const clearClipAudioDucking = useProjectStore((state) => state.clearClipAudioDucking);
  ...
  const audioTargetClip = useMemo(
    () => (selectedClip ? resolveAudibleAudioTarget(selectedClip, project.timeline) : null),
    [project.timeline, selectedClip]
  );

  const availableSourceTracks = useMemo(() => {
    const excludedClipIds = new Set([clipId, audioTargetClip?.id].filter(Boolean) as string[]);

    return project.timeline.tracks.filter(
      (track): track is Track =>
        (track.type === 'audio' || track.type === 'video') && !track.clips.every((clip) => excludedClipIds.has(clip.id))
    );
  ...
  const hasAppliedDucking = Boolean(
    persistedSettings?.enabled && (audioTargetClip?.automation?.volume?.length ?? 0) > 0
  );
  ...
  const updateSetting = useCallback(<K extends keyof AudioDuckingSettings>(key: K, value: AudioDuckingSettings[K]) => {
  ...
  const handleApplyDucking = useCallback(async () => {
    ...
    const sourceTrack = project.timeline.tracks.find((track) => track.id === settings.sourceTrackId);
    ...
    try {
      const triggerBuffer = await buildTriggerTrackBuffer(project, audioTargetClip, sourceTrack);
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Failed to apply ducking.');
    } finally {
      setIsApplying(false);
    }
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Audio Ducking</span>
          <p className='text-[9px] text-text-muted'>Auto-lower music when speech plays</p>
        </div>
      </div>

      <div className='flex items-center justify-between p-2 bg-background-tertiary rounded-lg'>
        <div className='flex items-center gap-2'>
          <div className={`w-2 h-2 rounded-full ${showControls ? 'bg-green-400' : 'bg-gray-500'}`} />
                ...
                {availableSourceTracks
                  ...
                  .map((track) => (
                    ...
                    >
                      ...
                      <span className='flex-1 text-[10px] text-text-primary'>{getTrackLabel(track)}</span>
                      {settings.sourceTrackId === track.id && <Check size={12} className='text-primary' />}
                    </button>
                  ))}
              </div>
            ) : (
              <div className='p-3 bg-background-tertiary rounded-lg text-center'>
                <Mic size={16} className='mx-auto mb-1 text-text-muted opacity-50' />
            ...
            <>
              ...
              <div className='space-y-3'>
                <div className='space-y-1.5'>
                  <div className='flex items-center justify-between'>
                    <label className='text-[10px] text-text-secondary'>Detection Threshold</label>
                    <span className='text-[10px] font-mono text-text-primary'>{settings.threshold} dB</span>
                  </div>
                  <Slider
                    min={-50}
                    max={-10}
                  ...
                  />
                  <p className='text-[8px] text-text-muted'>Voice level that triggers ducking</p>
                </div>

                <div className='space-y-1.5'>
                  <div className='flex items-center justify-between'>
                    <label className='text-[10px] text-text-secondary'>Volume Reduction</label>
                  ...
                  <Slider
                    ...
                    onValueChange={(value) => updateSetting('reduction', value[0] / 100)}
                  />
                  <p className='text-[8px] text-text-muted'>How much to lower background music</p>
                </div>
              </div>
              ...
              {showAdvanced && (
                <div className='space-y-3 p-2 bg-background-tertiary rounded-lg'>
                  <div className='space-y-1.5'>
                    <div className='flex items-center justify-between'>
                      <label className='text-[10px] text-text-secondary'>Attack</label>
                      <span className='text-[10px] font-mono text-text-primary'>{settings.attack.toFixed(2)}s</span>
                    </div>
                    <Slider
                      min={0.01}
                      max={0.5}
                    ...
                    />
                    <p className='text-[8px] text-text-muted'>How fast volume drops when voice starts</p>
                  </div>

                  <div className='space-y-1.5'>
                    <div className='flex items-center justify-between'>
                      <label className='text-[10px] text-text-secondary'>Release</label>
                      <span className='text-[10px] font-mono text-text-primary'>{settings.release.toFixed(2)}s</span>
                    </div>
                    <Slider
                      min={0.1}
                      max={1}
                    ...
                    />
                    <p className='text-[8px] text-text-muted'>How fast volume returns after voice stops</p>
                  </div>

                  <div className='space-y-1.5'>
                    <div className='flex items-center justify-between'>
                      <label className='text-[10px] text-text-secondary'>Hold Time</label>
                      <span className='text-[10px] font-mono text-text-primary'>{settings.holdTime.toFixed(2)}s</span>
                    </div>
                    <Slider
                      min={0}
                      max={0.5}
                      ...
                      onValueChange={(value) => updateSetting('holdTime', value[0])}
                    />
                    <p className='text-[8px] text-text-muted'>Minimum time to stay ducked between words</p>
                  </div>
                </div>
              )}
              ...
              {!hasAppliedDucking ? (
                ...
                <div className='space-y-2'>
                  <div className='flex items-center gap-2 p-2 bg-green-500/10 border border-green-500/20 rounded-lg'>
                    ...
                    <span className='text-[10px] text-green-400'>Ducking Applied</span>
                  </div>
                  <div className='grid grid-cols-2 gap-2'>
                    ...
                    >
                      <RefreshCw size={10} className={isApplying ? 'animate-spin' : undefined} />
