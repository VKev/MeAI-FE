...
export const SpeedSection: React.FC<SpeedSectionProps> = ({ clip }) => {
  ...
  const [currentSpeed, setCurrentSpeed] = useState(speedEngine.getClipSpeed(clip.id) || 1);
  const [isReversed, setIsReversed] = useState(() => {
    const speedData = speedEngine.getClipSpeedData(clip.id);
    return speedData?.reverse || false;
  });

  const [customSpeed, setCustomSpeed] = useState<string>(currentSpeed.toString());
  ...
  const hasAudio = () => {
    const audioTrack = project.timeline.tracks.find(
      (track) => track.type === 'audio' && track.clips.some((audioClip) => audioClip.mediaId === clip.mediaId)
    );
    return !!audioTrack;
  };
    ...
    const tracks = project.timeline.tracks.map((track) => {
      ...
      if (clipIndex === -1) {
        if (affectAudio && track.type === 'audio') {
          const audioClipIndex = track.clips.findIndex((c) => c.mediaId === clip.mediaId);
          if (audioClipIndex !== -1) {
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='space-y-2'>
        ...
        <p className='text-xs text-text-tertiary'>Range: 0.1× (slowest) to 100× (fastest)</p>
      </div>
      ...
      {hasAudio() && (
        <div className='flex items-center justify-between p-3 rounded-lg bg-background-tertiary border border-border'>
          ...
          <Switch id='affect-audio' checked={affectAudio} onCheckedChange={setAffectAudio} />
        </div>
      )}
        ...
        <div className='space-y-2 p-3 rounded-lg bg-background-tertiary border border-border'>
          ...
          {clip.smoothSlowMo && (
            <div className='space-y-1'>
              ...
              <p className='text-[10px] text-text-tertiary'>Uses optical flow to generate smooth in-between frames</p>
            </div>
          )}
        </div>
      )}
      ...
      {(currentSpeed !== 1 || isReversed) && (
        <div className='p-3 rounded-lg bg-background-tertiary border border-border'>
          <div className='text-xs text-text-tertiary mb-1'>Current Settings</div>
