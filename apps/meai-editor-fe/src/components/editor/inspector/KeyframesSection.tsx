...
import { Key, Plus, Trash2, ChevronDown, Diamond, DiamondIcon } from 'lucide-react';
import { useProjectStore } from '../../../stores/project-store';
import { useTimelineStore } from '../../../stores/timeline-store';
import { useEngineStore } from '../../../stores/engine-store';
import { KeyframeEngine, EASING_CATEGORIES, type EasingName } from '@meai-editor/core';
import type { Keyframe, EasingType } from '@meai-editor/core';
...
const PropertySelector: React.FC<{
  ...
  const selectedLabel = selectedProperty
    ? ANIMATABLE_PROPERTIES.find((p) => p.id === selectedProperty)?.label || selectedProperty
    : 'Select Property';
          ...
          <div className='absolute top-full left-0 right-0 mt-1 bg-background-secondary border border-border rounded-lg shadow-lg z-20 max-h-64 overflow-y-auto'>
            {categories.map((category) => (
              <div key={category}>
                ...
                {ANIMATABLE_PROPERTIES.filter((p) => p.category === category).map((prop) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left text-[10px] flex items-center justify-between hover:bg-background-tertiary transition-colors ${
                        selectedProperty === prop.id ? 'bg-primary/10 text-primary' : 'text-text-primary'
                      }`}
                    >
                      <span>{prop.label}</span>
                      {hasKeyframes && <Diamond size={10} className='text-primary fill-primary' />}
                    </button>
                  );
                })}
              </div>
...
const EasingCurvePreview: React.FC<{ easing: string; size?: number }> = ({ easing, size = 16 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox='0 0 16 16' className='text-primary'>
      <path d={getPath(easing)} fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' />
    </svg>
  );
};
...
const KeyframeItem: React.FC<{
  ...
  return (
    <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg border border-border'>
      <DiamondIcon size={12} className='text-primary fill-primary flex-shrink-0' />
      <div className='flex-1 min-w-0'>
        <div className='flex items-center gap-2'>
          <span className='text-[10px] text-text-secondary'>{keyframe.time.toFixed(2)}s</span>
 ...
 */
export const KeyframesSection: React.FC<KeyframesSectionProps> = ({ clipId }) => {
  ...
  const currentValue = useMemo(() => {
    ...
    const result = keyframeEngine.getValueAtTime(propertyKeyframes, playheadPosition);
    return result.value;
  }, [selectedProperty, propertyKeyframes, playheadPosition, propertyDef]);

  const hasKeyframeAtPlayhead = useMemo(() => {
    if (!selectedProperty) return false;
    return propertyKeyframes.some((kf) => Math.abs(kf.time - playheadPosition) < 0.01);
  }, [selectedProperty, propertyKeyframes, playheadPosition]);

  const handleAddKeyframe = useCallback(() => {
    if (!selectedProperty || !clip) return;

    const newKeyframe = keyframeEngine.addKeyframe(clipId, selectedProperty, playheadPosition, currentValue, 'linear');

    const updatedKeyframes = [...keyframes, newKeyframe].sort((a, b) => a.time - b.time);
    updateClipKeyframes(clipId, updatedKeyframes);
  }, [clipId, clip, selectedProperty, playheadPosition, currentValue, keyframes, updateClipKeyframes]);

  const handleUpdateKeyframe = useCallback(
    (keyframeId: string, updates: Partial<Omit<Keyframe, 'id'>>) => {
      const updatedKeyframes = keyframeEngine.updateKeyframe(keyframes, keyframeId, updates);
      updateClipKeyframes(clipId, updatedKeyframes);
    },
  ...
  const handleDeleteKeyframe = useCallback(
    (keyframeId: string) => {
      const updatedKeyframes = keyframeEngine.removeKeyframe(keyframes, keyframeId);
      updateClipKeyframes(clipId, updatedKeyframes);
    },
    [clipId, keyframes, updateClipKeyframes]
  );
  ...
  const handleEasingChange = useCallback(
    (keyframeId: string, easing: EasingName) => {
      const updatedKeyframes = keyframeEngine.updateKeyframe(keyframes, keyframeId, { easing: easing as EasingType });
      updateClipKeyframes(clipId, updatedKeyframes);
    },
    [clipId, keyframes, updateClipKeyframes]
  );
  ...
  if (!clip) {
    return <div className='text-[10px] text-text-muted text-center py-4'>No clip selected</div>;
  }

  return (
    <div className='space-y-4'>
      <div className='space-y-2'>
        <label className='text-[10px] text-text-secondary font-medium'>Animate Property</label>
        <PropertySelector
      ...
      {selectedProperty && (
        <div className='flex items-center justify-between p-2 bg-background-tertiary rounded-lg border border-border'>
          <span className='text-[10px] text-text-secondary'>Value at {playheadPosition.toFixed(2)}s</span>
      ...
      {selectedProperty && propertyKeyframes.length > 0 && (
        <div className='space-y-2'>
          <div className='flex items-center justify-between'>
            <span className='text-[10px] text-text-secondary font-medium'>Keyframes ({propertyKeyframes.length})</span>
          </div>
      ...
      {!selectedProperty && (
        <div className='text-center py-4'>
          ...
          <p className='text-[10px] text-text-muted'>Select a property to animate</p>
        </div>
      )}
