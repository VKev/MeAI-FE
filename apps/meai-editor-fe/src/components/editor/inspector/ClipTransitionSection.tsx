...
function detectCurrentTransitions(clip: ClipLike): {
  ...
  if (entryKfs.length > 0) {
    ...
    else if (scaleKf && Number(scaleKf.value) === 0) entry.preset = 'iris-circle';
    else if (scaleKf && Number(scaleKf.value) < 1) entry.preset = 'zoom-in';
    else if (scaleKf && Number(scaleKf.value) > 1) entry.preset = 'zoom-out';
    else if (posXKf && Number(posXKf.value) < clip.transform.position.x) entry.preset = 'slide-left';
    else if (posXKf && Number(posXKf.value) > clip.transform.position.x) entry.preset = 'slide-right';
    else if (posYKf && Number(posYKf.value) < clip.transform.position.y) entry.preset = 'slide-up';
    else if (posYKf && Number(posYKf.value) > clip.transform.position.y) entry.preset = 'slide-down';
  ...
  if (exitKfs.length > 0) {
    const opacityKf = exitKfs.find((kf) => kf.property === 'opacity' && kf.time === clip.duration);
    const posXKf = exitKfs.find((kf) => kf.property === 'position.x' && kf.time === clip.duration);
    const posYKf = exitKfs.find((kf) => kf.property === 'position.y' && kf.time === clip.duration);
    const scaleKf = exitKfs.find((kf) => kf.property === 'scale.x' && kf.time === clip.duration);
    const rotKf = exitKfs.find((kf) => kf.property === 'rotation' && kf.time === clip.duration);
    const blurKf = exitKfs.find((kf) => kf.property === 'blur' && kf.time === clip.duration);

    if (blurKf) exit.preset = 'blur';
    else if (rotKf) exit.preset = 'rotate';
    else if (scaleKf && Number(scaleKf.value) === 0) exit.preset = 'iris-circle';
    else if (scaleKf && Number(scaleKf.value) > 1) exit.preset = 'zoom-in';
    else if (scaleKf && Number(scaleKf.value) < 1) exit.preset = 'zoom-out';
    else if (posXKf && Number(posXKf.value) < clip.transform.position.x) exit.preset = 'slide-left';
    else if (posXKf && Number(posXKf.value) > clip.transform.position.x) exit.preset = 'slide-right';
    else if (posYKf && Number(posYKf.value) < clip.transform.position.y) exit.preset = 'slide-up';
    else if (posYKf && Number(posYKf.value) > clip.transform.position.y) exit.preset = 'slide-down';
    else if (opacityKf) exit.preset = 'fade';

    const minTime = Math.min(...exitKfs.filter((kf) => kf.id.includes('-0')).map((kf) => kf.time));
...
export const ClipTransitionSection: React.FC<ClipTransitionSectionProps> = ({ clipId }) => {
  const {
    project,
    updateClipKeyframes,
      ...
      return {
        ...
        incomingTransition: previousClip
          ? track.transitions.find(
              (transition) => transition.clipAId === previousClip.id && transition.clipBId === currentClip.id
            )
          : undefined,
        outgoingTransition: nextClip
          ? track.transitions.find(
              (transition) => transition.clipAId === currentClip.id && transition.clipBId === nextClip.id
            )
          : undefined
      };
    }
  ...
  const clip = useMemo(() => {
    ...
    if (regularClip) return { type: 'regular' as const, data: regularClip as ClipLike };
    ...
    if (shapeClip) return { type: 'shape' as const, data: shapeClip as ClipLike };
    ...
    if (stickerClip) return { type: 'sticker' as const, data: stickerClip as ClipLike };
  ...
  const [entryPreset, setEntryPreset] = useState<TransitionPreset>(detected.entry.preset);
  const [entryDuration, setEntryDuration] = useState(detected.entry.duration);
  const [entryEasing, setEntryEasing] = useState<EasingType>(detected.entry.easing);

  const [exitPreset, setExitPreset] = useState<TransitionPreset>(detected.exit.preset);
  const [exitDuration, setExitDuration] = useState(detected.exit.duration);
  const [exitEasing, setExitEasing] = useState<EasingType>(detected.exit.easing);
  ...
  const applyTransitions = useCallback(() => {
    ...
    const clipTypeForKeyframes: ClipType = clip.type === 'regular' || clip.type === 'text' ? clip.type : 'regular';
    ...
    } else if (clip.type === 'shape' || clip.type === 'svg' || clip.type === 'sticker') {
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='space-y-2'>
        <span className='text-[10px] font-medium text-text-secondary uppercase tracking-wider'>Entry Animation</span>
          ...
          <div className='flex gap-2 mt-2'>
            <div className='flex-1'>
              ...
              <input
                ...
                value={entryDuration}
                onChange={(e) => setEntryDuration(parseFloat(e.target.value) || 0.5)}
      ...
      {/* Exit Transition */}
      <div className='space-y-2'>
        <span className='text-[10px] font-medium text-text-secondary uppercase tracking-wider'>Exit Animation</span>
        <div className='grid grid-cols-3 gap-1'>
          {PRESETS.map((preset) => (
            <button
              key={`exit-${preset.id}`}
          ...
          <div className='flex gap-2 mt-2'>
            <div className='flex-1'>
              ...
              <input
                ...
                max={clip.data.duration / 2}
                value={exitDuration}
                onChange={(e) => setExitDuration(parseFloat(e.target.value) || 0.5)}
                className='w-full px-2 py-1 text-[10px] bg-background-tertiary border border-border rounded'
              />
            </div>
            <div className='flex-1'>
      ...
      <div className='space-y-3 border-t border-border pt-3'>
        ...
        {timelineClipContext ? (
          adjacentTransitions.length > 0 ? (
            adjacentTransitions.map((adjacentTransition) => (
              ...
              >
                <div className='space-y-1'>
                  <p className='text-[10px] font-medium text-text-primary'>{adjacentTransition.title}</p>
                  <p className='text-[9px] text-text-muted'>{adjacentTransition.description}</p>
                </div>
            ...
            <p className='text-[10px] text-text-muted'>No adjacent clips are available on this track.</p>
          )
        ) : (
