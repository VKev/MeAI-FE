...
import { Play, ArrowRight, ArrowLeft, Zap, RefreshCw, Check } from 'lucide-react';
import { useProjectStore } from '../../../stores/project-store';
import { useUIStore } from '../../../stores/ui-store';
import { useEngineStore } from '../../../stores/engine-store';
import { toast } from '../../../stores/notification-store';
import { getPresetLibrary, type MotionPreset, type PresetCategory } from '../../../services/motion-presets';
import type { Keyframe, EasingType, Transform, GraphicClip } from '@meai-editor/core';
...
function generateKeyframesFromPreset(
  ...
  const prefix = category === 'entrance' ? 'motion-in' : category === 'exit' ? 'motion-out' : 'motion-emphasis';
  ...
  for (const track of preset.tracks) {
    for (let i = 0; i < track.keyframes.length; i++) {
      ...
      } else if ((track.property === 'scale.x' || track.property === 'scale') && kf.value === 1) {
...
const PresetCard: React.FC<PresetCardProps> = ({ preset, isApplied, onApply }) => {
  ...
  return (
    <button
      ...
      className={`relative w-full rounded-lg border transition-all overflow-hidden ${
        isApplied ? 'border-primary bg-primary/10' : 'border-border bg-background-tertiary hover:border-primary/50'
      }`}
    >
      <div className='relative h-20 bg-gradient-to-br from-background-secondary to-background-tertiary flex items-center justify-center overflow-hidden'>
        <div ref={previewRef} className='w-10 h-10 rounded bg-primary/80 flex items-center justify-center'>
        ...
        {isHovered && !isApplied && (
          <div className='absolute inset-0 bg-black/40 flex items-center justify-center'>
            <span className='text-[10px] text-white font-medium px-2 py-1 bg-primary rounded'>Apply</span>
          </div>
        )}
      </div>
      <div className='p-2 text-left'>
        <span className='text-[10px] font-medium text-text-primary block truncate'>{preset.name}</span>
...
export const MotionPresetsPanel: React.FC<MotionPresetsPanelProps> = ({ clipId }) => {
  ...
  const updateTextClipKeyframes = useProjectStore((state) => state.updateTextClipKeyframes);
  ...
  const [selectedCategory, setSelectedCategory] = useState<PresetCategory>('entrance');
  ...
  const clip = useMemo((): { type: string; data: ClipLike } | null => {
    ...
    const regularClip = project.timeline.tracks.flatMap((t) => t.clips).find((c) => c.id === targetClipId);
  ...
  const detectAppliedPresets = useCallback(() => {
    ...
    const hasEmphasis = keyframes.some((kf) => kf.id.startsWith('motion-emphasis-'));
  ...
  const handleApplyPreset = useCallback(
    (preset: MotionPreset) => {
      ...
      const prefix =
        preset.category === 'entrance' ? 'motion-in-' : preset.category === 'exit' ? 'motion-out-' : 'motion-emphasis-';

      const existingKeyframes = (clip.data.keyframes || []).filter((kf) => !kf.id.startsWith(prefix));
  ...
  const handleRemovePresets = useCallback(
    (category: PresetCategory) => {
      ...
      const filteredKeyframes = (clip.data.keyframes || []).filter((kf) => !kf.id.startsWith(prefix));
    ...
    [clip, targetClipId, updateClipKeyframes, updateTextClipKeyframes, getGraphicsEngine]
  );
  ...
  if (!targetClipId) {
    return (
      <div className='p-4 text-center'>
        ...
        <p className='text-[10px] text-text-muted'>Select a clip to apply motion presets</p>
      </div>
    );
  }
  ...
  return (
    <div className='space-y-4'>
      {(appliedState.entrance || appliedState.exit || appliedState.emphasis) && (
        <div className='space-y-1 p-2 bg-background-tertiary rounded-lg border border-border'>
          <span className='text-[10px] text-text-secondary font-medium'>Applied Animations</span>
        ...
        {CATEGORY_CONFIG.map((category) => {
          ...
          return (
            ...
            >
              <Icon size={12} />
              {category.name}
              {isApplied && <span className='absolute -top-1 -right-1 w-2 h-2 bg-green-500 rounded-full' />}
            </button>
          );
        })}
      </div>
      ...
      <div className='grid grid-cols-3 gap-2 max-h-72 overflow-y-auto'>
        {presets.map((preset) => {
          const categoryKey =
            preset.category === 'entrance' ? 'entrance' : preset.category === 'exit' ? 'exit' : 'emphasis';
