...
export const AutoCutSilenceSection: React.FC<AutoCutSilenceSectionProps> = ({ clipId }) => {
  const { getClip, getMediaItem } = useProjectStore();
  const [settings, setSettings] = useState<SilenceSettings>(DEFAULT_SILENCE_SETTINGS);
  ...
  const handleCutSilence = useCallback(async () => {
    ...
    try {
      const bridge = getSilenceCutBridge();
      const result = await bridge.cutSilence(clipId, analysisResult.silentRegions, (prog, msg) => {
        setProgress(prog);
        setProgressMessage(msg);
      });
      ...
      toast.error('Cut Failed', error instanceof Error ? error.message : 'Unknown error');
    } finally {
      setIsCutting(false);
    }
      ...
      <div className='space-y-3'>
        <div>
          <div className='flex items-center justify-between mb-1'>
            ...
            <span className='text-[10px] text-text-muted font-mono'>{settings.threshold} dB</span>
          </div>
          <Slider
            min={-80}
            max={-20}
          ...
          />
          <p className='text-[8px] text-text-muted mt-1'>Lower values detect more silence</p>
        </div>

        <div>
          <div className='flex items-center justify-between mb-1'>
            <label className='text-[10px] text-text-secondary'>Min Duration</label>
            <span className='text-[10px] text-text-muted font-mono'>{settings.minSilenceDuration.toFixed(1)}s</span>
          </div>
          <Slider
            min={0.1}
            max={2.0}
            ...
            onValueChange={(value) => updateSettings({ minSilenceDuration: value[0] })}
          />
          <p className='text-[8px] text-text-muted mt-1'>Minimum silence length to detect</p>
        </div>

        <div className='grid grid-cols-2 gap-2'>
          <div>
            <div className='flex items-center justify-between mb-1'>
              <label className='text-[10px] text-text-secondary'>Pad Before</label>
              <span className='text-[10px] text-text-muted font-mono'>{settings.paddingBefore.toFixed(1)}s</span>
            </div>
            <Slider
              min={0}
              max={2}
              ...
              onValueChange={(value) => updateSettings({ paddingBefore: value[0] })}
            />
          </div>
          <div>
            <div className='flex items-center justify-between mb-1'>
              <label className='text-[10px] text-text-secondary'>Pad After</label>
              <span className='text-[10px] text-text-muted font-mono'>{settings.paddingAfter.toFixed(1)}s</span>
            </div>
            <Slider
              min={0}
              max={2}
              ...
              onValueChange={(value) => updateSettings({ paddingAfter: value[0] })}
            />
          </div>
        </div>
        ...
        {analysisResult && (
          <div className='p-2 bg-background-secondary rounded border border-primary/20'>
            <div className='flex items-center justify-between mb-1'>
              <span className='text-[10px] text-text-secondary'>Silent Sections Found</span>
              <span className='text-sm font-bold text-primary'>{analysisResult.silentRegions.length}</span>
            </div>
            <div className='flex items-center justify-between'>
              <span className='text-[10px] text-text-secondary'>Total Silence</span>
              <span className='text-[10px] text-text-primary'>
                {analysisResult.totalSilenceDuration.toFixed(1)}s of {analysisResult.clipDuration.toFixed(1)}s (
                {Math.round((analysisResult.totalSilenceDuration / analysisResult.clipDuration) * 100)}
                %)
              </span>
            </div>
          </div>
        ...
        {(isAnalyzing || isCutting) && (
          <div className='space-y-1'>
            <div className='flex items-center justify-between'>
              <span className='text-[9px] text-text-muted'>{progressMessage}</span>
        ...
        <p className='text-[9px] text-text-muted text-center'>Tip: Use Ctrl+Z to undo all cuts at once</p>
      </div>
    </div>
  );
};
