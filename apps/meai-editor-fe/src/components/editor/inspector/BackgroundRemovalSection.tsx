...
const PRESET_COLORS = ['#00ff00', '#0000ff', '#ffffff', '#000000', '#ff0000', '#ffff00', '#00ffff', '#ff00ff'];

export const BackgroundRemovalSection: React.FC<BackgroundRemovalSectionProps> = ({ clipId, onSettingsChange }) => {
  const [settings, setSettings] = useState<BackgroundRemovalSettings>(DEFAULT_BACKGROUND_SETTINGS);
  const [isInitializing, setIsInitializing] = useState(false);
  const [isInitialized, setIsInitialized] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const { addTask, updateTaskProgress, completeTask, failTask } = useProcessingStore();
  ...
  }, [clipId, isInitialized, handleInitialize, updateSettings, addTask, updateTaskProgress, completeTask, failTask]);
    ...
    <div className='space-y-3'>
      ...
      {settings.enabled && (
        <div className='space-y-3 p-3 bg-background-tertiary rounded-lg'>
          <div>
            <label className='text-[10px] text-text-secondary block mb-2'>Background Mode</label>
            <div className='grid grid-cols-4 gap-1'>
              {BACKGROUND_MODES.map((mode) => (
                ...
                >
                  {mode.icon}
                  <span className='text-[9px] text-text-primary'>{mode.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {settings.mode === 'blur' && (
            <div>
              <div className='flex items-center justify-between mb-1'>
                <label className='text-[10px] text-text-secondary'>Blur Amount</label>
                <span className='text-[10px] text-text-muted font-mono'>{settings.blurAmount}px</span>
              </div>
              <Slider
                min={0}
                max={50}
                ...
                onValueChange={(value) => updateSettings({ blurAmount: value[0] })}
              />
            </div>
          )}
          ...
          {settings.mode === 'color' && (
            <div>
              <label className='text-[10px] text-text-secondary block mb-2'>Background Color</label>
          ...
          {settings.mode === 'image' && (
            <div>
              <label className='text-[10px] text-text-secondary block mb-2'>Background Image</label>
              <button
                onClick={() => {
              ...
              {settings.backgroundImageUrl && (
                <div className='mt-2 text-[9px] text-text-muted truncate'>Image loaded</div>
              )}
            </div>
          )}
          ...
          <div>
            <div className='flex items-center justify-between mb-1'>
              <label className='text-[10px] text-text-secondary'>Edge Smoothing</label>
              <span className='text-[10px] text-text-muted font-mono'>{settings.edgeBlur}</span>
            </div>
            <Slider
              min={0}
              max={10}
          ...
          <div>
            <div className='flex items-center justify-between mb-1'>
              ...
              <span className='text-[10px] text-text-muted font-mono'>{Math.round(settings.threshold * 100)}%</span>
            </div>
            <Slider
              min={0}
              max={100}
              ...
              onValueChange={(value) => updateSettings({ threshold: value[0] / 100 })}
            />
          </div>
