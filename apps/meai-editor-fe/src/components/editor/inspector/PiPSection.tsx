...
const PresetIcon: React.FC<{ type: string; className?: string }> = ({ type, className = '' }) => {
  switch (type) {
    case 'corner':
...
const ControlSlider: React.FC<{
  ...
  step?: number;
  unit?: string;
}> = ({ label, value, onChange, min = -1, max = 1, step = 0.01, unit = '' }) => {
...
export const PiPSection: React.FC<PiPSectionProps> = ({ clipId }) => {
  ...
  const handleScaleChange = useCallback(
    (axis: 'x' | 'y' | 'both', value: number) => {
      if (!clip) return;
      const newScale = axis === 'both' ? { x: value, y: value } : { ...currentTransform.scale, [axis]: value };
      const newTransform: Transform = {
        ...currentTransform,
        scale: newScale
      };
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Picture-in-Picture</span>
          <p className='text-[9px] text-text-muted'>Position and scale video overlay</p>
        </div>
      </div>

      <div className='space-y-2'>
        <span className='text-[10px] font-medium text-text-secondary'>Corner Positions</span>
      ...
      <div className='space-y-2'>
        <span className='text-[10px] font-medium text-text-secondary'>Split Screen</span>
        <div className='grid grid-cols-4 gap-1'>
      ...
      <div className='space-y-2'>
        <span className='text-[10px] font-medium text-text-secondary'>Center & Full</span>
      ...
      {showAdvanced && (
        <div className='space-y-3 pt-2 border-t border-border'>
          ...
          <div className='space-y-2'>
            <span className='text-[10px] font-medium text-text-secondary'>Scale</span>
            <ControlSlider
              label='Uniform Scale'
          ...
          <div className='space-y-2'>
            <span className='text-[10px] font-medium text-text-secondary'>Appearance</span>
            <ControlSlider
              label='Border Radius'
      ...
      <p className='text-[9px] text-text-muted text-center'>Drag clip in preview to fine-tune position</p>
    </div>
  );
};
