...
export const AutoReframeSection: React.FC<AutoReframeSectionProps> = ({ clipId, onReframeComplete }) => {
  const updateProjectDimensions = useProjectStore((state) => state.updateSettings);
  const [reframeSettings, setReframeSettings] = useState<ReframeSettings>(DEFAULT_REFRAME_SETTINGS);
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<PlatformPreset | null>('tiktok');
  ...
  const updateLocalSettings = useCallback((updates: Partial<ReframeSettings>) => {
  ...
  const handleSelectPlatform = useCallback(
    (platform: PlatformPreset) => {
      ...
      const aspectRatio = Object.entries(ASPECT_RATIO_PRESETS).find(([, v]) => Math.abs(v.ratio - config.ratio) < 0.01);
      if (aspectRatio) {
        updateLocalSettings({
  ...
  }, [isInitialized, handleInitialize, reframeSettings, selectedPlatform, onReframeComplete, updateProjectDimensions]);

  return (
    <div className='space-y-3'>
      <div className='space-y-3'>
        <div>
          <label className='text-[10px] text-text-secondary block mb-2'>Platform Presets</label>
          <div className='grid grid-cols-3 gap-1'>
            {(Object.keys(PLATFORM_PRESETS) as PlatformPreset[]).map((platform) => (
              <button
                key={platform}
              ...
              >
                ...
                <span className='truncate'>{PLATFORM_PRESETS[platform].name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className='text-[10px] text-text-secondary block mb-2'>Aspect Ratio</label>
          <div className='grid grid-cols-3 gap-1'>
              ...
              .map((ratio) => (
                <button
                  ...
                  className={`p-2 rounded text-[9px] transition-colors ${
                    reframeSettings.targetAspectRatio === ratio && !selectedPlatform
                      ? 'bg-primary/20 border border-primary text-text-primary'
                      : 'bg-background-secondary hover:bg-background-primary border border-transparent text-text-secondary'
                  }`}
                >
        ...
        <div>
          <div className='flex items-center justify-between mb-1'>
            <label className='text-[10px] text-text-secondary'>Tracking Speed</label>
        ...
        <div>
          ...
          <Slider
            ...
            onValueChange={(value) => updateLocalSettings({ smoothing: value[0] / 100 })}
          />
        </div>

        <div>
          <div className='flex items-center justify-between mb-1'>
            <label className='text-[10px] text-text-secondary'>Center Bias</label>
            <span className='text-[10px] text-text-muted font-mono'>
        ...
        <div className='flex items-center justify-between'>
          <label className='text-[10px] text-text-secondary'>Follow Subject</label>
          <button
            onClick={() =>
              updateLocalSettings({
            ...
            className={`w-8 h-4 rounded-full transition-colors ${
              reframeSettings.followSubject ? 'bg-primary' : 'bg-background-secondary'
            }`}
          >
            <div
              className={`w-3 h-3 rounded-full bg-white transition-transform ${
                reframeSettings.followSubject ? 'translate-x-4' : 'translate-x-0.5'
              }`}
            />
          </button>
        </div>
        ...
        <div className='text-[9px] text-text-muted text-center'>
          Output: {ASPECT_RATIO_PRESETS[reframeSettings.targetAspectRatio].width} x{' '}
