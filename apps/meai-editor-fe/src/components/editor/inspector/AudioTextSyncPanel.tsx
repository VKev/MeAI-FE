...
import { Button, LabeledSlider } from '@meai-editor/ui';
import {
  getBeatSyncBridge,
  type BeatSyncState,
  DEFAULT_BEAT_SYNC_CONFIG
} from '../../../bridges/audio-text-sync-bridge';
import type { SyncMode } from '@meai-editor/core';
...
export const AudioTextSyncPanel: React.FC<BeatSyncPanelProps> = ({ clipId }) => {
  ...
  const { isProcessing, progress, beatAnalysis, selectedTrackIds, clipsToSync, previewTimings, config, error } = state;
        ...
        <div className='space-y-4'>
          <div className='flex items-center justify-between p-3 bg-primary/10 rounded-lg border border-primary/30'>
            ...
            <div className='text-right'>
              ...
              <span className='text-sm font-medium text-text-primary'>{beatAnalysis.beats.length}</span>
            </div>
          </div>

          <div className='space-y-2'>
            <span className='text-[10px] text-text-secondary block'>Select tracks to sync to beats:</span>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex gap-1'>
                    ...
                    <button
                      ...
                      className={`px-2 py-1 text-[9px] rounded transition-colors ${
                        config.snapToDownbeats ? 'bg-primary text-black' : 'bg-background-secondary text-text-secondary'
                      }`}
                    >
                      Downbeats
                    </button>
            ...
            <div className='space-y-2'>
              ...
              <div className='max-h-24 overflow-y-auto bg-background-tertiary rounded-lg p-2 space-y-1'>
                {previewTimings.slice(0, 5).map((timing, idx) => (
                  <div key={timing.clipId} className='flex items-center justify-between text-[9px]'>
                ...
                {previewTimings.length > 5 && (
                  <span className='text-[9px] text-text-muted'>...and {previewTimings.length - 5} more</span>
                )}
              </div>
            </div>
          )}
          ...
          <Button onClick={handleAnalyzeBeats} variant='outline' className='w-full'>
            Re-analyze Beats
          </Button>
        </div>
      )}
