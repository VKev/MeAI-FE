...
import { Layers, Plus, Trash2, Eye, EyeOff, ChevronDown, ChevronRight, Palette, Droplet, Copy } from 'lucide-react';
import { Slider } from '@meai-editor/ui';
import { useEngineStore } from '../../../stores/engine-store';
import { useProjectStore } from '../../../stores/project-store';
import type { AdjustmentLayer, BlendMode, Effect } from '@meai-editor/core';
...
export const AdjustmentLayerSection: React.FC<AdjustmentLayerSectionProps> = ({ clipId }) => {
  const getAdjustmentLayerEngine = useEngineStore((state) => state.getAdjustmentLayerEngine);
  const project = useProjectStore((state) => state.project);

  const [expandedLayer, setExpandedLayer] = useState<string | null>(null);
  const [showBlendModes, setShowBlendModes] = useState(false);
  const [adjustmentLayerEngine, setAdjustmentLayerEngine] = useState<
    import('@meai-editor/core').AdjustmentLayerEngine | null
  >(null);
  ...
  const handleCreateLayer = useCallback(() => {
    ...
    const layer = adjustmentLayerEngine.createAdjustmentLayer(currentTrack.id, startTime, {
      duration,
      name: `Adjustment ${allLayers.length + 1}`
    });
  ...
  const renderLayerItem = (layer: AdjustmentLayer) => {
    ...
    return (
      <div key={layer.id} className='bg-background-tertiary rounded-lg overflow-hidden'>
        <div
        ...
        >
          ...
          <span className='flex-1 text-[10px] text-text-primary truncate'>{layer.name}</span>
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          <div className='px-2 pb-2 space-y-3'>
            <div className='space-y-1.5'>
              <div className='flex items-center justify-between'>
                ...
                <span className='text-[10px] font-mono text-text-primary'>{Math.round(layer.opacity * 100)}%</span>
              </div>
              <Slider
                min={0}
                max={100}
            ...
            <div className='space-y-1.5'>
              ...
              <div className='relative'>
                ...
                >
                  <span>{BLEND_MODES.find((m) => m.id === layer.blendMode)?.name || 'Normal'}</span>
                  <ChevronDown size={10} />
                </button>
                {showBlendModes && (
                    ...
                    {BLEND_MODES.map((mode) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-[10px] hover:bg-background-tertiary transition-colors ${
                          layer.blendMode === mode.id ? 'text-indigo-400 bg-indigo-500/10' : 'text-text-secondary'
                        }`}
                      >
                        {mode.name}
                      </button>
              ...
              {layer.effects.length > 0 && (
                <div className='space-y-1'>
                  {layer.effects.map((effect) => (
                    ...
                    >
                      <span className='text-[9px] text-text-primary capitalize'>{effect.type}</span>
                      <button
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r from-indigo-500/20 to-violet-500/20 rounded-lg border border-indigo-500/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Adjustment Layers</span>
          <p className='text-[9px] text-text-muted'>Non-destructive effects on clips below</p>
        </div>
      </div>
      ...
      {trackLayers.length > 0 && (
        <div className='space-y-2'>
          <span className='text-[10px] font-medium text-text-secondary'>Track Layers ({trackLayers.length})</span>
      ...
      {allLayers.length > trackLayers.length && (
        <div className='space-y-2 pt-2 border-t border-border'>
          <span className='text-[10px] font-medium text-text-secondary'>Other Layers</span>
          <div className='space-y-1.5'>
            {allLayers.filter((l) => !trackLayers.some((tl) => tl.id === l.id)).map(renderLayerItem)}
          </div>
        </div>
      )}

      <div className='pt-2 border-t border-border'>
        <p className='text-[9px] text-text-muted text-center'>Apply color, effects to all clips below</p>
      </div>
    </div>
  );
};
