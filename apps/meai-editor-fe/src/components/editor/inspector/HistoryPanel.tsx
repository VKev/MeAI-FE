    ...
    <div className='flex flex-col h-full'>
      ...
      <div className='border-b border-border'>
        ...
        >
          ...
          <span className='text-xs text-text-secondary'>Snapshots ({snapshots.length})</span>
        </button>

        {showSnapshots && (
          <div className='px-2 pb-2'>
            {snapshots.length === 0 && !isCreatingSnapshot && (
              <p className='text-[10px] text-text-muted py-2 text-center'>No snapshots saved</p>
            )}
              ...
              >
                <div className='flex items-center gap-2'>
                  ...
                  <div>
                    ...
                    <p className='text-[10px] text-text-muted'>{formatTime(snapshot.timestamp)}</p>
                  </div>
                </div>
                <button
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-2 p-2 rounded transition-colors ${
                    item.isCurrent ? 'bg-primary/20 border border-primary/30' : 'hover:bg-background-tertiary'
                  }`}
                >
                  {item.isClipEntry && ClipIcon ? (
                    <ClipIcon size={12} className={item.isCurrent ? 'text-primary' : 'text-text-muted'} />
                  ) : (
                    <div className={`w-1.5 h-1.5 rounded-full ${item.isCurrent ? 'bg-primary' : 'bg-text-muted'}`} />
                  )}
                  <div className='flex-1 min-w-0'>
                    <p className={`text-xs truncate ${item.isCurrent ? 'text-text-primary' : 'text-text-secondary'}`}>
                      {item.description}
                    </p>
                  ...
                  {item.isClipEntry && (
                    <span className='px-1 py-0.5 bg-amber-500/20 rounded text-[8px] text-amber-400'>clip</span>
                  )}
                </div>
              );
            })}
