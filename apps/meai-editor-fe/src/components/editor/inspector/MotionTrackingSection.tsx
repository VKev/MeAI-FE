...
import { Slider, Checkbox, Label } from '@meai-editor/ui';
import { getMotionTrackingBridge, type MotionTrackingState } from '../../../bridges/motion-tracking-bridge';
import type { Rectangle } from '@meai-editor/core';
...
export const MotionTrackingSection: React.FC<MotionTrackingSectionProps> = ({ clipId }) => {
  ...
  const hasTrackingData = state.trackingData !== null || bridge.hasTrackingData(clipId);
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Motion Tracking</span>
          <p className='text-[9px] text-text-muted'>Track objects to attach elements</p>
        </div>
      </div>
      ...
      {!state.isTracking && !hasTrackingData && (
        <>
          <div className='space-y-2'>
            <label className='text-[10px] font-medium text-text-secondary'>Tracking Region</label>
            ...
            <p className='text-[9px] text-text-muted text-center'>Draw region in preview or enter coordinates</p>
          </div>
          ...
          {showAdvanced && (
            <div className='space-y-3 p-2 bg-background-tertiary rounded-lg'>
              <div className='space-y-1.5'>
                <label className='text-[10px] font-medium text-text-secondary'>Algorithm</label>
                    ...
                    >
                      <div
                        className={`w-3 h-3 rounded-full border-2 flex items-center justify-center ${
                          algorithm === algo.id ? 'border-primary' : 'border-border'
                        }`}
                      >
                        {algorithm === algo.id && <div className='w-1.5 h-1.5 bg-primary rounded-full' />}
                      </div>
                      <div className='flex-1'>
                        <span className='text-[10px] font-medium text-text-primary'>{algo.name}</span>
                        <p className='text-[8px] text-text-muted'>{algo.description}</p>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className='space-y-1.5'>
                <div className='flex items-center justify-between'>
                  <label className='text-[10px] text-text-secondary'>Confidence Threshold</label>
                  <span className='text-[10px] font-mono text-text-primary'>{confidenceThreshold}%</span>
                </div>
                <Slider
                  min={30}
                  max={95}
                ...
                />
                <p className='text-[8px] text-text-muted'>Higher = more accurate but may lose track easier</p>
              </div>

              <div className='space-y-1.5'>
                <div className='flex items-center justify-between'>
                  <label className='text-[10px] text-text-secondary'>Path Smoothing</label>
                  <span className='text-[10px] font-mono text-text-primary'>{smoothing}</span>
                </div>
                <Slider
                  min={0}
                  max={10}
                ...
                />
                <p className='text-[8px] text-text-muted'>Reduces jitter in tracking path</p>
              </div>
            </div>
          )}
        ...
        <div className='space-y-3 p-3 bg-background-tertiary rounded-lg'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-2'>
              ...
              <span className='text-[11px] font-medium text-primary'>Tracking in Progress</span>
            </div>
            <button
          ...
          <div className='space-y-1'>
            <div className='flex items-center justify-between text-[10px]'>
              ...
              <span className='font-mono text-text-primary'>{Math.round(state.progress)}%</span>
            </div>
      ...
      {hasTrackingData && !state.isTracking && (
        <div className='space-y-3'>
          <div className='flex items-center gap-2 p-2 bg-green-500/10 border border-green-500/30 rounded-lg'>
            ...
            <div className='flex-1'>
              <span className='text-[10px] font-medium text-green-400'>Tracking Complete</span>
              {state.trackingData && (
                <p className='text-[9px] text-green-300/70'>
                  {state.trackingData.keyframes.length} keyframes captured
                  {state.trackingData.lostFrames.length > 0 && ` • ${state.trackingData.lostFrames.length} frames lost`}
                </p>
              )}
            </div>
          </div>
          ...
          <div className='space-y-2'>
            <Label className='text-[10px] font-medium text-text-secondary'>Transform Options</Label>
            <div className='grid grid-cols-2 gap-2'>
              <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg'>
                ...
                />
                <Label htmlFor='apply-scale' className='flex items-center gap-1 cursor-pointer'>
              ...
              <div className='flex items-center gap-2 p-2 bg-background-tertiary rounded-lg'>
                ...
                />
                <Label htmlFor='apply-rotation' className='flex items-center gap-1 cursor-pointer'>
                  <RotateCcw size={10} className='text-text-muted' />
                  <span className='text-[10px] text-text-secondary'>Rotation</span>
                </Label>
              </div>
            </div>
          </div>
          ...
          {!isApplied ? (
            ...
            <div className='space-y-2'>
              <div className='flex items-center gap-2 p-2 bg-primary/10 border border-primary/20 rounded-lg'>
                ...
                <span className='text-[10px] text-primary'>Tracking Applied</span>
              </div>
              <button
      ...
      <div className='pt-2 border-t border-border'>
        <p className='text-[9px] text-text-muted text-center'>Track objects to pin graphics, text, or effects</p>
      </div>
    </div>
  );
};
