...
import { Video, Camera, Plus, Trash2, ChevronDown, ChevronRight, Check, Link } from 'lucide-react';
import { useProjectStore } from '../../../stores/project-store';
import { useEngineStore } from '../../../stores/engine-store';
import type { MultiCamGroup, CameraAngle } from '@meai-editor/core';
  ...
  return (
    <div
      className={`p-2 rounded-lg border transition-colors cursor-pointer ${
        isActive ? 'bg-primary/20 border-primary' : 'bg-background-tertiary border-border hover:border-primary/50'
      }`}
      onClick={onSelect}
    >
...
const GroupSection: React.FC<{
  ...
  <div className='border border-border rounded-lg overflow-hidden'>
    ...
    >
      ...
      <span className='flex-1 text-left text-[10px] font-medium text-text-primary'>{group.name}</span>
      <span className='text-[9px] text-text-muted'>{group.angles.length} angles</span>
    </button>
    {isExpanded && (
...
export const MultiCameraPanel: React.FC<MultiCameraPanelProps> = () => {
  ...
  const [multiCamEngine, setMultiCamEngine] = useState<import('@meai-editor/core').MultiCamEngine | null>(null);
  ...
  const handleCreateGroup = useCallback(() => {
    ...
    const group = multiCamEngine.createGroup(`Multi-Cam ${groups.length + 1}`, selectedClips);
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Multi-Camera Editing</span>
          <p className='text-[9px] text-text-muted'>Sync and switch between camera angles</p>
        </div>
      </div>

      {groups.length > 0 && (
        <div className='space-y-2'>
          <span className='text-[10px] font-medium text-text-secondary'>Camera Groups</span>
          {groups.map((group) => (
            <GroupSection
              key={group.id}
              group={group}
              ...
              onRenameAngle={(angleId, name) => handleRenameAngle(group.id, angleId, name)}
              onOffsetChange={(angleId, offset) => handleOffsetChange(group.id, angleId, offset)}
      ...
      <div className='space-y-2 pt-2 border-t border-border'>
        <span className='text-[10px] font-medium text-text-secondary'>Create New Group</span>
        <p className='text-[9px] text-text-muted'>Select 2+ video clips to create a multi-camera group</p>

        {availableClips.length === 0 ? (
          <div className='text-center py-4'>
            <Video size={24} className='mx-auto mb-2 text-text-muted opacity-50' />
            <p className='text-[10px] text-text-muted'>Import video clips to use multi-camera editing</p>
          </div>
        ) : (
          <>
                ...
                >
                  <div
                    className={`w-4 h-4 rounded border flex items-center justify-center ${
                      selectedClips.includes(clip.id) ? 'bg-primary border-primary' : 'border-border'
                    }`}
                  >
                    {selectedClips.includes(clip.id) && <Check size={10} className='text-white' />}
                  </div>
                  <div className='flex-1'>
                    <span className='text-[10px] text-text-primary'>{clip.name}</span>
                    <span className='text-[8px] text-text-muted ml-1'>({clip.trackName})</span>
                  </div>
                </button>
              ))}
            </div>
      ...
      <p className='text-[9px] text-text-muted text-center'>Switch angles during playback to create cuts</p>
    </div>
  );
};
