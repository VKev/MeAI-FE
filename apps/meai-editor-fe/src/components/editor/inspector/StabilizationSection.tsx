...
import { getVidstabEngine, type VidstabProgress } from '@meai-editor/core';
import { useProjectStore } from '../../../stores/project-store';
import { Switch, Label, Slider, Button } from '@meai-editor/ui';

interface StabilizationSectionProps {
  clip: Clip;
}

export const StabilizationSection: React.FC<StabilizationSectionProps> = ({ clip }) => {
  ...
  return (
    <div className='space-y-3'>
      <div className='flex items-center justify-between'>
        ...
        <Switch checked={stabilization.enabled && isStabilized} onCheckedChange={handleToggle} disabled={processing} />
      </div>

      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          <Label className='text-xs text-muted-foreground'>Strength</Label>
          <span className='text-xs text-muted-foreground'>{stabilization.strength}%</span>
        </div>
        <Slider
      ...
      {isStabilized && !processing && (
        <Button variant='outline' size='sm' className='w-full' onClick={handleStabilize}>
          Re-stabilize
        </Button>
      )}
    </div>
