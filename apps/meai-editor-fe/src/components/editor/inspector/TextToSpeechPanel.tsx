...
import { Mic, Loader2, Volume2, Settings, Sparkles, AlertTriangle } from 'lucide-react';
import { Slider, Switch } from '@meai-editor/ui';
...
export const TextToSpeechPanel: React.FC = () => {
  ...
  const defaultProvider: TtsProvider = defaultTtsProvider === 'elevenlabs' && hasElevenLabsKey ? 'elevenlabs' : 'piper';

  const [provider, setProvider] = useState<TtsProvider>(defaultProvider);
  const [text, setText] = useState('');
  const [selectedVoice, setSelectedVoice] = useState<string>(
    defaultProvider === 'elevenlabs' && favoriteVoices.length > 0 ? favoriteVoices[0].voiceId : 'amy'
  );
  ...
  const handleProviderSwitch = useCallback(
    (newProvider: TtsProvider) => {
      ...
      setSelectedVoice(
        newProvider === 'elevenlabs' ? (favoriteVoices.length > 0 ? favoriteVoices[0].voiceId : '') : 'amy'
      );
        ...
        <div className='flex items-center justify-between'>
          {provider === 'elevenlabs' ? (
            <div className='flex items-center gap-1.5'>
              ...
              <label
                className='text-[9px] text-text-muted flex items-center gap-1 cursor-pointer'
