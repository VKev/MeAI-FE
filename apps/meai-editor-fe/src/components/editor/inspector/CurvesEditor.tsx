...
const ChannelTab: React.FC<{
  ...
  <button
    ...
    className={`px-3 py-1 text-[10px] font-medium rounded transition-colors ${
      isActive ? 'bg-background-tertiary text-text-primary' : 'text-text-muted hover:text-text-secondary'
    }`}
    style={{
 ...
 */
export const CurvesEditor: React.FC<CurvesEditorProps> = ({ values, onChange, onReset: _onReset }) => {
  void _onReset;
  ...
  const handlePointDrag = useCallback(
    (e: React.MouseEvent | MouseEvent) => {
      ...
      const x = Math.max(0, Math.min(1, (e.clientX - rect.left - padding) / (canvasWidth - 2 * padding)));
      const y = Math.max(0, Math.min(1, 1 - (e.clientY - rect.top - padding) / (canvasHeight - 2 * padding)));
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      const y = 1 - (e.clientY - rect.top - padding) / (canvasHeight - 2 * padding);
  ...
  const curvePath = useMemo(() => {
    return generateCurvePath(currentPoints, canvasWidth - 2 * padding, canvasHeight - 2 * padding);
  }, [currentPoints]);

  // Generate diagonal reference line
  const diagonalPath = `M ${padding} ${canvasHeight - padding} L ${canvasWidth - padding} ${padding}`;
    ...
    <div className='space-y-3'>
      ...
      <div className='relative bg-background-tertiary rounded-lg overflow-hidden'>
        ...
        >
          {/* Grid lines */}
          <defs>
            <pattern id='grid' width={canvasWidth / 4} height={canvasHeight / 4} patternUnits='userSpaceOnUse'>
              <path
          ...
          <path d={diagonalPath} fill='none' stroke='rgba(255,255,255,0.2)' strokeWidth='1' strokeDasharray='4,4' />
      ...
      <div className='flex justify-between items-center'>
        <span className='text-[9px] text-text-muted'>Click to add point • Double-click to remove</span>
        <button
      ...
      <div className='text-[9px] text-text-muted text-center'>{currentPoints.length} points</div>
    </div>
  );
};
