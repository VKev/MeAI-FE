...
import { Layers, FolderOpen, Plus, Copy, Trash2, Edit3, Maximize2, ChevronRight, Check, X } from 'lucide-react';
import { useEngineStore } from '../../../stores/engine-store';
import { useProjectStore } from '../../../stores/project-store';
import { useUIStore } from '../../../stores/ui-store';
import type { CompoundClip } from '@meai-editor/core';

interface NestedSequenceSectionProps {
  clipId: string;
}

export const NestedSequenceSection: React.FC<NestedSequenceSectionProps> = ({ clipId }) => {
  const getNestedSequenceEngine = useEngineStore((state) => state.getNestedSequenceEngine);
  ...
  const [nestedSequenceEngine, setNestedSequenceEngine] = useState<
    import('@meai-editor/core').NestedSequenceEngine | null
  >(null);
  ...
  const handleCreateCompound = useCallback(() => {
    ...
    const compound = nestedSequenceEngine.createCompoundClip(fullClips, project.timeline.tracks);
  ...
  }, [nestedSequenceEngine, selectedClips, selectedClipIds, project.timeline.tracks]);
    ...
    <div className='space-y-3'>
      <div className='flex items-center gap-2 p-2 bg-gradient-to-r bg-primary/10 rounded-lg border border-primary/30'>
        ...
        <div className='flex-1'>
          <span className='text-[11px] font-medium text-text-primary'>Nested Sequences</span>
          <p className='text-[9px] text-text-muted'>Create compound clips from selections</p>
        </div>
      </div>
      ...
      {currentCompound && (
        <div className='p-3 bg-primary/10 border border-primary/20 rounded-lg space-y-2'>
          <div className='flex items-center gap-2'>
            ...
            <span className='text-[11px] font-medium text-text-primary'>{currentCompound.name}</span>
          </div>
      ...
      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          <span className='text-[10px] font-medium text-text-secondary'>Create Compound Clip</span>
          <span className='text-[9px] text-text-muted'>{selectedClips.length} clips selected</span>
        </div>
        <button
        ...
        {selectedClips.length < 2 && (
          <p className='text-[9px] text-text-muted text-center'>Select 2+ clips to create a compound clip</p>
        )}
      </div>

      {allCompoundClips.length > 0 && (
        <div className='space-y-2'>
          <span className='text-[10px] font-medium text-text-secondary'>Compound Clips Library</span>
          <div className='space-y-1.5'>
            {allCompoundClips.map((compound) => {
              const instanceCount = nestedSequenceEngine?.getInstanceCount(compound.id) || 0;
              const isExpanded = expandedCompound === compound.id;
              const isRenaming = renamingId === compound.id;

              return (
                <div key={compound.id} className='bg-background-tertiary rounded-lg overflow-hidden'>
                  <div
                  ...
                  >
                    ...
                    />
                    <div className='w-3 h-3 rounded' style={{ backgroundColor: compound.color }} />
                    {isRenaming ? (
                      <input
                        type='text'
                        value={renameValue}
                      ...
                      <span className='flex-1 text-[10px] text-text-primary truncate'>{compound.name}</span>
                    )}
                        ...
                        {isRenaming ? (
                          ...
                          <>
                            ...
                            <button
                              ...
                              title={instanceCount > 0 ? 'Cannot delete - has instances' : 'Delete'}
                            >
                              <Trash2 size={10} />
                            </button>
                          </>
      ...
      <div className='pt-2 border-t border-border'>
        <p className='text-[9px] text-text-muted text-center'>Group clips into reusable compound clips</p>
      </div>
    </div>
  );
};
