...
import { getGSAPEngine, generateDefaultControlPoints, type GSAPMotionPathPoint } from '@meai-editor/core';
import { Button, Switch } from '@meai-editor/ui';

interface MotionPathSectionProps {
  clipId: string;
}

export const MotionPathSection: React.FC<MotionPathSectionProps> = ({ clipId }) => {
  ...
  if (!clip) {
    return <div className='text-center py-8 text-text-muted text-xs'>No clip selected</div>;
  }
    ...
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-xs font-medium text-text-primary'>Motion Path</span>
        </div>
      ...
      {isEnabled && (
        <>
          <div className='p-3 bg-background-tertiary rounded-lg space-y-3'>
              ...
              <div className='flex items-center gap-2'>
                <button
                  ...
                  className={`p-1.5 rounded transition-colors ${
                    showPath ? 'bg-primary/20 text-primary' : 'bg-background-elevated text-text-muted'
                  }`}
                >
            ...
            <div className='flex items-center justify-between'>
              <span className='text-[10px] text-text-secondary'>Auto Orient</span>
              <Switch checked={autoOrient} onCheckedChange={handleAutoOrientToggle} />
            </div>
          ...
          <div className='flex items-center justify-between p-3 bg-background-tertiary rounded-lg'>
            <div>
              <span className='text-[10px] text-text-secondary'>Path Points</span>
              <p className='text-sm font-medium text-text-primary'>{pointCount} points</p>
            </div>
          ...
          {isEditing && (
            <div className='p-2 bg-primary/10 border border-primary/30 rounded-lg'>
              <p className='text-[9px] text-primary'>
                <span className='font-medium'>Editing:</span> Click on the path to add points. Drag points to move them.
                Right-click to remove. Drag handles to adjust curves.
              </p>
            </div>
          )}
          ...
          <div className='p-2 bg-background-tertiary/50 border border-border rounded-lg'>
            <p className='text-[9px] text-text-muted'>
              <span className='text-text-secondary font-medium'>Tip:</span> Motion paths animate the clip's position
              along a curved path over time. Use bezier handles for smooth curves.
            </p>
          </div>
        </>
      )}
