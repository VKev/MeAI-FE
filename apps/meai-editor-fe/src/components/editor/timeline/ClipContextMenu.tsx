...
export const ClipContextMenu: React.FC<ClipContextMenuProps> = ({ clip, track, onClose }) => {
  const {
    copyClips,
    duplicateClip,
    removeClip,
  ...
  const isPlayheadOnClip = playheadPosition >= clip.startTime && playheadPosition <= clip.startTime + clip.duration;
  ...
  const isVideoWithAudio =
    isVideo && mediaItem?.type === 'video' && mediaItem?.metadata?.channels && mediaItem.metadata.channels > 0;
