...
export const KeyframeTrack: React.FC<KeyframeTrackProps> = ({
  ...
  if (propertyGroups.length === 0) {
    return <div className='h-8 flex items-center justify-center text-[9px] text-text-muted'>No keyframes</div>;
  }
      ...
      {propertyGroups.map((group) => (
        ...
        >
          <div className='absolute left-0 top-0 bottom-0 w-20 flex items-center px-2 bg-background-tertiary/50 border-r border-border/30 z-10'>
            <div className='w-2 h-2 rounded-full mr-1.5 flex-shrink-0' style={{ backgroundColor: group.color }} />
            <span className='text-[9px] text-text-muted truncate'>{group.label}</span>
          </div>
              ...
              return (
                <React.Fragment key={keyframe.id}>
                  ...
                  <KeyframeMarker
                    ...
                    onSelect={(addToSelection) => onKeyframeSelect(keyframe.id, addToSelection)}
                    onMove={(deltaPixels) => handleKeyframeMove(keyframe.id, deltaPixels)}
