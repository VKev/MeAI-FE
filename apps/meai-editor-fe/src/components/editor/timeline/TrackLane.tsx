...
interface TrackLaneProps {
  ...
  onTrimTextClip: (clipId: string, edge: 'left' | 'right', newTime: number) => void;
  onTrimShapeClip: (clipId: string, edge: 'left' | 'right', newTime: number) => void;
  scrollX: number;
  trackHeight: number;
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        ...
        const snapResult = calculateSnap(rawTime, '', allTracks, playheadPosition, snapSettings, pixelsPerSecond);
      ...
      try {
        ...
        if (!data || typeof data !== 'object' || typeof data.mediaId !== 'string' || !data.mediaId.trim()) {
          return;
        }
    ...
    <div className='relative'>
      <div
        ...
        className={`border-b border-border/50 relative transition-colors ${
          isDragOver ? 'bg-primary/10 border-primary/30' : 'bg-background-secondary/20'
        }`}
      ...
      >
        ...
        {isDragOver && (
          <div className='absolute inset-0 border-2 border-dashed border-primary/50 rounded pointer-events-none flex items-center justify-center'>
            <span className='text-xs text-primary bg-background/80 px-2 py-1 rounded'>Drop to add clip</span>
          </div>
        )}
      </div>
      <div
      ...
      {isExpanded && clipsWithKeyframes.length > 0 && (
        <div className='absolute left-0 right-0' style={{ top: trackHeight }}>
          {clipsWithKeyframes.map((clip) => (
            <div key={`keyframes-${clip.id}`} className='relative' style={{ left: clip.startTime * pixelsPerSecond }}>
              <KeyframeTrack
                clip={clip}
                pixelsPerSecond={pixelsPerSecond}
