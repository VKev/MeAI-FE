...
export const ClipComponent: React.FC<ClipComponentProps> = ({
  ...
  const effectApplicationLabel = useUIStore((state) => state.effectApplicationLabel);
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
          ...
          className={`group absolute top-1 bottom-1 rounded-lg overflow-hidden shadow-sm ${
            ...
            isSelected && !isDragging
              ...
              : !isDragging
                ...
                : ''
          } ${clipStyle.bg} border ${clipStyle.border} ${track.locked ? 'cursor-not-allowed opacity-60' : ''}`}
          style={{
            transform: isDragging ? `translate(${left}px, ${dragYOffset}px)` : `translateX(${left}px)`,
            width: `${width}px`,
        ...
        >
          ...
          {isVideo && (mediaItem?.filmstripThumbnails?.length || mediaItem?.thumbnailUrl) && (
            <div className='absolute inset-0 flex pointer-events-none'>
              {mediaItem?.filmstripThumbnails && mediaItem.filmstripThumbnails.length > 0
                ? Array.from({ length: thumbnailCount }).map((_, i) => {
                    const clipProgress = i / Math.max(1, thumbnailCount - 1);
                    const thumbIndex = Math.min(
                      Math.floor(clipProgress * mediaItem.filmstripThumbnails!.length),
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          borderRight: i < thumbnailCount - 1 ? '1px solid rgba(0,0,0,0.2)' : 'none'
                        }}
                      />
                    );
                  })
          ...
          {isImage && (
            <div className='absolute inset-0 bg-gradient-to-r from-purple-500/20 to-purple-500/10 flex items-center justify-center pointer-events-none'>
              {mediaItem?.thumbnailUrl ? (
                <img src={mediaItem.thumbnailUrl} alt={clipName} className='h-full object-cover opacity-60' />
              ) : (
          ...
          {(isAudio || isVideo) && (
            <>
              <div
                className={`absolute inset-x-0 px-1 pointer-events-none ${isAudio ? 'inset-y-0 flex items-center opacity-50' : 'bottom-0 h-1/3 flex items-end opacity-30'}`}
              >
