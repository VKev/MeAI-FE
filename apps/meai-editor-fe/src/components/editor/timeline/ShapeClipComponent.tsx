...
export const ShapeClipComponent: React.FC<ShapeClipComponentProps> = ({
  ...
  const shapeLabel =
    isShape && 'shapeType' in shapeClip
      ? shapeClip.shapeType.charAt(0).toUpperCase() + shapeClip.shapeType.slice(1)
      : isSticker
      ...
      <ContextMenuTrigger asChild>
        ...
        >
          <div
            className={`absolute left-0 top-0 bottom-0 w-2 cursor-ew-resize z-20 flex items-center justify-center transition-opacity ${
              isSelected
                ...
                : `opacity-0 group-hover:opacity-100 hover:bg-${colorClass}-400/50`
            }`}
          ...
          <div className='w-full h-full flex items-center gap-1 px-3'>
            <IconComponent size={12} className={`text-${colorClass}-400 flex-shrink-0`} />
            <span className={`text-[10px] font-medium text-${colorClass}-200 truncate`}>{shapeLabel}</span>
          </div>
          {isSelected && <div className='absolute inset-0 border-2 border-green-400 rounded-lg pointer-events-none' />}
        </div>
      </ContextMenuTrigger>
