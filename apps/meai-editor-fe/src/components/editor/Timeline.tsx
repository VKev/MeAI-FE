...
export const Timeline: React.FC = () => {
  ...
  const { select, selectMultiple, clearSelection, getSelectedClipIds, snapSettings, toggleSnap } = useUIStore();
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        setViewportDimensions(entry.contentRect.width, entry.contentRect.height);
      }
    });
  ...
  const handleSelectClip = useCallback(
    (clipId: string, addToSelection: boolean) => {
      ...
      if (isTextClip) {
        ...
        select({ type: 'text-clip', id: clipId, trackId: textClip?.trackId }, addToSelection);
        return;
      }
      const isShapeClip = allShapeClips.some((sc) => sc.id === clipId);
      if (isShapeClip) {
        const shapeClip = allShapeClips.find((sc) => sc.id === clipId);
        select({ type: 'shape-clip', id: clipId, trackId: shapeClip?.trackId }, addToSelection);
        return;
      }
    ...
    (keyframeId: string) => {
      for (const track of tracks) {
        for (const clip of track.clips) {
          const keyframe = clip.keyframes?.find((kf) => kf.id === keyframeId);
          if (keyframe) {
            const updatedKeyframes = clip.keyframes?.filter((kf) => kf.id !== keyframeId);
            if (updatedKeyframes) {
              updateClipKeyframes(clip.id, updatedKeyframes);
            }
  ...
  const handleBoxSelectionEnd = useCallback(() => {
    ...
    setIsBoxSelecting(false);
    setSelectionBox(null);
  }, [isBoxSelecting, selectionBox, pixelsPerSecond, tracks, getTrackHeight, selectMultiple]);
  ...
  const handleDropMedia = useCallback(async (trackId: string, mediaId: string, startTime: number) => {
  ...
  const [snapIndicatorTime, setSnapIndicatorTime] = React.useState<number | null>(null);
  ...
  const handleTrimTextClip = useCallback(
    (clipId: string, edge: 'left' | 'right', newTime: number) => {
      ...
      useProjectStore.getState().updateTextClipKeyframes(clipId, adjustedKeyframes);
  ...
  const handleTrimShapeClip = useCallback(
    (clipId: string, edge: 'left' | 'right', newTime: number) => {
      ...
      const newDuration =
        edge === 'left'
          ? Math.max(0.1, graphicClip.startTime + graphicClip.duration - newTime)
        ...
        project: {
          ...
          timeline: {
            ...
            tracks: state.project.timeline.tracks.map((track) => ({
              ...track,
              clips: track.clips.map((c) => (c.id === clipId ? { ...c, ...updates, keyframes: adjustedKeyframes } : c))
            }))
          },
          modifiedAt: Date.now()
        }
  ...
  return (
    <div data-tour='timeline' className='h-full bg-background border-t border-border flex flex-col'>
      <div className='h-12 border-b border-border flex items-center justify-between px-4 bg-background-secondary relative z-[100]'>
        <div className='flex items-center gap-2'>
          <div className='flex bg-background-tertiary rounded-lg p-1 border border-border'>
            <IconButton icon={Undo2} onClick={undo} disabled={!canUndo()} title='Undo (Cmd+Z)' />
            <IconButton icon={Redo2} onClick={redo} disabled={!canRedo()} title='Redo (Cmd+Shift+Z)' />
          </div>
          ...
          <Popover open={showLayersPanel} onOpenChange={setShowLayersPanel}>
            ...
            >
              <div className='flex items-center justify-between px-3 py-2.5 border-b border-border bg-background-tertiary'>
                <span className='text-xs font-semibold text-text-primary'>Track Layers</span>
              </div>
              <div className='p-2 max-h-60 overflow-y-auto'>
                {tracks.length === 0 ? (
                  <p className='text-xs text-text-muted text-center py-6'>No tracks yet</p>
                ) : (
                  <div className='space-y-0.5'>
                    {tracks.map((track, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`w-7 h-7 rounded-md flex items-center justify-center ${info.bgLight}`}>
                          ...
                          <div className='flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity'>
                            <button
                              onClick={() => index > 0 && reorderTrack(track.id, index - 1)}
                              disabled={index === 0}
                            ...
                            <button
                              onClick={() => index < tracks.length - 1 && reorderTrack(track.id, index + 1)}
      ...
      <div ref={containerRef} className='flex-1 flex flex-col overflow-hidden relative' onClick={handleBackgroundClick}>
        ...
        <div className='flex-1 flex overflow-hidden'>
          <div className='w-32 bg-background-secondary border-r border-border shrink-0 z-20 shadow-lg overflow-hidden'>
            <div className='flex flex-col' style={{ transform: `translateY(-${scrollY}px)` }}>
              {visualOrderTracks.map((track, i) => {
                const keyframeCount = track.clips.reduce((sum, clip) => sum + (clip.keyframes?.length || 0), 0);
                return (
                  <div key={track.id} className={draggedTrackId === track.id ? 'opacity-50' : ''}>
                    <TrackHeader
                      track={track}
                      index={i}
            ...
            onDrop={async (e) => {
              ...
              if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                ...
                for (const file of Array.from(e.dataTransfer.files)) {
                  try {
                    const beforeIds = new Set(useProjectStore.getState().project.mediaLibrary.items.map((i) => i.id));
          ...
          >
            <div style={{ width: `${timelineDuration * pixelsPerSecond}px` }} className='min-w-full'>
              {visualOrderTracks.map((track) => (
                <TrackLane
                  ...
                  onTrimClip={
                    track.type === 'video' || track.type === 'image' || track.type === 'audio'
                      ? handleTrimClip
                      : undefined
                  }
              ...
              {isBoxSelecting && selectionBox && (
                <div
                  ...
                  style={{
                    left: Math.min(selectionBox.startX, selectionBox.currentX) - scrollX,
                    top: Math.min(selectionBox.startY, selectionBox.currentY) - scrollY,
        ...
        <Playhead position={playheadPosition} pixelsPerSecond={pixelsPerSecond} scrollX={scrollX} headerOffset={128} />
      </div>
    </div>
  );
};
