...
interface MotionPathOverlayProps {
  ...
  onControlPointMove: (pointIndex: number, handleType: 'cp1' | 'cp2', x: number, y: number) => void;
  disabled?: boolean;
}
  ...
  const currentPosition = useMemo(() => {
    ...
    for (let i = 0; i < sortedPoints.length - 1; i++) {
      if (normalizedTime >= sortedPoints[i].time && normalizedTime <= sortedPoints[i + 1].time) {
        ...
        const t = segmentDuration > 0 ? (normalizedTime - p0.time) / segmentDuration : 0;
    ...
    (e: React.MouseEvent<SVGPathElement>) => {
      ...
      if (screenPoints.length >= 2) {
        ...
        for (let i = 0; i < screenPoints.length - 1; i++) {
          ...
          const t = Math.max(0, Math.min(1, ((x - p0.screenX) * dx + (y - p0.screenY) * dy) / (len * len)));
