...
export const ParticleRenderer: React.FC<ParticleRendererProps> = ({
  ...
  const hexToRgbNormalized = useCallback((hex: string): { r: number; g: number; b: number } => {
  ...
  return (
    <div ref={containerRef} className='absolute inset-0 pointer-events-none w-full h-full' style={{ zIndex: 50 }} />
  );
};
