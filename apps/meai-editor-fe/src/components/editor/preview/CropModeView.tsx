...
export const CropModeView: React.FC<CropModeViewProps> = ({
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    if (dragHandle === 'center') {
      newCrop.x = Math.max(0, Math.min(1 - cropStart.width, cropStart.x + deltaX));
      newCrop.y = Math.max(0, Math.min(1 - cropStart.height, cropStart.y + deltaY));
      ...
      if (lockedAspect) {
        ...
        newCrop.width = Math.min(1 - cropStart.x, Math.max(0.1, cropStart.width + avgDelta));
        newCrop.height = Math.min(1 - cropStart.y, Math.max(0.1, cropStart.height + avgDelta));
      } else {
        newCrop.width = Math.min(1 - cropStart.x, Math.max(0.1, cropStart.width + deltaX));
        newCrop.height = Math.min(1 - cropStart.y, Math.max(0.1, cropStart.height + deltaY));
      }
    ...
    <div className='absolute inset-0 z-10 bg-background-secondary flex flex-col'>
      ...
      <div className='flex items-center justify-between p-3 bg-background border-b border-border'>
        <div className='flex items-center gap-3'>
          <span className='text-xs font-medium text-text-primary'>Crop Video</span>
      ...
      <div className='flex-1 flex items-center justify-center p-4 overflow-hidden relative'>
        ...
        >
          {mediaType === 'image' ? (
            <img ref={imageDisplayRef} className='w-full h-full' style={{ objectFit: 'contain' }} alt='Crop preview' />
          ) : (
            <video
              ref={videoDisplayRef}
          ...
          {!isLoading && videoSize.width > 0 && (
            <>
              ...
              >
                <defs>
                  <mask id='crop-mask'>
                    <rect x='0' y='0' width={displaySize.width} height={displaySize.height} fill='white' />
                    <rect
                      x={cropPixels.x}
                      y={cropPixels.y}
