...
import { Dialog, DialogContent, DialogHeader, DialogTitle, Button } from '@meai-editor/ui';
import {
  IMAGE_MODELS,
  type ImageModelId,
  type SeedreamInput,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (needsUpload) {
        ...
        uploadedUrl =
          uploaded.fileUrl || uploaded.downloadUrl || (uploaded as unknown as Record<string, string>)['url'] || '';
        if (!uploadedUrl) {
      ...
      <DialogContent className='max-w-md'>
        ...
        {step === 'form' && (
          <div className='mt-2 flex justify-start'>
            <button onClick={handleBack} className='text-xs text-text-muted hover:text-text-primary transition-colors'>
