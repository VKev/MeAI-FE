...
const applyStabilizationTransform = (
  ...
  return getStabilizedTransform(clip, transform, sourceTime, {
    canvasWidth,
    canvasHeight,
...
const hasBehindSubjectText = (textClips: TextClip[]): boolean => textClips.some((textClip) => textClip.behindSubject);
...
const captureSubjectFrame = async (
  ...
  try {
    return await createImageBitmap(ctx.canvas as HTMLCanvasElement | OffscreenCanvas, 0, 0, width, height);
  } catch {
    return null;
  }
};
...
const renderTextClipWithSubjectMask = async (
  ...
  if (textClip.behindSubject) {
    await drawMaskedSubjectFromFrame(ctx, subjectFrame, canvasWidth, canvasHeight);
  }
};
...
export const Preview: React.FC = () => {
  ...
  const decodeDebounceResolveRef = useRef<((value: ImageBitmap | null) => void) | null>(null);
  const decodeRequestSeqRef = useRef(0);
  const scrubVideoReleaseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const getAudioEffectSignature = useCallback(
  ...
  const getPreviewAudioBufferForEffects = useCallback(
    async (
      ...
      const previewEffects = getPreviewAudioEffects(effects.filter((effect) => effect.enabled));
      const { profileAwareNoiseEffects, realtimeEffects } = splitProfileAwareNoiseReductionEffects(previewEffects);
  ...
  const getResolvedClipVolumeAutomation = useCallback(
    (clip: PreviewClip) =>
  ...
  const [interactionTargetType, setInteractionTargetType] = useState<'clip' | 'text-clip' | 'shape-clip' | null>(null);
  ...
  const videoElementCacheRef = useRef<Map<string, { video: HTMLVideoElement; url: string; lastUsed: number }>>(
    new Map()
  );

  const releaseVideoElement = useCallback((entry: { video: HTMLVideoElement; url: string }): void => {
  ...
  const updateTextTransform = useProjectStore((state) => state.updateTextTransform);
  const updateShapeTransform = useProjectStore((state) => state.updateShapeTransform);
  ...
  const handleMotionPathPointMove = React.useCallback((index: number, x: number, y: number) => {
  ...
  const handleMotionPathPointAdd = React.useCallback((point: GSAPMotionPathPoint) => {
    setMotionPathConfig((prev) => {
      if (!prev) return prev;
  ...
  const [particleUpdateTrigger, setParticleUpdateTrigger] = React.useState(() => particleEngine.getChangeVersion());
  ...
  const renderOverlayClipsInTrackOrder = useCallback(
    async (
      ...
      const videoImageTrackIndices = tracks
        ...
        .filter(({ track }) => (track.type === 'video' || track.type === 'image') && !track.hidden)
        .map(({ originalIndex }) => originalIndex);

      const lowestVideoIndex = videoImageTrackIndices.length > 0 ? Math.min(...videoImageTrackIndices) : Infinity;
      const highestVideoIndex = videoImageTrackIndices.length > 0 ? Math.max(...videoImageTrackIndices) : -1;

      const overlayTracksWithIndex = tracks
        .map((t, idx) => ({ track: t, originalIndex: idx }))
        .filter(({ track }) => (track.type === 'text' || track.type === 'graphics') && !track.hidden);

      const tracksToRender = overlayTracksWithIndex.filter(({ originalIndex }) => {
        if (mode === 'below-video') {
      ...
      for (const { track } of tracksToRender) {
        if (track.type === 'graphics') {
          const trackShapeClips = shapeClips.filter((sc) => sc.trackId === track.id);
          for (const shapeClip of trackShapeClips) {
            renderShapeClipToCanvas(ctx, shapeClip, canvasWidth, canvasHeight, time);
          }
        } else if (track.type === 'text') {
          const trackTextClips = textClips.filter((tc) => tc.trackId === track.id);
          for (const textClip of trackTextClips) {
            await renderTextClipWithSubjectMask(ctx, textClip, canvasWidth, canvasHeight, time, subjectFrame);
          }
        }
      }
    },
    ...
    async (timelinePosition: number): Promise<void> => {
      ...
      for (const audioTrack of audioTracks) {
        ...
        for (const audioClip of audioTrack.clips) {
          ...
          if (timelinePosition >= audioClip.startTime && timelinePosition < clipEnd) {
            const mediaItem = getMediaItem(audioClip.mediaId);
            if (!mediaItem?.blob) {
              continue;
            }

            const audioCacheKey = getAudioBufferCacheKey(audioClip.mediaId, audioClip.audioTrackIndex);
            let audioBuffer = audioBufferCacheRef.current.get(audioCacheKey);
            if (!audioBuffer) {
              try {
                const audioContext = audioGraph.getAudioContext();
                const loaded = await loadAudioBuffer(audioContext, mediaItem.blob, audioClip.audioTrackIndex ?? 0);
                if (!loaded) {
                  continue;
                }
                audioBuffer = loaded;
                ...
                console.warn(`[Preview] Failed to decode audio for clip ${audioClip.id}:`, error);
                continue;
              }
            }
            ...
            const previewAudio = await getPreviewAudioBufferForEffects(audioBuffer, audioCacheKey, enabledEffects);
  ...
  const preDecodeAllAudioBuffers = useCallback(async (): Promise<void> => {
    const tracks = timelineTracksRef.current;
    const audioTracks = tracks.filter((t) => t.type === 'audio' && !t.hidden);
    const videoTracks = tracks.filter((t) => (t.type === 'video' || t.type === 'image') && !t.hidden);
    ...
    for (const track of allTracks) {
      for (const clip of track.clips) {
        ...
        let audioBuffer: AudioBuffer | null | undefined = audioBufferCacheRef.current.get(cacheKey);
        ...
        if (!audioBuffer) {
          ...
          try {
            audioBuffer = await loadAudioBuffer(audioContext, mediaItem.blob, clip.audioTrackIndex ?? 0);
            if (audioBuffer) {
        ...
        if (audioBuffer) {
          const audioEffects = getResolvedClipAudioEffects(clip).filter((effect: Effect) => effect.enabled);
          if (audioEffects.length > 0) {
            try {
              await getPreviewAudioBufferForEffects(audioBuffer, cacheKey, audioEffects);
            } catch (error) {
              console.warn(`[Preview] Failed to pre-process audio effects for clip ${clip.id}:`, error);
            }
          }
        }
      }
  ...
  const getAudioClipsForScheduler = useCallback(
    (time: number): AudioClipSchedule[] => {
      const tracks = timelineTracksRef.current;
      const tracksWithAudio = tracks.filter((t) => (t.type === 'audio' || t.type === 'video') && !t.hidden && !t.muted);
      ...
      for (const track of tracksWithAudio) {
        for (const clip of track.clips) {
          ...
          const audioEffects = getResolvedClipAudioEffects(clip).filter((e: Effect) => e.enabled);
          ...
          if (profileAwareNoiseEffects.length > 0) {
            ...
            const processedAudioBuffer = processedAudioBufferCacheRef.current.get(processedCacheKey);
    ...
    [getAudioEffectSignature, getResolvedClipAudioEffects, getResolvedClipVolumeAutomation]
  );
  ...
  const decodeClipFrame = useCallback(
    async (
      ...
      const mediaBlob = (vidstab.hasStabilized(clip.id) ? vidstab.getStabilizedBlob(clip.id) : mediaItem.blob)!;
      ...
      return new Promise<ImageBitmap | null>((resolve) => {
        ...
        decodeDebounceRef.current = setTimeout(async () => {
          ...
          try {
            ...
            const speedEngine = getSpeedEngine();
            const adjustedLocalTime = speedEngine.getSourceTimeAtPlaybackTime(clip.id, clipLocalTime);
            ...
            if (!cached) {
              ...
              await new Promise<void>((res, rej) => {
                const timeoutId = setTimeout(() => rej(new Error('Video load timeout')), 10000);
            ...
            const clampedTime = Math.max(0, Math.min(mediaTime, video.duration - 0.001));
            const seekTime = clampedTime <= 0 && video.duration > 0.002 ? 0.001 : clampedTime;
            if (
    ...
    [evictOldestVideoElement, getMediaItem, releaseVideoElement, scheduleScrubVideoRelease]
  );
  ...
  const renderFrameDirectly = useCallback(
    async (time: number): Promise<boolean> => {
      ...
      const videoTracks = timelineTracks.filter((t) => (t.type === 'video' || t.type === 'image') && !t.hidden);
      ...
      if (transitionInfo) {
        try {
          const outgoingFrame = await decodeClipFrame(transitionInfo.clipA, time, canvas.width, canvas.height);
          const incomingFrame = await decodeClipFrame(transitionInfo.clipB, time, canvas.width, canvas.height);

          if (outgoingFrame && incomingFrame) {
            const processedOutgoing = await applyEffectsToFrame(transitionInfo.clipA.id, outgoingFrame);
            const processedIncoming = await applyEffectsToFrame(transitionInfo.clipB.id, incomingFrame);

            const validOutgoing =
              processedOutgoing.width > 0 && processedOutgoing.height > 0 ? processedOutgoing : outgoingFrame;
            const validIncoming =
              processedIncoming.width > 0 && processedIncoming.height > 0 ? processedIncoming : incomingFrame;

            const blendedFrame = await renderTransitionFrame(transitionInfo, validOutgoing, validIncoming);

            if (blendedFrame && blendedFrame.width > 0 && blendedFrame.height > 0) {
            ...
            const processed = await applyEffectsToFrame(transitionInfo.clipA.id, outgoingFrame);
            const validFrame = processed.width > 0 && processed.height > 0 ? processed : outgoingFrame;
            if (shouldClearCanvas) {
              ctx.fillStyle = '#000000';
              ctx.fillRect(0, 0, canvas.width, canvas.height);
              shouldClearCanvas = false;
            ...
            const processed = await applyEffectsToFrame(transitionInfo.clipB.id, incomingFrame);
            const validFrame = processed.width > 0 && processed.height > 0 ? processed : incomingFrame;
            if (shouldClearCanvas) {
              ctx.fillStyle = '#000000';
              ctx.fillRect(0, 0, canvas.width, canvas.height);
              shouldClearCanvas = false;
      ...
      if (!hasRenderedFrame) {
        const hasVideoContent = videoTracks.some((track) =>
          track.clips.some((clip) => time >= clip.startTime && time < clip.startTime + clip.duration)
        );

        if (shouldClearCanvas && (hasVideoContent || activeShapeClips.length > 0 || activeTextClips.length > 0)) {
          ctx.fillStyle = hasVideoContent ? '#000000' : isDark ? '#0f0f11' : '#ffffff';
          ctx.fillRect(0, 0, canvas.width, canvas.height);
          shouldClearCanvas = false;
        }
        ...
        for (const { track } of allRenderableTracks) {
          if (track.type === 'video' || track.type === 'image') {
            for (const clip of track.clips) {
              ...
              if (time >= clipStart && time < clipEnd) {
                ...
                if (frame) {
                  ...
                  const speedEngine = getSpeedEngine();
                  const adjustedLocalTime = speedEngine.getSourceTimeAtPlaybackTime(clip.id, clipLocalTime);
                  const sourceTime = Math.max(clip.inPoint, Math.min(clip.outPoint, clip.inPoint + adjustedLocalTime));
                  ...
                  if (clip.emphasisAnimation && clip.emphasisAnimation.type !== 'none') {
                    const emphasisState = applyEmphasisAnimation(clip.emphasisAnimation, clipLocalTime);
                    animatedTransform = {
                      ...animatedTransform,
                      opacity: animatedTransform.opacity * emphasisState.opacity,
                      scale: {
                        x: animatedTransform.scale.x * emphasisState.scale * emphasisState.scaleX,
                        y: animatedTransform.scale.y * emphasisState.scale * emphasisState.scaleY
                      },
                      position: {
                        x: animatedTransform.position.x + emphasisState.offsetX * canvas.width,
                        y: animatedTransform.position.y + emphasisState.offsetY * canvas.height
                      },
                      rotation: animatedTransform.rotation + emphasisState.rotation
                    };
                  }
                  ...
                  try {
                    ...
                    if (processedFrame.width > 0 && processedFrame.height > 0) {
                      drawFrameWithTransform(ctx, processedFrame, stabilizedTransform, canvas.width, canvas.height);
                      hasRenderedFrame = true;
                    } else {
                      drawFrameWithTransform(ctx, frame, stabilizedTransform, canvas.width, canvas.height);
                      hasRenderedFrame = true;
                    }
                  } catch {
                    drawFrameWithTransform(ctx, frame, stabilizedTransform, canvas.width, canvas.height);
                  ...
                  if (shouldCompositeSubject) {
                    subjectFrame?.close();
                    subjectFrame = await captureSubjectFrame(ctx, canvas.width, canvas.height);
                  }
                  frame.close();
                }
              }
            ...
            const trackShapeClips = activeShapeClips.filter((sc) => sc.trackId === track.id);
            for (const shapeClip of trackShapeClips) {
              renderShapeClipToCanvas(ctx, shapeClip, canvas.width, canvas.height, time);
              hasRenderedFrame = true;
            }
          } else if (track.type === 'text') {
            const trackTextClips = activeTextClips.filter((tc) => tc.trackId === track.id);
            for (const textClip of trackTextClips) {
              await renderTextClipWithSubjectMask(ctx, textClip, canvas.width, canvas.height, time, subjectFrame);
      ...
      if (activeSubtitles.length > 0 && ctx) {
        for (const subtitle of activeSubtitles) {
          renderSubtitleToCanvas(ctx, subtitle, canvas.width, canvas.height, time);
        }
      }
  ...
  const renderFallbackFrame = useCallback(
    (time: number) => {
      ...
      ctx.fillStyle = hasVideoContent ? (isDark ? '#18181b' : '#f4f4f5') : emptyBg;
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ...
      const allRenderableTracks = timelineTracks
        ...
        .filter(
          ({ track }) =>
            (track.type === 'video' || track.type === 'image' || track.type === 'text' || track.type === 'graphics') &&
            !track.hidden
        )
        .sort((a, b) => b.originalIndex - a.originalIndex);
      ...
      for (const { track } of allRenderableTracks) {
        if (track.type === 'video' || track.type === 'image') {
          for (const clip of track.clips) {
            ...
            if (time >= clipStart && time < clipEnd) {
              ...
              if (mediaItem) {
                ...
                ctx.fillText(mediaItem.name, canvas.width / 2, canvas.height / 2);
                ...
                ctx.textAlign = 'center';
                ctx.fillText('Drop media here', canvas.width / 2, canvas.height / 2);
                ctx.font = '14px Inter, sans-serif';
                ctx.fillStyle = emptyText;
                ctx.fillText('Replace this placeholder with your content', canvas.width / 2, canvas.height / 2 + 28);
              }
            }
          }
        } else if (track.type === 'graphics') {
          ...
          const trackTextClips = activeTextClips.filter((tc) => tc.trackId === track.id);
          for (const textClip of trackTextClips) {
            renderTextClipToCanvas(ctx, textClip, canvas.width, canvas.height, time);
            hasRenderedContent = true;
          }
        }
      }
      ...
      const audioTracks = timelineTracks.filter((t) => t.type === 'audio' && !t.hidden);
      const hasActiveAudioClip = audioTracks.some((track) =>
        track.clips.some((clip) => time >= clip.startTime && time < clip.startTime + clip.duration)
      );

      if (!hasRenderedContent && activeTextClips.length === 0 && activeShapeClips.length === 0 && !hasActiveAudioClip) {
        ctx.fillStyle = emptyText;
        ctx.font = '24px Inter, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Import media to get started', canvas.width / 2, canvas.height / 2);
      }
    },
    [timelineTracks, getMediaItem, settings.width, settings.height, allTextClips, allShapeClips, allSubtitles, isDark]
  );
      ...
      const hasActiveAudioEffects = tracks.some(
        (track) =>
          ...
          track.clips.some((clip) => {
            ...
            return getPreviewAudioEffects(getResolvedClipAudioEffects(clip)).some((effect) => effect.enabled);
          })
      );
  ...
  const startNativeVideoPlayback = useCallback(
    async (
      ...
      const videoCache = new Map<string, { video: HTMLVideoElement; url: string }>();
      ...
      const loadVideoForClip = (
        ...
        const playBlob = (isStabilized ? vidstabEng.getStabilizedBlob(clip.id) : mediaItem.blob)!;
      ...
      const activeStartClip = clips.find(
        ({ clip }) => startPosition >= clip.startTime && startPosition < clip.startTime + clip.duration
      );
      if (activeStartClip) {
      ...
      const tracksWithAudio = timelineTracks.filter((t) => (t.type === 'audio' || t.type === 'video') && !t.hidden);
      ...
      const drawFrame = async () => {
        ...
        if (!activeClip) {
          ...
          const sortedImageClipsNoVideo = [...imageClips].sort((a, b) => b.trackIndex - a.trackIndex);
          for (const { clip: imgClip } of sortedImageClipsNoVideo) {
            if (currentPlayhead >= imgClip.startTime && currentPlayhead < imgClip.startTime + imgClip.duration) {
              ...
              if (bitmap) {
                ...
                drawFrameWithTransform(ctx, bitmap, imgTransform, canvas.width, canvas.height);
              }
            }
          }

          const activeShapeClipsNoVideo = getActiveShapeClips(allShapeClipsRef.current, currentPlayhead);
          const activeTextClipsNoVideo = getActiveTextClips(allTextClipsRef.current, currentPlayhead);
          ...
          const activeSubtitlesNoVideo = getActiveSubtitles(allSubtitles, currentPlayhead);
          for (const subtitle of activeSubtitlesNoVideo) {
            renderSubtitleToCanvas(ctx, subtitle, canvas.width, canvas.height, currentPlayhead);
          }
        ...
        const speedEngine = getSpeedEngine();
        const adjustedLocalTime = speedEngine.getSourceTimeAtPlaybackTime(latestClip.id, clipLocalTime);
        ...
        if (latestClip.emphasisAnimation && latestClip.emphasisAnimation.type !== 'none') {
          const emphasisState = applyEmphasisAnimation(latestClip.emphasisAnimation, clipLocalTime);
          transform = {
            ...transform,
        ...
        const activeShapeClips = getActiveShapeClips(allShapeClipsData, currentPlayhead);
        const activeTextClips = getActiveTextClips(allTextClipsRef.current, currentPlayhead);
        ...
        const activeSubtitles = getActiveSubtitles(allSubtitles, currentPlayhead);
        for (const subtitle of activeSubtitles) {
          renderSubtitleToCanvas(ctx, subtitle, canvas.width, canvas.height, currentPlayhead);
        }
  ...
  useEffect(() => {
    ...
    const findAllClipsAtTime = (time: number) => {
      ...
      tracks.forEach((track, originalIndex) => {
        if ((track.type === 'video' || track.type === 'image') && !track.hidden) {
          for (const clip of track.clips) {
            if (time >= clip.startTime && time < clip.startTime + clip.duration) {
      ...
      try {
        ...
        if (!mediaItem?.blob) {
          ...
          if (nextResult && clipEndTime < actualEndTime && isActive) {
            startPlaybackForClip(nextResult.clip, nextResult.track, clipEndTime);
          } else {
            pause();
          }
          return;
        ...
        try {
          ...
          if (canvas.width === 0 || canvas.height === 0) {
            console.warn('[Preview] Canvas has zero dimensions, setting from project settings');
            canvas.width = settings.width;
            canvas.height = settings.height;
          }
          ...
          if (currentSpeed === 1 && !isReverse) {
            ...
            for (const audioTrack of audioTracks) {
              for (const audioClip of audioTrack.clips) {
                if (audioClip.mediaId === clip.mediaId && Math.abs(audioClip.startTime - clip.startTime) < 0.01) {
          ...
          const adjustedLocalTime = speedEngine.getSourceTimeAtPlaybackTime(speedSourceClip, clipLocalTime);
          ...
          const processNextFrame = async () => {
            ...
            try {
              ...
              const frameHeight = 'height' in frameCanvas ? frameCanvas.height : 0;
              ...
              const useGPU = rendererRef.current && rendererRef.current.type === 'webgpu';
              const preparedFrame = await preparePreviewFrame(clip.id, frameCanvas, Boolean(useGPU));
              const stabilizedTransform = applyStabilizationTransform(
                clip,
                transform,
                currentMediaTime,
              ...
              if (useGPU && preparedFrame.frame instanceof ImageBitmap) {
                ...
                if (gpuResult) {
                  ...
                  drawFrameWithTransform(ctx, preparedFrame.frame, stabilizedTransform, canvas.width, canvas.height);
                }
              } else {
                drawFrameWithTransform(ctx, preparedFrame.frame, stabilizedTransform, canvas.width, canvas.height);
              }
              ...
              const actualFrameDuration = duration > 0 ? duration * 1000 : frameDuration;
        ...
        console.error('[Preview] startPlaybackForClip outer error:', outerError);
        pause();
      }
    };

    const initClipResources = async (clip: (typeof timelineTracksRef.current)[0]['clips'][0], trackIndex: number) => {
      const mediaItem = getMediaItem(clip.mediaId);
      if (!mediaItem?.blob) {
        return null;
      }
      ...
      try {
        ...
        console.error(`[Preview] Failed to init resources for clip ${clip.id}:`, error);
        return null;
      }
    };
        ...
        for (const clip of track.clips) {
          ...
          if (mediaItem?.type === 'image' && mediaItem.blob) {
            try {
              ...
              console.warn(`[Preview] Failed to pre-cache image clip ${clip.id}:`, error);
            }
          }
        }
      }
    ...
    const startMultiTrackPlayback = async () => {
      ...
      const activeTextClips = getActiveTextClips(allTextClipsRef.current, playbackStartPosition);
      const activeShapeClips = getActiveShapeClips(allShapeClipsRef.current, playbackStartPosition);

      const audioTracks = timelineTracksRef.current.filter((t) => t.type === 'audio' && !t.hidden);
      const hasActiveAudioClip = audioTracks.some((track) =>
        track.clips.some(
          (clip) => playbackStartPosition >= clip.startTime && playbackStartPosition < clip.startTime + clip.duration
        )
      );

      const hasAnyVisualContent = initialClips.length > 0 || activeTextClips.length > 0 || activeShapeClips.length > 0;
      ...
      const hasTextOrShapeContent = activeTextClips.length > 0 || activeShapeClips.length > 0;
      if (
      ...
      const processMultiTrackFrame = async () => {
        ...
        try {
          ...
          const currentTextClips = getActiveTextClips(allTextClipsRef.current, currentPlayhead);
          const currentShapeClips = getActiveShapeClips(allShapeClipsRef.current, currentPlayhead);

          const audioTracksForFrame = timelineTracksRef.current.filter((t) => t.type === 'audio' && !t.hidden);
          const hasCurrentAudioClip = audioTracksForFrame.some((track) =>
            track.clips.some(
              (clip) => currentPlayhead >= clip.startTime && currentPlayhead < clip.startTime + clip.duration
            )
          );

          const hasVisualContent =
            activeClips.length > 0 || currentTextClips.length > 0 || currentShapeClips.length > 0;
          const hasAnyContentAtPlayhead = hasVisualContent || hasCurrentAudioClip;
          ...
          if (!hasAnyContentAtPlayhead) {
            ...
            const nextTimes = [nextClipTime, nextTextTime, nextShapeTime, nextAudioTime].filter(
              (t): t is number => t !== null && t < actualEndTime
            );
            const nextTime = nextTimes.length > 0 ? Math.min(...nextTimes) : null;
          ...
          const sortedClips = [...activeClips].sort((a, b) => b.trackIndex - a.trackIndex);
          const activeShapeClips = getActiveShapeClips(allShapeClipsRef.current, currentPlayhead);
          const activeTextClips = getActiveTextClips(allTextClipsRef.current, currentPlayhead);
          const activeTextNeedsSubject = hasBehindSubjectText(activeTextClips);
          const useGPUFrames = rendererRef.current?.type === 'webgpu' && !activeTextNeedsSubject;
              ...
              (async () => {
                ...
                try {
                  ...
                  if (frameResult?.canvas) {
                    const preparedFrame = await preparePreviewFrame(clip.id, frameResult.canvas, useGPUFrames);
                  ...
                  console.warn(`[Preview] Failed to get frame for clip ${clip.id}:`, error);
                }
                return null;
              })()
            );
          ...
          const validVideoFrames = videoFrameResults.filter((f): f is NonNullable<typeof f> => f !== null);

          const validFrames = [...imageClipFrames, ...validVideoFrames];

          if (validFrames.length > 0 || currentTextClips.length > 0 || currentShapeClips.length > 0) {
            ctx.fillStyle = '#000000';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            ...
            const useGPU = rendererRef.current && rendererRef.current.type === 'webgpu' && !activeTextNeedsSubject;
            ...
            if (useGPU) {
              ...
              for (const { track, originalIndex } of allRenderableTracks) {
                if (track.type === 'video') {
                  const trackFrames = validFrames.filter((f) => clipToTrackIndex.get(f.clip.id) === originalIndex);
                  ...
                  const trackFrames = validFrames.filter((f) => clipToTrackIndex.get(f.clip.id) === originalIndex);
                  for (const { transform, frame } of trackFrames) {
                    drawFrameWithTransform(ctx, frame, transform, canvas.width, canvas.height);
                  }
                } else if (track.type === 'graphics') {
                  const trackShapeClips = activeShapeClips.filter((sc) => sc.trackId === track.id);
                  for (const shapeClip of trackShapeClips) {
                    renderShapeClipToCanvas(ctx, shapeClip, canvas.width, canvas.height, currentPlayhead);
                  }
                } else if (track.type === 'text') {
                  const trackTextClips = activeTextClips.filter((tc) => tc.trackId === track.id);
                  for (const textClip of trackTextClips) {
              ...
              if (gpuLayers.length > 0) {
                ...
                if (gpuResult) {
                  ...
                  for (const layer of gpuLayers) {
                    drawFrameWithTransform(ctx, layer.bitmap, layer.transform, canvas.width, canvas.height);
                  }
                }
              }
            ...
            const activeSubtitles = getActiveSubtitles(allSubtitlesRef.current, currentPlayhead);
            for (const subtitle of activeSubtitles) {
              renderSubtitleToCanvas(ctx, subtitle, canvas.width, canvas.height, currentPlayhead);
            }
            ...
            ctx.drawImage(lastGoodFrameRef.current, 0, 0, canvas.width, canvas.height);
    ...
    const startPlayback = async () => {
      ...
      if (nativeCheck.canUse && nativeCheck.clips.length > 0) {
        try {
          ...
          console.warn('[Preview] Native video playback failed, falling back to MediaBunny:', error);
        }
      }
  ...
  useEffect(() => {
    ...
    const isLargeJump =
      Math.abs(playheadPosition - previousRenderTime) > 1 || playheadPosition < previousRenderTime - 0.25;
    if (isLargeJump) {
      releaseScrubVideoElements();
    }
    lastPreviewRenderTimeRef.current = playheadPosition;
  ...
  const selectedTextClipId = useMemo(() => {
    const textClipSelection = selectedItems.find((item) => item.type === 'text-clip');
  ...
  }, [selectedClip, clipAtPlayhead, settings.width, settings.height, canvasSize, liveTransform]);

  const textClipBounds = useMemo(() => {
    if (!selectedTextClip || !canvasRef.current || !overlayRef.current) return null;
    ...
    const estimatedWidth = style.fontSize * Math.max(...lines.map((l) => l.length)) * 0.6;
  ...
  const selectedShapeClipId = useMemo(() => {
    const shapeClipSelection = selectedItems.find((item) => item.type === 'shape-clip');
    return shapeClipSelection?.id || null;
  }, [selectedItems]);

  const selectedShapeClip = useMemo<ShapeClip | SVGClip | StickerClip | null>(() => {
    if (!selectedShapeClipId) return null;
    return allShapeClips.find((clip) => clip.id === selectedShapeClipId) || null;
  ...
  const shapeClipBounds = useMemo(() => {
    if (!selectedShapeClip || !canvasRef.current || !overlayRef.current) return null;
  ...
  const subtitleBounds = useMemo(() => {
    if (!selectedSubtitleObj || !canvasRef.current || !overlayRef.current) return null;
    if (playheadPosition < selectedSubtitleObj.startTime || playheadPosition >= selectedSubtitleObj.endTime)
      return null;
  ...
  }, [selectedSubtitleObj, settings.width, settings.height, canvasSize, playheadPosition]);
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (interactionTargetType === 'text-clip' && textClipBounds && activeTextClip) {
        ...
        if (interactionMode === 'move') {
          const newX = interactionStartRef.current.transform.x + deltaX / displayScale / settings.width;
          const newY = interactionStartRef.current.transform.y + deltaY / displayScale / settings.height;
        ...
        if (!rafIdRef.current) {
          rafIdRef.current = requestAnimationFrame(() => {
            const now = performance.now();
            if (now - lastStoreUpdateRef.current >= STORE_UPDATE_THROTTLE_MS && interactionTargetIdRef.current) {
      ...
      if (interactionTargetType === 'shape-clip' && shapeClipBounds && activeShapeClip) {
        const deltaX = e.clientX - interactionStartRef.current.x;
        const deltaY = e.clientY - interactionStartRef.current.y;
        ...
        if (!rafIdRef.current) {
          rafIdRef.current = requestAnimationFrame(() => {
            ...
            if (now - lastStoreUpdateRef.current >= STORE_UPDATE_THROTTLE_MS && interactionTargetIdRef.current) {
              lastStoreUpdateRef.current = now;
              updateShapeTransform(interactionTargetIdRef.current, newTransform);
            }
            rafIdRef.current = null;
          });
        }
      ...
      if (interactionMode === 'move') {
        const newX = interactionStartRef.current.transform.x + deltaX / displayScale;
        const newY = interactionStartRef.current.transform.y + deltaY / displayScale;
      ...
      if (!rafIdRef.current) {
        rafIdRef.current = requestAnimationFrame(() => {
          const now = performance.now();
          if (pendingTransformRef.current && now - lastStoreUpdateRef.current >= STORE_UPDATE_THROTTLE_MS) {
            lastStoreUpdateRef.current = now;
            updateClipTransform(pendingTransformRef.current.clipId, pendingTransformRef.current.transform);
          }
          rafIdRef.current = null;
        });
      }
  ...
  }, [interactionMode, renderFrameDirectly, playheadPosition, updateClipTransform]);
  ...
  const progressPercentage = actualEndTime > 0 ? (playheadPosition / actualEndTime) * 100 : 0;
  ...
  const showShapeClipHandles = !isPlaying && selectedShapeClip && shapeClipBounds;

  const showSubtitleOverlay = !isPlaying && selectedSubtitleObj && subtitleBounds;
        ...
        >
              ...
              <div className='bg-background-secondary/95 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl border border-border'>
                <div className='flex items-center gap-3 mb-4'>
                  ...
                  <div>
                    ...
                    <p className='text-xs text-text-muted'>{exportState.phase || 'Preparing...'}</p>
                  </div>
                </div>

                <div className='mb-4'>
                  <div className='flex items-center justify-between mb-1'>
                    <span className='text-[10px] text-text-secondary'>Export Progress</span>
                    <span className='text-[10px] text-text-muted font-mono'>{Math.round(exportState.progress)}%</span>
                  </div>
          ...
          {showShapeClipHandles && shapeClipBounds && (
            ...
            >
              ...
              {/* Move handle (center) */}
              <div
                className='absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-purple-500/80 rounded-full flex items-center justify-center cursor-move pointer-events-auto hover:bg-purple-500 transition-colors'
              ...
              {/* Corner resize handles */}
              <div
                className='absolute -left-2 -top-2 w-4 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-nw-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'nw')}
              />
              <div
                className='absolute -right-2 -top-2 w-4 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-ne-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'ne')}
              />
              <div
                className='absolute -left-2 -bottom-2 w-4 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-sw-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'sw')}
              />
              <div
                className='absolute -right-2 -bottom-2 w-4 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-se-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'se')}
              />

              {/* Edge resize handles */}
              <div
                className='absolute left-1/2 -translate-x-1/2 -top-2 w-6 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-n-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'n')}
              />
              <div
                className='absolute left-1/2 -translate-x-1/2 -bottom-2 w-6 h-4 bg-white border-2 border-purple-500 rounded-sm cursor-s-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 's')}
              />
              <div
                className='absolute top-1/2 -translate-y-1/2 -left-2 w-4 h-6 bg-white border-2 border-purple-500 rounded-sm cursor-w-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
                onMouseDown={(e) => handleShapeHandleMouseDown(e, 'w')}
              />
              <div
                className='absolute top-1/2 -translate-y-1/2 -right-2 w-4 h-6 bg-white border-2 border-purple-500 rounded-sm cursor-e-resize pointer-events-auto hover:bg-purple-500 hover:border-white transition-colors'
      ...
      >
        ...
        <div className='h-12 px-6 flex items-center justify-between'>
            ...
            {rendererType !== 'none' && (
              <span
                className={`text-[10px] px-1.5 py-0.5 rounded ${
                  rendererType === 'webgpu' ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20 text-gray-400'
                }`}
          ...
          <div className='flex items-center gap-6'>
            <IconButton icon={SkipBack} onClick={handleSkipBack} title='Skip back 5s' />
            <button
              onClick={() => {
                togglePlayback();
              }}
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center text-white transition-all ${
                playbackLockedReason
                  ...
                  : 'bg-primary hover:bg-primary-hover active:bg-primary-active shadow-[0_0_15px_rgba(124,58,237,0.3)] hover:shadow-[0_0_25px_rgba(124,58,237,0.5)] transform hover:scale-105'
              }`}
            >
              {isPlaying ? (
            ...
            <IconButton icon={SkipForward} onClick={handleSkipForward} title='Skip forward 5s' />
          </div>
          ...
          <div className='flex gap-2 items-center'>
            <button
              ...
              className={`p-2 rounded-lg hover:bg-background-elevated transition-colors ${
                isMuted ? 'text-red-500' : 'text-text-secondary hover:text-text-primary'
              }`}
            >
                    ...
                    {ZOOM_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={`w-full px-3 py-1.5 text-xs font-mono text-left hover:bg-background-secondary transition-colors ${
                          zoomLevel === opt.value ? 'text-primary' : 'text-text-secondary'
                        }`}
                      >
                        {opt.label}
                      </button>
