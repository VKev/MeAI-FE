...
import { Dialog, DialogContent, Input } from '@meai-editor/ui';
...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose }) => {
  ...
  const filteredEffects = useMemo(() => {
    ...
    if (selectedClipType) {
      effects = effects.filter((e) =>
        e.clipTypes.includes(selectedClipType as 'video' | 'audio' | 'text' | 'shape' | 'image')
      );
    }

    if (selectedCategory !== 'all') {
      effects = effects.filter((e) => e.category.toLowerCase().includes(selectedCategory.toLowerCase()));
    }

    if (query.trim()) {
      const searchTerms = query.toLowerCase().split(' ');
      effects = effects.filter((e) => {
        const searchText = [e.name, e.description, ...e.keywords, e.category].join(' ').toLowerCase();
  ...
  const handleSelect = useCallback(
    (effect: SearchItem) => {
      ...
      setTimeout(() => {
        const sectionElement = document.querySelector(`[data-section-id="${effect.sectionId}"]`);
        if (sectionElement) {
      ...
      <DialogContent className='max-w-2xl p-0 gap-0 top-[15vh] translate-y-0 bg-background-secondary border-border rounded-2xl overflow-hidden'>
        <div className='flex items-center gap-3 px-4 py-3 border-b border-border'>
          ...
          <Input
            ...
            placeholder={
              selectedClipType ? `Search effects for ${selectedClipType} clip...` : 'Search all effects and tools...'
            }
        ...
        <div ref={listRef} className='max-h-[50vh] overflow-y-auto'>
          {filteredEffects.length === 0 ? (
            <div className='py-12 text-center'>
              <Search size={32} className='mx-auto mb-3 text-text-muted opacity-50' />
                  ...
                  >
                    <div
                      className={`p-2 rounded-lg ${
                        index === selectedIndex ? 'bg-primary text-white' : 'bg-background-tertiary text-text-secondary'
                      }`}
                    >
                      <Icon size={16} />
                    </div>
                    <div className='flex-1 min-w-0'>
                      <div className='flex items-center gap-2'>
                        <span
                          className={`text-sm font-medium ${
                            index === selectedIndex ? 'text-primary' : 'text-text-primary'
                          }`}
                        >
                          {effect.name}
                        </span>
                      ...
                      <p className='text-xs text-text-muted mt-0.5 truncate'>{effect.description}</p>
                    </div>
                    <div className='text-[10px] text-text-muted'>↵ to select</div>
                  </button>
                );
              })}
            </div>
