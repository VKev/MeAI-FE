...
export const KeyboardShortcutsOverlay: React.FC<KeyboardShortcutsOverlayProps> = ({ isOpen, onClose }) => {
  ...
  const [activePreset, setActivePreset] = useState(keyboardShortcuts.getActivePreset());
  ...
  const handleShortcutCapture = useCallback((e: React.KeyboardEvent, shortcutId: string) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (conflict) {
      alert(`This shortcut conflicts with "${conflict.name}". Choose a different key.`);
      return;
    }
      ...
      <DialogContent className='max-w-3xl max-h-[80vh] p-0 gap-0 bg-background-secondary border-border overflow-hidden flex flex-col'>
        <DialogHeader className='p-4 border-b border-border bg-background-tertiary space-y-0'>
          <div className='flex items-center gap-3'>
            ...
            <DialogTitle className='text-lg font-bold text-text-primary'>Keyboard Shortcuts</DialogTitle>
          </div>
        </DialogHeader>
          ...
          {Object.entries(groupedShortcuts).map(([category, categoryShortcuts]) => (
            <div key={category}>
              <h3 className='text-xs font-bold text-text-muted uppercase tracking-wider mb-3'>
                {keyboardShortcuts.getCategoryName(category as ShortcutCategory)}
              </h3>
                ...
                {categoryShortcuts.map((shortcut) => (
                  ...
                  >
                    <div className='flex-1'>
                      <div className='text-sm text-text-primary'>{shortcut.name}</div>
                      <div className='text-[10px] text-text-muted'>{shortcut.description}</div>
                    </div>
                    <div className='flex items-center gap-2'>
                      {editingId === shortcut.id ? (
                        <input
                          autoFocus
                          onKeyDown={(e) => handleShortcutCapture(e, shortcut.id)}
        ...
        <div className='p-3 border-t border-border bg-background-tertiary text-center'>
          <p className='text-[10px] text-text-muted'>
            ...
            <kbd className='px-1.5 py-0.5 bg-background-secondary border border-border rounded text-[10px]'>?</kbd> to
            toggle this overlay
          </p>
        </div>
      </DialogContent>
