...
export const Toolbar: React.FC = () => {
  ...
  return (
    <div className='h-16 border-b border-border flex items-center px-6 justify-between bg-background shrink-0 z-30 relative'>
          ...
          {exportState.isExporting ? (
            <div className='h-10 px-4 bg-background-secondary border border-border rounded-lg flex items-center gap-3 min-w-[200px]'>
              ...
              <div className='flex-1'>
                <div className='text-[10px] text-text-secondary'>{exportState.phase}</div>
            ...
            <DropdownMenu open={isExportOpen} onOpenChange={setIsExportOpen}>
              <DropdownMenuTrigger asChild>
                <button
                  className={`h-10 px-4 bg-primary hover:bg-primary-hover active:bg-primary-active text-white font-bold rounded-lg flex items-center gap-2 transition-all shadow-[0_0_20px_rgba(124,58,237,0.3)] hover:shadow-[0_0_30px_rgba(124,58,237,0.5)] transform hover:-translate-y-0.5 ${
                    ...
                    option.separator ? (
                      ...
                      <DropdownMenuItem
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer ${
                          option.recommended ? 'bg-primary/10 hover:bg-primary/20 border border-primary/30' : ''
                        }`}
                        ...
                        <div className='flex-1'>
                          <div
                            className={`text-sm font-medium transition-colors ${
                              option.recommended ? 'text-primary' : 'text-text-primary'
                            }`}
                          >
                            {option.label}
                            {option.recommended && (
      ...
      <Dialog open={exportModeDialogOpen} onOpenChange={setExportModeDialogOpen}>
        <DialogContent className='max-w-sm'>
          <DialogHeader>
            <DialogTitle>Export Options</DialogTitle>
            <DialogDescription>Choose how you want to export your video</DialogDescription>
          </DialogHeader>
          <div className='flex gap-3'>
            <Button variant='outline' className='flex-1' onClick={() => handleExportModeSelect('download')}>
              Download to Computer
            </Button>
            <Button variant='default' className='flex-1' onClick={() => handleExportModeSelect('upload')}>
              Upload to Server
            </Button>
          </div>
        </DialogContent>
