    ...
    <div className='flex items-center gap-3 p-2 bg-black/20 rounded'>
      ...
      <div className='flex-1 min-w-0'>
        <div className='flex items-center justify-between'>
          <span className='text-xs font-medium text-text-primary truncate'>{PROCESSING_TYPE_LABELS[task.type]}</span>
        ...
        {task.status === 'processing' && (
          <div className='mt-1'>
            ...
            <p className='text-[9px] text-text-muted mt-0.5 truncate'>{task.message}</p>
          </div>
        )}
...
export const ProcessingOverlay: React.FC = () => {
  ...
  const activeTasks = taskList.filter((t) => t.status === 'queued' || t.status === 'processing');
    ...
    <div className='absolute inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50'>
      <div className='bg-background-secondary/95 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl border border-border'>
        <div className='flex items-center gap-3 mb-4'>
          ...
          <div>
            <h3 className='text-sm font-semibold text-text-primary'>Processing Effects</h3>
        ...
        <p className='text-[10px] text-text-muted text-center mt-4'>Please wait while effects are being applied...</p>
      </div>
    </div>
  );
};
