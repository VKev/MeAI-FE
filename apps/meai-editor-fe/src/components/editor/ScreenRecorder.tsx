...
import { Monitor, Mic, MicOff, Volume2, VolumeX, Camera, Circle, Settings, AlertCircle } from 'lucide-react';
...
export const ScreenRecorder: React.FC<ScreenRecorderProps> = ({ isOpen, onClose, onRecordingComplete }) => {
  const {
    status,
    options,
    webcamStream,
      ...
      <DialogContent className='max-w-2xl p-0 gap-0 bg-background-secondary border-border overflow-hidden'>
        <DialogHeader className='p-4 border-b border-border bg-background-tertiary space-y-0'>
          <div className='flex items-center gap-3'>
            ...
            <DialogTitle className='text-lg font-bold text-text-primary'>Screen Recording</DialogTitle>
          </div>
        </DialogHeader>
        ...
        <div className='p-6 space-y-6'>
          {!isSupported && (
            <div className='flex items-start gap-3 p-4 bg-error/10 border border-error/30 rounded-lg'>
              ...
              <div>
                <p className='text-sm font-medium text-error'>Screen recording not supported</p>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <p className='text-[10px] text-text-muted mt-1'>
                  {RESOLUTION_OPTIONS.find((o) => o.value === options.video.resolution)?.desc}
                </p>
              </div>

              <div>
                <label className='block text-xs text-text-muted mb-2'>Frame Rate</label>
                <Select
          ...
          <div className='space-y-4'>
            ...
            <div className='flex gap-4'>
              <button
                onClick={() => setAudioOption('systemAudio', !options.audio.systemAudio)}
              ...
              >
                {options.audio.systemAudio ? <Volume2 size={18} /> : <VolumeX size={18} />}
                <span className='text-sm'>System Audio</span>
              </button>

              <button
                onClick={() => setAudioOption('microphone', !options.audio.microphone)}
                disabled={!isSupported}
                className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border transition-all ${
              ...
              >
                {options.audio.microphone ? <Mic size={18} /> : <MicOff size={18} />}
          ...
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <button
                onClick={() => setWebcamOption('enabled', !options.webcam.enabled)}
                disabled={!isSupported || !features.webcam}
                className={`relative w-12 h-6 rounded-full transition-colors ${
                  options.webcam.enabled ? 'bg-primary' : 'bg-background-tertiary'
            ...
            {options.webcam.enabled && (
              <div className='flex gap-4'>
                <div className='flex-1'>
                  <label className='block text-xs text-text-muted mb-2'>Webcam Resolution</label>
                  <Select
        ...
        <div className='flex items-center justify-between p-4 border-t border-border bg-background-tertiary'>
          <p className='text-xs text-text-muted'>Recording will start after a 3-second countdown</p>
