...
import { CheckCircle2, Search, SlidersHorizontal, Sparkles, Wand2 } from 'lucide-react';
import {
...
import { EditingTemplateControls, getEditingTemplateDefaultControlValues } from './EditingTemplateControls';

const formatCategoryLabel = (category: string): string => category.replace(/-/g, ' ');
...
export const RecipesTab: React.FC = () => {
  ...
  const applyEditingTemplate = useProjectStore((state) => state.applyEditingTemplate);
  const getSelectedClipIds = useUIStore((state) => state.getSelectedClipIds);

  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<'all' | EditingTemplateCategory>('all');
  ...
  const selectedTrack = useMemo(() => {
    ...
    return project.timeline.tracks.find((track) => track.clips.some((clip) => clip.id === selectedClip.id)) || null;
  }, [project.timeline.tracks, selectedClip]);

  const selectedTargetType =
    selectedTrack?.type === 'image' ? 'image' : selectedTrack?.type === 'video' ? 'video' : null;

  const selectedMedia = selectedClip ? getMediaItem(selectedClip.mediaId) : undefined;
  ...
  useEffect(() => {
    ...
    const expandedTemplate = templates.find((template) => template.id === expandedTemplateId);
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((template) => {
      if (selectedTargetType && template.supportedTargets && !template.supportedTargets.includes(selectedTargetType)) {
        return false;
      }
  ...
  const handleControlChange = (templateId: string, controlId: string, value: EditingTemplatePrimitive): void => {
    setControlValuesByTemplate((current) => ({
      ...current,
      [templateId]: {
        ...(current[templateId] || {}),
  ...
  const handleApply = async (template: EditingTemplate): Promise<void> => {
    if (!selectedClip || !selectedTargetType) {
      toast.warning('Select a clip', 'Recipes apply to one selected video or image clip.');
      return;
    }
    ...
    try {
      const applicationId = applyEditingTemplate(
        template.id,
        selectedClip.id,
        controlValuesByTemplate[template.id] || getEditingTemplateDefaultControlValues(template)
      );

      if (!applicationId) {
        toast.error('Could not apply recipe', 'This recipe could not be applied to the current clip.');
        return;
      }

      toast.success('Recipe applied', `${template.name} was added to ${selectedMedia?.name || 'the selected clip'}.`);
    } finally {
      ...
      <div className='p-4 border-b border-border bg-background-secondary/80 backdrop-blur sticky top-0 z-10 space-y-3'>
        <div className='flex items-center gap-3 bg-background-tertiary rounded-xl p-2 pr-3 border border-border'>
          <div className='w-10 h-10 rounded-lg bg-background-elevated flex items-center justify-center border border-border shrink-0'>
            {selectedTargetType === 'video' ? (
              <span className='text-primary/70 text-[10px]'>VIDEO</span>
            ) : (
              <span className='text-primary/70 text-[10px]'>IMAGE</span>
            )}
          </div>
          <div className='flex-1 min-w-0'>
            <p
              className='text-xs font-semibold text-text-primary truncate'
              title={selectedMedia?.name || selectedClip.id}
            >
        ...
        {filteredTemplates.length === 0 ? (
          ...
          <div className='grid grid-cols-1 gap-3'>
            {filteredTemplates.map((template) => {
              const currentValues =
                controlValuesByTemplate[template.id] || getEditingTemplateDefaultControlValues(template);
              const appliedCount = appliedTemplates.filter((at) => at.templateId === template.id).length;
                    ...
                    <div className='px-3 pb-4 pt-1 bg-background-tertiary border-t border-border/50'>
                      ...
                      <div className='space-y-3 pt-3'>
                        <EditingTemplateControls
                          ...
                          onChange={(controlId, value) => handleControlChange(template.id, controlId, value)}
                        />
                      </div>
                    </div>
                  )}
