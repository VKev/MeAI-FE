...
import { getTranscriptionService, initializeTranscriptionService, type TranscriptWord } from '@meai-editor/core';
...
export const HighlightExtractorPanel: React.FC<HighlightExtractorPanelProps> = ({ clipId }) => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const subtitles = await transcriptionService.transcribeClip(clip, mediaItem, (p) =>
        setProgress(Math.round(p.progress * 20))
      );
      ...
      const results = await extractHighlights(audioBuffer, transcript, preferences, (phaseName, prog) => {
        setPhase(phaseName);
    ...
    <div className='space-y-3'>
      <div className='space-y-2'>
        <div className='flex items-center gap-2'>
          ...
          <input
            ...
            onChange={(e) => setPreferences((p) => ({ ...p, targetClipCount: parseInt(e.target.value) || 5 }))}
          ...
          <input
            ...
            onChange={(e) => setPreferences((p) => ({ ...p, maxClipDuration: parseInt(e.target.value) || 60 }))}
            className='w-12 px-1 py-0.5 text-[10px] bg-background-secondary border border-border rounded text-text-primary'
          />
      ...
      {highlights.length > 0 && (
        <div className='space-y-1.5'>
            ...
            >
              <div className='flex items-center justify-between mb-1'>
                <div className='flex items-center gap-1.5'>
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold text-white ${
                      highlight.score >= 8 ? 'bg-green-500' : highlight.score >= 5 ? 'bg-yellow-500' : 'bg-gray-500'
                    }`}
                  >
                    {highlight.score}
                  </div>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  {selected.has(index) && <Check size={12} className='text-primary' />}
                </div>
              </div>
              <div className='flex items-center justify-between'>
                ...
                <span className='text-[9px] text-text-muted italic truncate max-w-[120px]'>{highlight.reason}</span>
              </div>
            </div>
          ))}
          ...
          <button
            onClick={async () => {
              const selectedHighlights = highlights.filter((_, i) => selected.has(i)).sort((a, b) => a.start - b.start);
              if (selectedHighlights.length === 0) return;

              const store = useProjectStore.getState();
              const proj = store.project;
              const originalTrack = proj.timeline.tracks.find((t) => t.clips.some((c) => c.id === clipId));
              ...
              const clipsToRemove = finalTrack.clips.filter((c) => {
                ...
                return !selectedHighlights.some((h) => h.start < cSourceEnd && h.end > cSourceStart);
              });
