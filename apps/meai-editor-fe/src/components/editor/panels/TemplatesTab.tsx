...
export const TemplatesTab: React.FC = () => {
  ...
  const handleApplyTemplate = useCallback(
    async (templateId: string) => {
      const hasClips = useProjectStore.getState().project.timeline.tracks.length > 0;
      if (hasClips) {
        const confirmed = window.confirm('Applying a template will replace your current project. Continue?');
        if (!confirmed) return;
      }
  ...
  if (loading) {
    return <div className='flex items-center justify-center py-12 text-text-muted text-xs'>Loading templates...</div>;
  }
  ...
  return (
    <div className='px-5 py-4 space-y-3 flex-1 min-h-0 h-full overflow-y-auto bg-background-secondary'>
      ...
      {filteredTemplates.length === 0 ? (
        <div className='text-center py-8 text-text-muted text-xs'>No templates found</div>
      ) : (
            ...
            >
              <div className='w-full aspect-video bg-background-secondary rounded mb-2 flex items-center justify-center'>
                {template.thumbnailUrl ? (
                  <img src={template.thumbnailUrl} alt={template.name} className='w-full h-full object-cover rounded' />
                ) : (
                  <Layout size={20} className='text-text-muted' />
                )}
              </div>
              <span className='text-[10px] font-medium text-text-primary truncate w-full'>{template.name}</span>
              <div className='flex items-center gap-2 mt-1'>
                <span className='text-[9px] text-text-muted capitalize'>{template.category.replace('-', ' ')}</span>
