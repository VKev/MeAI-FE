...
import type { EditingTemplate, EditingTemplatePrimitive } from '@meai-editor/core';

export const getEditingTemplateDefaultControlValues = (
  template: EditingTemplate
): Record<string, EditingTemplatePrimitive> =>
  (template.controls || []).reduce<Record<string, EditingTemplatePrimitive>>((values, control) => {
          ...
          return (
            <div key={control.id} className='space-y-2'>
              <div className='flex items-center justify-between gap-3'>
                <label className='text-[11px] font-medium text-text-primary'>{control.label}</label>
            ...
            <div key={control.id} className='flex items-center justify-between gap-3'>
              ...
              <button
                ...
                className={`inline-flex h-7 w-12 items-center rounded-full px-1 transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                  Boolean(value) ? 'justify-end bg-primary' : 'justify-start bg-background-tertiary'
                }`}
              >
