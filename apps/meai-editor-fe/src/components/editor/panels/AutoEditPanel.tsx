...
export const AutoEditPanel: React.FC<AutoEditPanelProps> = ({ onClose }) => {
  ...
  const [selectedAudioClipId, setSelectedAudioClipId] = useState<string>(audioClips[0]?.id ?? '');
    ...
    try {
      ...
      if (existing && existing.sourceClipId === selectedAudioClipId) {
        beatAnalysis = {
          ...
          beats:
            project.timeline.beatMarkers?.map((m, i) => ({
              time: m.time,
              strength: m.strength,
              index: i
            })) ?? [],
          duration: project.timeline.duration,
          downbeats: project.timeline.beatMarkers?.filter((m) => m.isDownbeat).map((m) => m.time) ?? []
        };
      } else {
        const mediaItem = useProjectStore.getState().project.mediaLibrary.items.find((m) => m.id === audioClip.mediaId);
        if (!mediaItem?.blob) {
      ...
      const result = autoEditService.generateCuts(beatAnalysis, videoClips, options);
      setPreview(result);
    } catch (err) {
  ...
  }, [audioClips, selectedAudioClipId, videoClips, cutMode, sensitivity, minClipDuration, project.timeline]);
  ...
  const handleApply = useCallback(() => {
    ...
    const newClips: Clip[] = preview.cuts
      .map((cut, index) => {
    ...
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-[11px] font-medium text-text-primary'>Beat-Synced Auto-Edit</span>
        </div>
      </div>
      ...
      {audioClips.length === 0 ? (
        ...
        <>
          <div className='space-y-2'>
            <label className='text-[10px] font-medium text-text-secondary'>Audio Source</label>
            <select
          ...
          <div className='space-y-2'>
            <label className='text-[10px] font-medium text-text-secondary'>Cut Mode</label>
          ...
          <div className='space-y-1.5'>
            <div className='flex justify-between'>
              <label className='text-[10px] font-medium text-text-secondary'>Sensitivity</label>
              <span className='text-[9px] text-text-muted'>{Math.round(sensitivity * 100)}%</span>
            </div>
            <Slider min={0} max={1} step={0.05} value={[sensitivity]} onValueChange={(v) => setSensitivity(v[0])} />
          </div>

          <div className='space-y-1.5'>
            <div className='flex justify-between'>
              <label className='text-[10px] font-medium text-text-secondary'>Min Clip Duration</label>
              <span className='text-[9px] text-text-muted'>{minClipDuration.toFixed(1)}s</span>
            </div>
            <Slider
              min={0.1}
              max={3}
          ...
          {preview && (
            <div className='space-y-2 p-2.5 bg-background-tertiary rounded-lg border border-border'>
              <p className='text-[10px] font-medium text-text-primary'>Preview</p>
              <div className='grid grid-cols-2 gap-2 text-[9px]'>
                ...
                <div>
                  ...
                  <span className='text-text-primary'>{preview.totalDuration.toFixed(1)}s</span>
                </div>
              </div>
              <button
                onClick={handleApply}
