...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@meai-editor/ui';
...
export const MasterPasswordDialog: React.FC<MasterPasswordDialogProps> = ({ isOpen, onClose, mode, onSubmit }) => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      setLoading(true);
      try {
        const success = await onSubmit(password, mode === 'change' ? newPassword : undefined);
        if (success) {
          resetForm();
        } else {
          setError(mode === 'unlock' ? 'Incorrect password' : 'Operation failed. Check your current password.');
        }
      } catch (err) {
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              <div className='relative'>
                <Input
                  ...
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder={mode === 'setup' ? 'Min. 8 characters' : 'Enter master password'}
                  autoFocus
                  className='pr-10'
                />
                <button
              ...
              <div className='space-y-2'>
                ...
                <div className='relative'>
                  <Input
                    ...
                    onChange={(e) =>
                      mode === 'change' ? setNewPassword(e.target.value) : setConfirmPassword(e.target.value)
                    }
                    placeholder={mode === 'change' ? 'Min. 8 characters' : 'Repeat password'}
                    className='pr-10'
                  />
                  <button
                    type='button'
          ...
          {mode === 'setup' && (
            <div className='flex items-start gap-2 text-xs text-text-muted bg-background-secondary px-3 py-2 rounded-lg'>
              ...
              <span>
                Your password is used to derive an encryption key via PBKDF2 (100k iterations). API keys are encrypted
                with AES-256-GCM. If you forget this password, stored keys cannot be recovered.
              </span>
            </div>
          )}
