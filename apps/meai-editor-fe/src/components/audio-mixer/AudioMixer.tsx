...
import { getRealtimeAudioGraph } from '@meai-editor/core';
    ...
    <div className='flex flex-col items-center gap-2 p-3 bg-gray-900 rounded-lg min-w-[100px] border border-gray-700'>
      ...
      <div className='flex gap-1 h-32 w-6'>
        <div className='flex-1 bg-gray-800 rounded-sm overflow-hidden relative'>
          <div
            className={`absolute bottom-0 left-0 right-0 transition-all duration-75 ${getColor(levelPercent)}`}
      ...
      <div className='flex flex-col items-center gap-1'>
        <span className='text-xs text-gray-400 font-mono w-12 text-center'>{formatDb(dbValue)} dB</span>
        <input
          type='range'
          min='0'
          max='4'
 ...
 */
export const AudioMixer: React.FC<AudioMixerProps> = ({ visible = true, onClose }) => {
  ...
  const [trackLevels, setTrackLevels] = useState<Record<string, { peak: number; rms: number }>>({});

  // Get audio tracks from the timeline (Requirement 20.1) – safe if project/timeline not ready
  const audioTracks = useMemo(() => {
    const tracks = project?.timeline?.tracks ?? [];
    return tracks.filter((track) => track.type === 'audio' || track.type === 'video');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      audioTracks.forEach((track) => {
        const isAudible = !track.muted && (!hasSoloedTracks || track.solo);
      ...
      if (activeLevels.length > 0) {
        const avgRms = activeLevels.reduce((sum, l) => sum + l.rms, 0) / activeLevels.length;
  ...
  return (
    ...
    >
      ...
      <div className='flex gap-2 overflow-x-auto pb-2'>
        ...
        {/* Separator */}
        {channels.length > 0 && <div className='w-px bg-gray-700 mx-2 self-stretch' />}
      ...
      <div className='mt-3 pt-3 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500'>
        <span>
          ...
          {hasSoloedTracks && <span className='ml-2 text-yellow-500'>• Solo active</span>}
        </span>
        <span>
          Sample Rate: {project.settings.sampleRate}Hz | Channels: {project.settings.channels}
        </span>
      </div>
    </div>
  );
