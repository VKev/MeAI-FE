...
import {
  fetchAdminReports,
  updateAdminReport,
  fetchAdminReportPreview,
  type UpdateAdminReportPayload
...
export default function AdminReports() {
  ...
  const reportMutation = useMutation({
    mutationFn: ({ reportId, payload }: { reportId: string; payload: UpdateAdminReportPayload }) =>
  ...
  const handleAction = (intent: 'start_review' | 'resolve' | 'dismiss' | 'delete_target') => {
    ...
    switch (intent) {
      ...
      case 'delete_target':
        status = 'Resolved';
        action = selectedReport.targetType === 'Comment' ? 'DeleteTargetComment' : 'DeleteTargetPost';
        break;
    }

    reportMutation.mutate({
      reportId: selectedReport.id,
      payload: { status, resolutionNote, action }
    });
  };
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
            <div>
              <h1 className='text-2xl font-bold text-white flex items-center gap-2'>Manage Reports</h1>
            ...
            <div className='flex items-center gap-2'>
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant='outline'
                    className='h-10 border-white/[0.08] bg-white/[0.04] text-slate-300 hover:text-white hover:bg-white/[0.08]'
                  >
              ...
              <table className='w-full text-left'>
                <thead>
                  <tr className='border-b border-white/[0.06] bg-white/[0.02]'>
                    ...
                    <th className='px-5 py-4 text-[11px] font-bold uppercase tracking-widest text-slate-500'>
                      Reporter
                    </th>
                    ...
                    <th className='px-5 py-4 text-[11px] font-bold uppercase tracking-widest text-slate-500 text-right'>
                      Actions
                    </th>
                  </tr>
                </thead>
                  ...
                  {filteredReports.map((report) => {
                    ...
                    return (
                      <tr key={report.id} className='group hover:bg-white/[0.015] transition-colors'>
                        <td className='px-5 py-4'>
                          <div className='flex items-center gap-3'>
                            ...
                            <div>
                              ...
                              <p className='text-[13px] font-medium text-white truncate max-w-[150px]'>
                                {report.targetId}
                              </p>
                            </div>
                          </div>
                        ...
                        <td className='px-5 py-4'>
                          <div className='flex items-center gap-2'>
                            <span className='text-[12px] text-slate-400 font-mono truncate max-w-[100px]'>
                              {report.reporterId}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className='px-5 py-4'>
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border ${STATUS_STYLES[report.status] || STATUS_STYLES.Dismissed}`}
                          >
                        ...
                        <td className='px-5 py-4'>
                          <div className='flex items-center gap-1.5 text-slate-500'>
                            ...
                            <span className='text-[12px]'>
                              {report.createdAt ? format(new Date(report.createdAt), 'MMM dd, HH:mm') : 'N/A'}
                            </span>
                          </div>
                        </td>
              ...
              <DialogHeader>
                <div className='flex items-center gap-3 mb-2'>
                  ...
                  <div className='ml-auto'>
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border ${selectedReport?.status ? STATUS_STYLES[selectedReport.status] || STATUS_STYLES.Dismissed : ''}`}
                    >
                ...
                <div className='space-y-4'>
                  ...
                  <div>
                    ...
                    <div className='bg-white/[0.03] border border-white/[0.06] p-3 rounded-lg space-y-2'>
                      <div className='flex items-center gap-2 text-violet-400'>
                        {(() => {
                          const Icon =
                            (selectedReport?.targetType && TARGET_ICONS[selectedReport.targetType]) || AlertCircle;
                          ...
                          {preview.post && (
                            <div className='space-y-2'>
                              ...
                              {preview.post.media && preview.post.media.length > 0 && (
                                <div className='relative w-full h-24 rounded bg-black/50 overflow-hidden border border-white/[0.04]'>
                                  {preview.post.media[0].resourceType === 'Video' ? (
                                    ...
                                    <img
                                      src={preview.post.media[0].presignedUrl}
                                      alt='media'
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  window.open(
                                    `http://localhost:3030/${preview.post!.username}/post/${preview.post!.id}`,
                                    '_blank'
                                  )
                                }
                              >
                              ...
                              <div className='flex items-center gap-2'>
                                <div className='size-6 rounded-full bg-slate-800 overflow-hidden shrink-0'>
                                  {preview.comment.targetComment.avatarUrl ? (
                                    <img
                                      src={preview.comment.targetComment.avatarUrl}
                                      alt='avatar'
                                      className='w-full h-full object-cover'
                                    />
                                  ) : (
                                ...
                                <span className='text-xs font-bold text-slate-300'>
                                  @{preview.comment.targetComment.username}
                                </span>
                              </div>
                  ...
                  <div>
                    ...
                    <div className='bg-red-500/5 border border-red-500/10 p-3 rounded-lg max-h-[100px] overflow-y-auto'>
                      <p className='text-sm text-slate-300 leading-relaxed italic'>"{selectedReport?.reason}"</p>
                    </div>
                  </div>
                </div>
