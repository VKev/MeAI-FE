  ...
  try {
    ...
    return {
      ...
      error: confirmation.isSuccess
        ? confirmation.value.subscriptionActivated
          ? null
...
export default function StripeResult() {
  ...
  const {
    planId,
    error: initialError,
    subscriptionActivated: initialActivated,
    metadata
  } = useLoaderData<typeof loader>();

  const [isActivated, setIsActivated] = useState(initialActivated);
  const [isPolling, setIsPolling] = useState(
    !initialActivated && initialError !== 'Stripe reported that the payment was not completed.'
  );
