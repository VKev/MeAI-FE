...
const ALL_STATUSES = ['incomplete', 'scheduled', 'succeeded', 'paid', 'active', 'complete'];
...
const STATUS_CONFIG: Record<string, { label: string; cls: string }> = {
  ...
  scheduled: { label: 'Scheduled', cls: 'bg-sky-500/10 text-sky-300' },
...
export default function AdminTransactions() {
  ...
  const failedCount = transactions.filter(
    (t) => t.status.toLowerCase() === 'failed' || t.status.toLowerCase() === 'incomplete'
  ).length;
