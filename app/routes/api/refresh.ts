...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    const res = await axios.post(`${API_URL}/api/User/auth/refresh`, null, {
      headers: {
    ...
    if (setCookie) {
      (Array.isArray(setCookie) ? setCookie : [setCookie]).forEach((c) => headers.append('Set-Cookie', c));
    }
    ...
    if (beStatus === 400 && beData?.type === 'Auth.InvalidRefreshToken') {
