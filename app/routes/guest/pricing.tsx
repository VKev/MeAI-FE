...
export default function Pricing() {
  ...
  const handleSubscribeClick = (planId: string) => {
    if (!user || sessionData?.hasSession === false) {
  ...
  return (
    <div className='min-h-screen bg-[#0a0a0f] relative'>
      ...
      <div className='fixed inset-0 pointer-events-none overflow-hidden'>
        ...
        <div
          className='glow-orb-magenta top-[30%] -right-[5%] opacity-15 animate-pulse-glow'
          style={{ animationDelay: '2s' }}
        />
        <div
          className='glow-orb-cyan top-[60%] -left-[8%] opacity-15 animate-pulse-glow'
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
              {subscriptions.map((plan: Subscription, index: number) => (
                <PricingCard
                  ...
                  hasSession={!!user && sessionData?.hasSession !== false}
                />
              ))}
            </div>
          )}
