...
import type { StripeConfirmPurchaseResponse, SubscriptionChangeType } from '@/models/stripe.model';
...
interface PaymentFormProps {
  ...
  onSuccess: (result: StripeConfirmPurchaseResponse['value']) => void;
  onCancel: () => void;
  lightMode?: boolean;
}
...
export function PaymentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!confirmation.value.subscriptionActivated && !confirmation.value.scheduledChangeCreated) {
        setErrorMessage('Payment is still being confirmed. Please reopen the checkout result page in a moment.');
        setPaymentStatus('error');
        return;
      }

      setConfirmationResult(confirmation.value);
      setPaymentStatus('success');
      setTimeout(() => onSuccess(confirmation.value), 1500);
    } catch (error) {
        ...
        <div className='flex gap-3'>
          ...
          >
            {isProcessing ? (
              ...
              `Start subscription ${formatCurrency(amount, currency)}`
            )}
          </Button>
        </div>
      )}
...
function formatEffectiveDate(value: string | null | undefined) {
  if (!value) {
    return 'after this checkout completes';
  }
