...
function parseConfig(item: TChat) {
...
export function isVideoGenerationItem(item: TChat) {
  const config = parseConfig(item);
  if (config) {
    const type = String(
      config.type ?? config.Type ?? config.generationType ?? config.GenerationType ?? '' ?? ''
    ).toLowerCase();
    if (type === 'video') return true;

    const enableTranslation = config.EnableTranslation ?? config.enableTranslation;
    if (enableTranslation === true) return true;
  }

  const resultUrls = item.resultResourceUrls ?? [];
  const referenceUrls = item.referenceResourceUrls ?? [];
  return [...resultUrls, ...referenceUrls].some((url) => VIDEO_EXTENSIONS.test(url));
}

interface ItemGenerationProps {
  item: TChat;
  isLoading?: boolean;
  isDeleting?: boolean;
...
export default function ItemGeneration({
  item,
  isLoading = false,
  isDeleting = false,
  handleDelete,
  ...
  const isVideo = useMemo(() => isVideoGenerationItem(item), [item]);
  ...
  const handleDownload = async (url: string) => {
    try {
      ...
      a.download = `${item.id}.${isVideo ? 'mp4' : 'png'}`;
      a.click();
    } catch (error) {
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${isVideo ? 'border-sky-500/20 bg-sky-950/5' : 'border-zinc-800 bg-zinc-950'}`}
    >
      <div className='grid gap-5 md:grid-cols-4'>
        <div className={`col-span-2 rounded-xl overflow-hidden ${showMultiGrid ? 'w-full' : 'w-90 h-90 bg-zinc-900'}`}>
          {showMultiGrid ? (
            ...
            isVideo ? (
              <video src={previewUrl} controls muted playsInline className='w-full h-full object-contain bg-black' />
            ) : (
              <img
                src={previewUrl}
                loading='lazy'
          ...
          <div className='flex justify-end items-center gap-2'>
            <Button
              ...
              onClick={(e) => {
                ...
                void handleDownload(previewUrl);
              }}
