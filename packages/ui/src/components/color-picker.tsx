...
import { cn } from '@meai-editor/ui/lib/utils';
...
export const ColorPicker = React.forwardRef<HTMLButtonElement, ColorPickerProps>(
  ({ value, onChange, showAlpha = false, allowTransparent = false, disabled = false, className }, ref) => {
    ...
    const handleColorChange = React.useCallback(
      (event: React.ChangeEvent<HTMLInputElement>) => {
        commit(event.target.value, showAlpha ? (parsed.isTransparent ? 1 : Math.max(parsed.alpha, 0)) : 1);
      },
        ...
        <PopoverTrigger asChild>
          ...
          >
            ...
            >
              ...
              {parsed.isTransparent && <Slash className='absolute inset-0 m-auto h-3 w-3 text-text-muted' />}
            </span>
        ...
        <PopoverContent align='end' className='w-64 space-y-3'>
          <div className='space-y-2'>
            <div className='flex items-center justify-between'>
              <span className='text-[10px] font-medium uppercase tracking-wide text-text-secondary'>Color</span>
              {allowTransparent && (
                <button
                  type='button'
          ...
          {showAlpha && (
            <div className='space-y-2'>
              <div className='flex items-center justify-between text-[10px] text-text-secondary'>
                ...
                <span className='font-mono text-text-muted'>{Math.round(parsed.alpha * 100)}%</span>
              </div>
              <Slider value={[parsed.alpha]} onValueChange={handleAlphaChange} min={0} max={1} step={0.01} />
            </div>
          )}

          <div className='space-y-2'>
            <label className='block text-[10px] font-medium uppercase tracking-wide text-text-secondary'>Value</label>
            <input
              type='text'
              value={textValue}
